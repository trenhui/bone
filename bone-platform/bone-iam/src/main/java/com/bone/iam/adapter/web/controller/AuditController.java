...
public class AuditController {
     ...
     *
     * <p>响应头：{@code Content-Type=text/csv;charset=utf-8} +
     * {@code Content-Disposition=attachment; filename=iam-audit-logs.csv}；
     * 超过 {@value #EXPORT_MAX_SIZE} 条会截断（日志含警告），与账号导出一致。
     */
    ...
    public ResponseEntity<byte[]> export(AuditLogListQuery qry) {
        ...
        for (AuditLogDTO log : result.getRecords()) {
            sb.append(safe(log.getId()))
                    .append(',').append(safe(log.getTenantId()))
                    .append(',').append(safe(log.getUserId()))
                    .append(',').append(escape(log.getOperation() == null ? "" : log.getOperation().name()))
                    .append(',').append(escape(log.getResourceType()))
                    .append(',').append(escape(log.getResourceId()))
                    .append(',').append(escape(log.getIp()))
                    .append(',').append(escape(log.getUserAgent()))
                    .append(',').append(escape(log.getResult()))
                    .append(',').append(safe(log.getDuration()))
                    .append(',').append(escape(log.getCreatedAt() == null ? "" : log.getCreatedAt().format(CSV_TS)))
                    .append('\n');
        }
