...
import com.bone.iam.application.command.cmd.UpdateTenantCmd;
...
public class TenantController {
    ...
    @GetMapping
    public ApiResponse<PageResult<TenantDTO>> list(TenantPageQry qry) {
        return ApiResponse.success(tenantPageQueryHandler.handle(qry));
    }

    @PostMapping
    public ApiResponse<Long> create(@RequestBody CreateTenantCmd cmd) {
    ...
    @PutMapping("/{id}")
    public ApiResponse<Void> update(@PathVariable Long id, @RequestBody UpdateTenantCmd cmd) {
        cmd.setId(id);
