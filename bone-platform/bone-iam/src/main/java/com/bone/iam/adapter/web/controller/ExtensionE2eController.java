...
/**
 * 扩展管理 API 联调占位（仅当 Studio 未部署且显式开启 stub 时生效）。 生产/联调请启动 bone-extension-studio（8088），由网关或 Shell 将
 * {@code /api/v1/extension} 转发至 Studio。
 */
@RestController
...
public class ExtensionE2eController {
  ...
  private static Map<String, Object> plugin(
      Long id, Long extPointId, String name, boolean enabled) {
    Map<String, Object> m = new LinkedHashMap<>();
    m.put("id", id);
