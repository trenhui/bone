...
public class AuthService {
    ...
    public Optional<Account> findByUsername(String username) {
        ...
        try {
            Account account = accountRepository.findOneByCriteria(
    ...
    /**
     * @deprecated 仅保留向后兼容；新调用方应使用 {@link #findByUsername(String)} + {@link #matches(String, Account)}，
     *             以便登录处理器统一编排锁定/失败计数。
     */
    @Deprecated
    public Account authenticate(String username, String password) {
        return findByUsername(username)
                .filter(a -> matches(password, a))
                .orElse(null);
    }
