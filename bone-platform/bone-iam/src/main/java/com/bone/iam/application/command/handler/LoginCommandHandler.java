 ...
 *
 * <p>账号锁定与计数依赖 {@link Account#recordLoginFailure()}，阈值由
 * {@link IamPasswordProperties}（默认 5 次/30 分钟）声明，对齐详设 §7.1 / IAM-19。
 */
@Capability(
    name = "Login",
    description = "账号登录",
...
public class LoginCommandHandler {
    ...
    public Map<String, Object> handle(LoginCommand cmd) {
        Account account = authService.findByUsername(cmd.getUsername())
                .orElseThrow(() -> BizException.of(401,
                        IamErrorCodes.LOGIN_FAILED + ": 用户名或密码错误"));

        if (account.getStatus() == AccountStatus.DISABLED) {
            throw BizException.of(403, IamErrorCodes.ACCOUNT_DISABLED + ": 账号已禁用，请联系管理员");
        }
        if (account.isLocked()) {
            long remainingSec = account.getLockedAt() == null
                    ? 0
                    : Math.max(0, Duration.between(LocalDateTime.now(), account.getLockedAt()).getSeconds());
            throw BizException.of(423,
                    IamErrorCodes.ACCOUNT_LOCKED + ": 账号已锁定，剩余 " + remainingSec + " 秒");
        }

        if (!authService.matches(cmd.getPassword(), account)) {
            account.recordLoginFailure(
                    passwordProperties.getLockoutThreshold(),
                    passwordProperties.getLockoutMinutes());
