package com.bone.iam.infrastructure.tenant;
 ...
 *
 * <p><b>不做默认值兜底</b>：与 metadata-server 的 {@code TenantProviderAdapter} 不同，这里不把缺失的租户 回退成 {@code
 * 1}——IAM 的多张表都以 {@code tenant_id} 为隔离键，凭空造一个租户会让「未认证查询」 被静默翻译成「以某租户身份查询」。缺失就是 {@code
 * null}，由调用方决定放行还是拒绝。
 */
@Component
