...
public class AccountPageQueryHandler {
     ...
     * 解析有效租户过滤：
     * <ul>
     *   <li>JWT/TenantContext 中非平台租户（&gt; 0）→ 强制按其过滤，忽略调用方传参；</li>
     *   <li>平台租户（0L）→ 允许调用方传 {@code qry.tenantId} 显式选择，未传则不加过滤；</li>
     *   <li>无 TenantContext（未走 JWT 链路，例如单元测试）→ 回退到 {@code qry.tenantId}。</li>
     * </ul>
     */
