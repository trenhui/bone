 ...
 *
 * <p>验证：未认证→401、缺权限→403、含权限→200。Token 使用与生产同样的 HS256 验签流程， 仅密钥由 test profile 注入；不依赖 bone-iam 实例。
 */
@SpringBootTest
...
class ConsoleSecurityTest {
  ...
  void overview_with_token_lacking_scope_returns_403() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/console/overview").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
  ...
  void services_with_correct_scope_returns_200() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/console/services").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk());
  }

  @Test
  void quick_actions_without_token_returns_401() throws Exception {
    mockMvc.perform(get("/api/v1/console/quick-actions")).andExpect(status().isUnauthorized());
  }
