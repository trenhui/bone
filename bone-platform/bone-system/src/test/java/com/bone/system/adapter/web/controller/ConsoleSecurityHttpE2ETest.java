 ...
 *
 * <p>用 {@code RANDOM_PORT} 启动整个 Spring Boot Web 容器，{@link TestRestTemplate} 通过 TCP 真打 HTTP 请求，覆盖完整的
 * Filter Chain → SecurityConfig → @PreAuthorize 路径， 输出真实状态码与报文（详设 §5.0 As-Is 验证证据）。
 */
...
class ConsoleSecurityHttpE2ETest {
  ...
  void quick_actions_with_correct_scope_returns_200() {
    ...
    System.out.println(
        "[HTTP-200 quick-actions] status=" + resp.getStatusCode() + " body=" + resp.getBody());
    assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.OK);
  ...
  void actuator_health_is_public() {
    ...
    System.out.println(
        "[HTTP actuator/health] status=" + resp.getStatusCode() + " body=" + resp.getBody());
    assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.OK);
  }
  ...
  void metrics_with_correct_scope_returns_200() {
    ...
    System.out.println(
        "[HTTP-200 metrics] status=" + resp.getStatusCode() + " body=" + resp.getBody());
    assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.OK);
  }
