 ...
 *
 * <p>原则：<b>graceful degradation</b> —— 任一表缺失或查询抛错时把该字段计 0 并 DEBUG，
 * 但绝不让概览整体失败。等价于业界 Operational Dashboard 的"best-effort"模式。
 *
 * <p>查询参考表：
 * <ul>
 *   <li>{@code iam_account}（活跃账号 = {@code deleted=0}）</li>
 *   <li>{@code meta_entity}（{@code deleted=0}）</li>
 *   <li>{@code int_flow}（{@code deleted=0}）</li>
 *   <li>{@code exts_plugin_version where is_active=1}（激活插件数）</li>
 * </ul>
 *
...
public class JdbcKeyMetricsGateway implements KeyMetricsGateway {
