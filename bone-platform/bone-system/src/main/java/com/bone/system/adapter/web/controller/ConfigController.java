...
public class ConfigController {
    ...
    public ApiResponse<Long> create(@Valid @RequestBody CreateConfigReq req) {
        return ApiResponse.success(configCommandHandler.handle(ConfigWebConverter.INSTANCE.toCmd(req)));
    }

    @Operation(summary = "更新配置")
    @PutMapping
    public ApiResponse<Void> update(@Valid @RequestBody UpdateConfigReq req) {
        configCommandHandler.handle(ConfigWebConverter.INSTANCE.toCmd(req));
    ...
    public ApiResponse<PageResult<ConfigResp>> page(ConfigPageReq req) {
        PageResult<ConfigDTO> pageResult = configQueryHandler.page(ConfigWebConverter.INSTANCE.toQry(req));
