 ...
 * 后强制校验。
 *
 * <p>数据由 {@link ConsoleApplicationService} 编排 {@code domain/gateway/*} 出站端口获取。这里返回的是 {@code @Value}
 * 不可变读侧值对象（不是聚合），因此无需再做一层 Resp 映射——它们本来就是为这个契约造的投影。
 */
...
public class ConsoleController {
  ...
  public ApiResponse<ConsoleOverview> overview() {
    return ApiResponse.success(consoleApplicationService.overview());
  }
  ...
  public ApiResponse<List<ServiceStatus>> services() {
    return ApiResponse.success(consoleApplicationService.serviceStatuses());
  }
  ...
  public ApiResponse<ResourceUsage> resources() {
    return ApiResponse.success(consoleApplicationService.resourceUsage());
  }
  ...
  public ApiResponse<KeyMetrics> metrics() {
    return ApiResponse.success(consoleApplicationService.keyMetrics());
  }
  ...
  public ApiResponse<List<QuickAction>> quickActions() {
    return ApiResponse.success(consoleApplicationService.quickActions());
  }
}
