...
/**
 * 解析 {@code Authorization: Bearer <jwt>}，把 {@link JwtPrincipal} 写入 Spring Security {@link
 * SecurityContextHolder}；{@code scopes} 转 GrantedAuthority。
 *
 * <p>解析失败时不阻断请求，交由 {@code authorizeHttpRequests} 与方法级 {@code @PreAuthorize} 给出 401/403。这里不绑定 {@code
 * TenantContext}（控制台读侧不做租户过滤；如未来 metrics 需要租户隔离，再仿照 bone-iam 增加 finally clear()）。
 */
@Component
