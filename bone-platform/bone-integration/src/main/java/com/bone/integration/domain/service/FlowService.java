...
public class FlowService {
    ...
    public List<FlowConnection> getFlowConnections(Long flowId) {
        return connectionRepository.findByCriteria(Criteria.<FlowConnection>create().eq("flowId", flowId));
    }
    ...
    public void validateFlow(IntegrationFlow flow) {
        ...
        List<FlowNode> nodes = nodeRepository.findByCriteria(Criteria.<FlowNode>create().eq("flowId", flow.getId()));
        if (nodes.isEmpty()) {
    ...
    public void deleteNodesByFlowId(Long flowId) {
        List<FlowNode> nodes = nodeRepository.findByCriteria(Criteria.<FlowNode>create().eq("flowId", flowId));
        if (!nodes.isEmpty()) {
