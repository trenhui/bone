...
public class IntegrationExecutionMetrics {
    ...
    public void record(IntegrationLog log) {
        ...
        meterRegistry
                .counter("bone_integration_execution_total", "status", statusTag)
                .increment();
        if (log.getStartedAt() != null && log.getEndedAt() != null) {
            Duration duration = Duration.between(log.getStartedAt(), log.getEndedAt());
            meterRegistry
                    .timer("bone_integration_execution_duration_seconds", "flow_id", String.valueOf(log.getFlowId()))
                    .record(duration);
        }
    }
    ...
    public void refreshDeadLetterGauges(Map<Long, Long> pendingByTenant) {
        pendingByTenant.forEach((tenantId, count) -> deadLetterByTenant
                .computeIfAbsent(tenantId, this::registerDeadLetterGauge)
                .set(count));
