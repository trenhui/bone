...
import ClaimImageAPI, { IImageDoc, IImageType } from "@/api/claimImage";
...
const isAllowEdit = route.query.isAllowEdit === "1";
...
// 是否是初审阶段
const isPreExam = computed(() => {
  return (
    claimInfo.value?.main?.stage === "初审" ||
    claimInfo.value?.main?.stage === "PRE_EXAM"
  );
});

// 控制权限按钮
const hasPermission = computed(() => {
  return !isHangup.value && isPreExam.value && isSameUser.value;
});
...
const handleConfirm = async () => {
  ...
  try {
    await SignAPI.passSign(claimId);
    ElMessage.success("已通过初审, 正在关闭页面中, 若未关闭请手动关闭");
    getClaimInfo();
 ...
 * 挂起赔案
 */
const hangupDialog = ref({
  visible: false,
  params: {
    claimId,
  },
  close: (isRefresh: boolean) => {
    hangupDialog.value.visible = false;
    if (isRefresh) {
      getClaimInfo();
    }
  },
...
const handleHangup = async () => {
  hangupDialog.value.visible = true;
};

/**
 * 挂起记录
 */
const hangupRecordDialog = ref({
  visible: false,
  params: {
    claimNo: "",
  },
  close: () => {
    hangupRecordDialog.value.visible = false;
    hangupRecordDialog.value.params.claimNo = "";
  },
});
const handleShowHangupRecordDialog = () => {
  hangupRecordDialog.value.params.claimNo = claimInfo.value.main?.claimNo ?? "";
  hangupRecordDialog.value.visible = true;
};
...
const getOperatorName = () => {
  const [
    ...
    auditOperatorName,
    reviewOperatorName,
  ] = claimInfo.value?.main?.operatorNames || [];
...
const getReturnReason = () => {
  const rejectReason = claimInfo.value?.main?.rejectReason || "";
  const returnReason = claimInfo.value?.main?.returnReason || "";
...
<template>
  <div class="w-full h-full flex flex-col">
    <div class="flex flex-col flex-1 overflow-hidden bg-[--el-bg-color-page]">
        ...
        >
          <el-tooltip :content="getReturnReason()" raw-content>
            <div class="flex flex-col min-w-0 flex-1 max-w-lg text-red-500">
              <span v-if="claimInfo.main?.rejectReason" class="truncate">
                复核驳回原因：{{ claimInfo.main?.rejectReason }}
              </span>
              <span v-if="claimInfo.main?.returnReason" class="truncate">
                退回原因：{{ claimInfo.main?.returnReason }}
              </span>
            </div>
          </el-tooltip>
          <div class="flex flex-col flex-shrink-0 whitespace-nowrap">
            <span>时效：{{ claimInfo.main?.limitHour }}</span>
            <span>其中挂起时长：{{ claimInfo.main?.hangUpTotalHour }}</span>
          </div>
          <div class="flex flex-col flex-shrink-0 whitespace-nowrap">
            <span>登录账号：{{ claimInfo.main?.loginAccount }}</span>
            <el-tooltip :content="getOperatorName()" raw-content>
              <span>当前作业：{{ claimInfo.main?.currentOperatorName }}</span>
            </el-tooltip>
          </div>
        </div>
      </div>
      ...
      >
        <div
          class="flex justify-between px-6 py-4 border-b border-[var(--el-border-color-lighter)]"
        >
          <!-- 赔案信息 -->
          ...
          <div class="flex-basis-[30%] flex gap-2 flex-wrap button-group">
            ...
            <el-button
              plain
              type="primary"
              @click="handleShowHangupRecordDialog"
            >
              挂起原因
            </el-button>
            <el-button
        ...
        >
          ...
          <el-card
            shadow="never"
            class="image-container relative flex-1 flex flex-col"
          >
    ...
    <!-- 挂起赔案 -->
    <HangupDialog
      v-model="hangupDialog.visible"
      v-bind="hangupDialog.params"
      @close="hangupDialog.close"
    />

    <!-- 挂起记录 -->
    <HangupRecordDialog
      v-model="hangupRecordDialog.visible"
      v-bind="hangupRecordDialog.params"
      @close="hangupRecordDialog.close"
    />
