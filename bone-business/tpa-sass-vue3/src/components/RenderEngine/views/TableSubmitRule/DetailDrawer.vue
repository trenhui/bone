<script setup>
import TableSubmitRuleCreateDialog from "./CreateDialog.vue";
import TableSubmitRuleUpdateDialog from "./UpdateDialog.vue";
import TableRuleAPI from "@/api/rule/tableRule";
import { TableSubmitRuleEnum } from "@/enums/rule/TableSubmitRuleEnum";
import {
  TableSubmitRuleTypeOptions,
  TableSubmitRuleType,
...
defineOptions({
  name: "TableSubmitRuleDetailDrawer",
});
...
const ruleList = ref([]);
const ruleType = ref(TableSubmitRuleType.IN_ROW);

const initRuleList = async () => {
  try {
    if (ruleType.value === TableSubmitRuleType.IN_ROW) {
      ruleList.value = await TableRuleAPI.getSubmitRowRule(props.tableId);
    } else {
      ruleList.value = await TableRuleAPI.getSubmitCrossTableRule(
        props.tableId,
      ...
      <div class="drawer-content">
        ...
        <div class="mt-5">
          <div class="flex items-center mb-3">
            ...
            >
              <el-option
                v-for="item in TableSubmitRuleTypeOptions"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
