...
const ruleKey = TableSubmitRuleEnum.CROSS_TABLE_NUMERIC_MATCH_RULE;
...
const handleComfirm = async () => {
  ...
  try {
    if (isAdding.value) {
      await TableRuleAPI.createSubmitCrossTableRule(rule.value);
      ElMessage.success("添加成功");
      resetRule();
      emits("add", rule.value);
    } else {
      rule.value.id = props.originalRule.id;
      await TableRuleAPI.updateSubmitCrossTableRule(rule.value);
...
const handleDelete = () => {
  ElMessageBox.confirm("确定删除该规则？", "提示", {
    ...
    await TableRuleAPI.deleteSubmitCrossTableRule(props.originalRule.id);
...
.grid-container {
  ...
  grid-template-columns: repeat(8, 1fr); /* 8列布局 */
  gap: 10px; /* 设置列和行之间的间距 */
}
