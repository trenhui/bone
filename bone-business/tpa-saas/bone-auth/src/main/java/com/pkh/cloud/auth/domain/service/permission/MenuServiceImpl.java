...
import com.pkh.cloud.auth.application.enums.permission.MenuTypeEnum;
import com.pkh.cloud.auth.application.vo.MenuListReqVO;
import com.pkh.cloud.auth.application.vo.MenuSaveVO;
import com.pkh.cloud.auth.domain.entity.permission.MenuDO;
import com.pkh.cloud.auth.domain.mapper.permission.MenuMapper;
...
import static com.pkh.cloud.auth.domain.entity.permission.MenuDO.ID_ROOT;
...
public class MenuServiceImpl implements MenuService {
    ...
    public void updateMenu(MenuSaveVO updateReqVO) {
        ...
        if (menuMapper.selectById(updateReqVO.getId()) == null) {
            throw new BizException(MENU_NOT_EXISTS.getMsg());
        }
        // 校验父菜单存在
    ...
    public void deleteMenu(Long id) {
        ...
        if (menuMapper.selectCountByParentId(id) > 0) {
            throw new BizException(MENU_EXISTS_CHILDREN.getCode(),MENU_EXISTS_CHILDREN.getMsg());
        }
        // 校验删除的菜单是否存在
        if (menuMapper.selectById(id) == null) {
            throw new BizException(MENU_NOT_EXISTS.getCode(),MENU_NOT_EXISTS.getMsg());
        }
        // 标记删除
    ...
    void validateParentMenu(Long parentId, Long childId) {
        ...
        if (parentId.equals(childId)) {
            throw new BizException(MENU_PARENT_ERROR.getCode(), MENU_PARENT_ERROR.getMsg());
        }
        MenuDO menu = menuMapper.selectById(parentId);
        // 父菜单不存在
        if (menu == null) {
            throw new BizException(MENU_PARENT_NOT_EXISTS.getCode(), MENU_PARENT_NOT_EXISTS.getMsg());
        }
        // 父菜单必须是目录或者菜单类型
        if (!MenuTypeEnum.DIR.getType().equals(menu.getType())
                && !MenuTypeEnum.MENU.getType().equals(menu.getType()) &&
                !MenuTypeEnum.APP.getType().equals(menu.getType())) {
            throw new BizException(MENU_PARENT_NOT_DIR_OR_MENU.getCode(), MENU_PARENT_NOT_DIR_OR_MENU.getMsg());
        }
    }
    ...
    void validateMenu(Long parentId, String name, Long id) {
        ...
        if (id == null) {
            throw new BizException(MENU_NAME_DUPLICATE.getCode(), MENU_NAME_DUPLICATE.getMsg());
        }
