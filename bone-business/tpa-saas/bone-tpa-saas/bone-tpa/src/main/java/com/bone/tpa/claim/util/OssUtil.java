...
public class OssUtil {
     ...
     * @return
     */
    public static List<String> listFile(String keyPrefix, String bucketName, String endpoint, String accessKeyId, String accessKeySecret, String securityToken) {

        if (StringUtils.isBlank(keyPrefix)) {
            log.error("取指定下级文件夹前缀不能为空");
        ...
        try {
            ...
            ObjectListing listing = ossClientStatic.listObjects(listObjectsRequest);
            return listing.getCommonPrefixes();
        } catch (Exception e) {
            log.error("获取指定下级文件夹[{}]失败 --> {}", keyPrefix, e.getMessage(), e);
            return Collections.emptyList();
        }
    }
    ...
    public List<String> listFile(String fileHost) {
        ...
        for (int i = 0; i < listing.getObjectSummaries().size(); i++) {
            if (listing.getObjectSummaries().get(i).getKey().contains(".")) {
                String FILE_URL = VISITURL + listing.getObjectSummaries().get(i).getKey();
                list.add(FILE_URL);
            }
        }
        // 关闭OSSClient。
    ...
    public String upload(String fileName, InputStream is, String filePath) {
        ...
        Date date = new Date();
        String format = sdf.format(date);
        String uploadFilePath = KEY + format + "/" + fileName + "/" + fileName;
        try {
            // 创建上传Object的Metadata
