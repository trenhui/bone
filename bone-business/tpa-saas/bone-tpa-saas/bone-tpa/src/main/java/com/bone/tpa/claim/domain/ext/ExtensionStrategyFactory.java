...
public class ExtensionStrategyFactory {
     ...
     */
    @Autowired
    public ExtensionStrategyFactory(List<ExtensionStrategy> strategies) {
        // 根据责任类型（LiabilityType）将每个策略与其类型映射
        strategyMap = strategies.stream()
                .collect(Collectors.toMap(
                        ExtensionStrategy::getInsuranceName,  // 获取每个策略支持的责任类型
     ...
     * @return 对应的理算策略
     */
    public ExtensionStrategy getBizModelStrategy(BizModelEnum bizModelEnum) {
        ExtensionStrategy strategy = strategyMap.get(bizModelEnum);
     ...
     * @return 对应的理算策略
     */
    public ExtensionStrategy getBizModelStrategy(String bizModel) {
