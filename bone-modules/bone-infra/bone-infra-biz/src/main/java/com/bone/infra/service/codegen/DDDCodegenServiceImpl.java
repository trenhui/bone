package com.bone.infra.service.codegen;
...
import com.bone.infra.controller.admin.codegen.vo.CodegenCreateListReqVO;
import com.bone.infra.controller.admin.codegen.vo.CodegenUpdateReqVO;
import com.bone.infra.controller.admin.codegen.vo.table.CodegenTablePageReqVO;
import com.bone.infra.controller.admin.codegen.vo.table.DatabaseTableRespVO;
import com.bone.infra.convert.codegen.CodegenConvert;
import com.bone.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.bone.infra.dal.dataobject.codegen.CodegenTableDO;
import com.bone.infra.dal.mysql.codegen.CodegenColumnMapper;
import com.bone.infra.dal.mysql.codegen.CodegenTableMapper;
import com.bone.infra.enums.codegen.CodegenSceneEnum;
import com.bone.infra.service.codegen.inner.CodegenBuilder;
import com.bone.infra.service.codegen.inner.CodegenDDDEngine;
import com.bone.infra.service.codegen.inner.CodegenEngine;
import com.bone.infra.service.db.DatabaseTableService;
...
import static com.bone.infra.enums.ErrorCodeConstants.*;
 ...
 */
@Service
public class DDDCodegenServiceImpl implements CodegenService {
    ...
    @Resource
    private CodegenDDDEngine codegeDDDEngine;
    ...
    public Map<String, String> generationCodes(String basePackage,String module,List<Long> tableIds) {
        // 校验是否已经存在
       // CodegenTableDO table = codegenTableMapper.selectById(tableIds);

        Map<String, String> codes=new HashMap<>();
        List<CodegenTableDO> tables = codegenTableMapper.selectBatchIds(tableIds);
        if (tables == null) {
            throw exception(CODEGEN_TABLE_NOT_EXISTS);
        }
        for (CodegenTableDO table : tables) {
            if(StringUtils.hasText(basePackage)) {
                table.setBasePackage(StringUtils.trimAllWhitespace(basePackage));
            }
            if(StringUtils.hasText(module)) {
                table.setModuleName(StringUtils.trimAllWhitespace(module));
            }
            table.setTableName(StringUtils.trimAllWhitespace(table.getTableName()));
            table.setTableComment(StringUtils.trimAllWhitespace(table.getTableComment()));
            ...
            columns.forEach(item->{
                item.setColumnName(StringUtils.trimAllWhitespace(item.getColumnName()));
                item.setColumnComment(StringUtils.trimAllWhitespace(item.getColumnComment()));
            });

            // 执行生成
            //return codegenEngine.execute(table, columns);
            codes.putAll(codegeDDDEngine.executeDDD(table, columns));
            table.setClassNameVar(lowerFirst(table.getClassName()));
        }

        codes.putAll(codegeDDDEngine.executeDDDTables(module,tables));
