package com.bone.module.system.controller.admin.auth;
...
import static com.bone.base.security.core.util.SecurityFrameworkUtils.obtainAuthorization;
...
public class AuthController {
    ...
    @ApiOperation("使用账号密码登录")
    @OperateLog(enable = false) // 避免 Post 请求被记录操作日志
    public AuthLoginRespVO login2(@RequestBody @Valid AuthLoginReqVO reqVO) {

        //"status":"ok","type":"account","currentAuthority":"admin"
        AuthLoginRespVO authLoginRespVO=authService.login(reqVO);

        authLoginRespVO.setStatus("ok");
        authLoginRespVO.setType("account");
        authLoginRespVO.setCurrentAuthority("admin");
