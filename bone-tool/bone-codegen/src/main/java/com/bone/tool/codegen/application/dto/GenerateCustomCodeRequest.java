...
public class GenerateCustomCodeRequest {
    ...
    @Schema(description = "工程名称", example = "demo-project")
    ...
    @Schema(description = "基础包路径", example = "com.example")
    @NotEmpty(message = "基础包路径不能为空")
    @Size(max = 255, message = "基础包路径长度不能超过255个字符")
    private String basePackage;
    
    @Schema(description = "模板类型", example = "saas", allowableValues = {"saas", "ddd"})
    @NotEmpty(message = "模板类型不能为空")
    private String modelType;
    
    @Schema(description = "生成场景", example = "single", allowableValues = {"single", "batch"})
    ...
    @Schema(description = "表前缀", example = "t_,sys_")
    @Size(max = 255, message = "表前缀长度不能超过255个字符")
    private String tablePrefix;
    
    @Schema(description = "生成前端代码", example = "true")
    private Boolean generateFrontend = true;
    
    @Schema(description = "生成数据库脚本", example = "true")
    private Boolean generateSqlScript = true;
    
    @Schema(description = "覆盖已有文件", example = "false")
