...
interface JoinClause<T, J> {
    ...
    /**
     * 添加where条件，支持主实体的方法引用
     */
    <V> ConditionClause<T> where(FieldFunction<T, V> fieldFunction);
    
    /**
     * 添加where条件，支持连接实体的方法引用
     */
    <V> ConditionClause<T> where(FieldFunction<? super J, V> fieldFunction);
...
public class QueryBuilder {
    ...
    private static class JoinClauseImpl<T, J> implements JoinClause<T, J> {
        ...
        @Override
        public <V> ConditionClause<T> where(FieldFunction<? super J, V> fieldFunction) {
            // 处理连接实体的字段条件
         ...
         * 从连接实体的方法引用中提取字段名
         */
        private <V> String getRightFieldName(FieldFunction<? super J, V> fieldFunction) {
            try {
                // 获取方法引用的类名
