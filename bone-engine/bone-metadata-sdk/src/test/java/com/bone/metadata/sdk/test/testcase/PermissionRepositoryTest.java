...
public class PermissionRepositoryTest {
    ...
    void testFindByIds_ShouldReturnPermissionsWhenIdsExist() {
        ...
        permissions.forEach(permission -> assertTrue(permissionIds.contains(permission.getId()), "Permission ID should be in the requested list"));
    }
    ...
    void testFindByIds_ShouldReturnEmptyListWhenNoIdsExist() {
        ...
        assertTrue(permissions.isEmpty(), "Should return an empty list when no permissions exist with the given IDs");
    }
    ...
    void testInsert_ShouldInsertNewPermission() {
        ...
        assertEquals("NEW_PERMISSION", insertedPermission.getPermName(), "Permission name should be NEW_PERMISSION");
    }
    ...
    void testBatchInsert_ShouldInsertMultiplePermissions() {
        ...
        List<Permission> insertedPermissions = permissionRepository.findByCriteria(
                Criteria.<Permission>create().in("permName", Arrays.asList("BATCH_PERMISSION1", "BATCH_PERMISSION2")));
    ...
    void testUpdate_ShouldUpdateExistingPermission() {
        ...
        assertEquals("UPDATED_CODE", updatedPermission.getPermCode(), "Permission code should be updated");
    }
    ...
    void testUpdateByCriteria_ShouldUpdatePermissionsBasedOnCriteria() {
        ...
        assertEquals("BATCH_UPDATED_CODE", updatedPermission.getPermCode(), "Permission code should be updated");
    }
    ...
    void testBatchSave_ShouldInsertAndUpdatePermissions() {
        ...
        Permission updatedPermission = permissionRepository.findById(1L);
        assertEquals("UPDATED_VIA_BATCH", updatedPermission.getPermCode(), "Existing permission should be updated");
        List<Permission> newPermissions = permissionRepository.findByCriteria(
    ...
    void testFindByCriteria_ShouldReturnPermissionsBasedOnCriteria() {
        ...
        permissions.forEach(permission -> assertEquals("READ", permission.getPermName(), "Permission name should be READ"));
    }
    ...
    void testSetAndGetDataScope_ShouldSetAndRetrieveDataScope() {
        ...
        assertEquals(DataPermission.DataScope.DEPARTMENT, dataPermission.getDataScope(),
    ...
    void testGetDataScope_ShouldReturnDefaultWhenNotSet() {
        DataPermission dataPermission = new DataPermission();
        assertEquals(DataPermission.DataScope.ALL, dataPermission.getDataScope(),
    ...
    void testAddAndGetRowFilters_ShouldAddAndRetrieveRowFilters() {
        ...
        assertEquals("${currentUser.deptId}", filters.get("department_id"),
    ...
    void testSetAndCheckVisibleFields_ShouldSetAndVerifyVisibleFields() {
        ...
        assertTrue(dataPermission.getVisibleFields().contains("name"), "Field 'name' should be visible");
        assertFalse(dataPermission.getVisibleFields().contains("permCode"), "Field 'permCode' should not be visible");
    }
    ...
    void testIsFieldVisible_ShouldReturnTrueWhenNoFieldsSet() {
        ...
        assertTrue(dataPermission.getVisibleFields().contains("id"),
                "All fields should be visible by default when no visible fields are set");
    }

    @Test
    void testThreadSafetyOfExtraProperties_ShouldHandleConcurrentModifications() throws InterruptedException {
        DataPermission dataPermission = new DataPermission();
        Runnable task = () -> {
            for (int i = 0; i < 100; i++) {
        ...
        t1.join();
        t2.join();
        assertEquals(100, dataPermission.getExtraProperties().size(),
    ...
    void testInsertDataPermission_ShouldPersistBaseFieldsOnly() {
        ...
        if (retrieved instanceof DataPermission) {
            ...
            assertEquals(DataPermission.DataScope.ALL, retrievedDataPerm.getDataScope(),
                    "Data scope should reset to default after retrieval");
            assertTrue(retrievedDataPerm.getRowFilter().isEmpty(),
                    "Row filters should be empty after retrieval");
            assertTrue(retrievedDataPerm.getVisibleFields().contains("id"),
