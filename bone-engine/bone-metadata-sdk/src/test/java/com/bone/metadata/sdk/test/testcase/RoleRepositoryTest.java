...
public class RoleRepositoryTest {
  ...
  void testFindByIds_ShouldReturnRolesWhenIdsExist() {
    ...
    roles.forEach(
        role ->
            assertTrue(roleIds.contains(role.getId()), "Role ID should be in the requested list"));
  }
  ...
  void testFindByIds_ShouldReturnEmptyListWhenNoIdsExist() {
    ...
    assertTrue(
        roles.isEmpty(), "Should return an empty list when no roles exist with the given IDs");
  }
  ...
  void testBatchInsert_ShouldInsertMultipleRoles() {
    ...
    List<Role> insertedRoles =
        roleRepository.findByCriteria(
            Criteria.<Role>create().in("role_name", Arrays.asList("BATCH_ROLE1", "BATCH_ROLE2")));
  ...
  void testUpdate_ShouldUpdateExistingRole() {
    ...
    assertEquals(
        "Updated description", updatedRole.getDescription(), "Description should be updated");
  }
  ...
  void testUpdateByCriteria_ShouldUpdateRolesBasedOnCriteria() {
    ...
    assertEquals(
        "Batch updated description", updatedRole.getDescription(), "Description should be updated");
  }
  ...
  void testBatchSave_ShouldInsertAndUpdateRoles() {
    ...
    Role updatedRole = roleRepository.findById(1L);
    assertEquals(
        "Updated via batch save", updatedRole.getDescription(), "Existing role should be updated");
    List<Role> newRoles =
        roleRepository.findByCriteria(Criteria.<Role>create().eq("role_name", "BATCH_SAVE_NEW"));
  ...
  void testFindOneByCriteria_ShouldThrowExceptionWhenMultipleResults() {
    ...
    assertThrows(
        ...
        () -> roleRepository.findOneByCriteria(criteria),
  ...
  void testQueryByCondition_ShouldReturnPagedResultsWithConditions() {
    List<QueryParam> queryParams =
        Collections.singletonList(new QueryParam("roleName", "ADM", Operator.LIKE));
    List<SortingField> sortingFields = Collections.singletonList(new SortingField("id", "ASC"));
    PageResult<Role> pageResult =
        roleRepository.queryByCondition(queryParams, sortingFields, 1, 10, null);
    assertNotNull(pageResult, "Page result should not be null");
