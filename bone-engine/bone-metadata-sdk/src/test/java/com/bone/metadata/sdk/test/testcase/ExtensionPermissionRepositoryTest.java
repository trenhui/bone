...
public class ExtensionPermissionRepositoryTest {
  ...
  @Autowired private FieldMetadataRepository fieldMetadataRepository;

  @Autowired private ColumnAllocator columnAllocator;

  @Autowired private RedissonClient redissonClient; // 注入Redisson客户端
  ...
  private void cleanUpTestData(Long permissionId, List<FieldMetadata> fields) {
    // 删除权限记录（使用命名参数）
    jdbc.update("DELETE FROM sys_permission WHERE id = :id", Map.of("id", permissionId));

    // 删除字段元数据（使用IN子句）
    List<Long> fieldIds = fields.stream().map(FieldMetadata::getId).collect(Collectors.toList());

    jdbc.update("DELETE FROM field_metadata WHERE id IN (:ids)", Map.of("ids", fieldIds));
  }
  ...
  void testContinuousAllocationWithoutExceedingLimit() {
    ...
    for (int i = 0; i < maxLimit; i++) {
      List<FieldMetadata> field = List.of(createFieldMetadata(entityType, "field" + i, dataType));
      assertDoesNotThrow(
          () -> {
  ...
  void testSaveAndLoadExtensionFields_ShouldPersistExtraProperties() {
    ...
    // 预注册字段元数据
    List<FieldMetadata> fieldDefinitions =
        Arrays.asList(
    ...
    // 验证数据库存储
    // Map<String, Object> dbRecord = jdbc.queryForMap("SELECT * FROM sys_permission WHERE id =:id",
    // Map.of("id", id));
  ...
  void testQueryByExtensionField_ShouldFilterCorrectly() {
    ...
    // 预注册字段元数据
    List<FieldMetadata> fieldDefinitions =
        Arrays.asList(createFieldMetadata(entityType, "department", DataType.STRING));
    metadataService.allocateAndPersistFields(fieldDefinitions);
  ...
  void testConcurrentExtensionFieldUpdates_ShouldMaintainConsistency() throws InterruptedException {
    ...
    // 预注册字段元数据
    List<FieldMetadata> fieldDefinitions =
        Arrays.asList(createFieldMetadata(entityType, "counter", DataType.INTEGER));
    metadataService.allocateAndPersistFields(fieldDefinitions);
  ...
  void testUndefinedFieldQuery_ShouldThrowException() {
    Criteria<Permission> criteria = Criteria.<Permission>create().eq("undefinedField", "invalid");
  ...
  void testNullValueHandling_ShouldPersistCorrectly() {
    ...
    // 预注册字段元数据
    List<FieldMetadata> fieldDefinitions =
        List.of(createFieldMetadata(entityType, "nullableField", DataType.STRING));
    metadataService.allocateAndPersistFields(fieldDefinitions);
