...
public class TestDataHelper {
  ...
  public static void setUpPermissionTestData(NamedParameterJdbcOperations jdbc) {
    ...
    // 插入测试数据
    String sql =
        "INSERT INTO sys_permission (id,biz_identity_code, perm_name, perm_code, perm_type, parent_id, path, component, icon, sort_order, created_at, updated_at) "
    ...
    MapSqlParameterSource[] params =
        new MapSqlParameterSource[] {
          createPermissionParams(
              1L, "bone", "READ", "READ", 1, 0L, "/read", "ReadComponent", "read-icon", 1),
          createPermissionParams(
              2L, "bone", "WRITE", "WRITE", 1, 0L, "/write", "WriteComponent", "write-icon", 2),
          createPermissionParams(
              3L,
              "bone",
              "EXECUTE",
          ...
          createPermissionParams(
              ...
              4),
          createPermissionParams(
              5L, "bone", "MANAGE", "MANAGE", 1, 0L, "/manage", "ManageComponent", "manage-icon", 5)
        };
  ...
  private static MapSqlParameterSource createPermissionParams(
      Long id,
  ...
  public static void setUpTestData(NamedParameterJdbcOperations jdbc) {
    cleanTestData(jdbc);
    String sql =
        "INSERT INTO users (id, name, role_id, created_at, created_by, updated_at, updated_by, deleted) "
  ...
  private static MapSqlParameterSource createUserParams(
      Long id, String name, Long roleId, Long createdBy, boolean deleted) {
    // 使用固定的时间值以确保测试稳定性
