...
public class UserRepositoryCriteriaTest {
  ...
  void testFindByIds_ShouldReturnUsersWhenIdsExist() {
    ...
    users.forEach(
        user ->
            assertTrue(userIds.contains(user.getId()), "User ID should be in the requested list"));
  }
  ...
  void testFindByIds_ShouldReturnEmptyListWhenNoIdsExist() {
    ...
    // Assert
    assertTrue(
        users.isEmpty(), "Should return an empty list when no users exist with the given IDs");
  }
  ...
  private LocalDateTime parseDateTime(String timeStr) {
    try {
      ...
      throw new IllegalArgumentException(
          "时间格式错误！请使用 '" + DEFAULT_DATETIME_FORMATTER + "' 格式，输入值: " + timeStr, e);
    }
  }
  ...
  void testFindOneByCriteria_ShouldThrowExceptionWhenMultipleResults() {
    ...
    setUpTestData();
    Criteria<User> criteria =
        Criteria.<User>create().eq(User::getRoleId, 1L); // 假设有多个 role_id 为 1 的用户
    // Act & Assert
    assertThrows(
