...
public class UserRepositoryQueryByConditionTest {
  ...
  @Autowired
  public UserRepositoryQueryByConditionTest(
  ...
  void testQueryByCondition_ShouldReturnPagedResultsWithConditions() {
    ...
    assertEquals(
        ...
        "User role_id should match the query condition");
    // assertEquals("active", pageResult.getRecords().get(0).getStatus(), "User status should match
    // the query condition");
  }
  ...
  void testQueryByCondition_ShouldReturnEmptyPageWhenNoMatches() {
    ...
    assertNotNull(pageResult, "Page result should not be null");
    assertTrue(
        pageResult.getRecords().isEmpty(), "Page should be empty when no users match the criteria");
  }
  ...
  void testQueryByCondition_ShouldReturnSortedResults() {
    // Arrange
    List<QueryParam> queryParams = Arrays.asList(new QueryParam("role_id", 2L, Operator.EQ));
    List<SortingField> sortingFields =
        Arrays.asList(
    ...
    assertTrue(
        pageResult.getRecords().get(0).getName().compareTo(pageResult.getRecords().get(1).getName())
            < 0,
  ...
  void testQueryByCondition_ShouldHandleLargePageNumber() {
    ...
    assertTrue(
        ...
        "Page should be empty for a large page number without enough data");
  }
  ...
  void testQueryByCondition_ShouldReturnUsersForBetweenCondition() {
    ...
    List<QueryParam> queryParams =
        List.of(
            new QueryParam(
                "roleId", Arrays.asList(1, 10), Operator.BETWEEN) // Query for age between 18 and 30
            );
    ...
    assertNotNull(pageResult, "Page result should not be null");
    assertFalse(
        pageResult.getRecords().isEmpty(), "There should be users with age between 18 and 30");
    pageResult
        .getRecords()
        .forEach(
            user -> {
              assertTrue(
                  user.getRoleId() >= 1 && user.getRoleId() <= 10,
                  "User age should be between 18 and 30");
            });
  }
}
