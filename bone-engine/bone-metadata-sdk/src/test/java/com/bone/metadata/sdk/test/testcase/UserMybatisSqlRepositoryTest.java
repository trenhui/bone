...
public class UserMybatisSqlRepositoryTest {
  ...
  @Autowired private UserMybatisSqlRepository userMybatisSqlRepository;
  ...
  class BasicCrudTests {
    ...
    void testBatchInsert_MultipleUsers_InsertsAll() {
      List<User> users =
          Arrays.asList(createUser("User1", 1L, 1001L), createUser("User2", 2L, 1002L));
    ...
    void testDeleteByIds_MultipleUsers_SoftDeletesAll() {
      ...
      List<User> deletedUsers =
          userMybatisSqlRepository.findByIdsIncludingDeleted(Arrays.asList(1L, 2L));
  ...
  class CriteriaTests {
    ...
    void testFindByCriteria_WithConditions_ReturnsMatchingUsers() {
      ...
      assertTrue(
          users.stream().allMatch(u -> u.getRoleId() == 2L), "All users should have role ID 2");
    }
    ...
    void testFindOneByCriteria_WithUniqueCondition_ReturnsSingleUser() {
      ...
      Criteria<User> criteria = Criteria.<User>builder().eq(User::getId, 1L);
    ...
    void testPageByCriteria_WithPaging_ReturnsPagedResults() {
      ...
      Criteria<User> criteria =
          Criteria.<User>builder().page(1, 2).addSort(User::getId, SortDirection.ASC);
  ...
  class GenericQueryTests {
    ...
    void testQueryByCondition_WithParameters_ReturnsResults() {
      ...
      List<SortingField> sortingFields =
          Arrays.asList(new SortingField("created_at", SortingField.ORDER_DESC));

      PageResult<User> page =
          userMybatisSqlRepository.queryByCondition(queryParams, sortingFields, 1, 10, "user");
  ...
  class AggregationTests {
    ...
    void testAggregate_WithGroupBy_ReturnsGroupedResults() {
      ...
      List<Map<String, Object>> results =
          userMybatisSqlRepository.aggregate(aggregations, criteria, groupBy);
      ...
      // 查找 role_id 1 的计数
      Optional<Map<String, Object>> role1Result =
          results.stream()
              .filter(
                  r -> {
      ...
      // 添加对 role_id 2 的验证
      Optional<Map<String, Object>> role2Result =
          results.stream()
              .filter(
                  r -> {
                    Object roleId = r.get("role_id");
    ...
    void testAggregate_WithGroupByAndHaving_ReturnsFilteredResults() {
      ...
      List<Map<String, Object>> results =
          userMybatisSqlRepository.aggregate(aggregations, criteria, groupBy, having);
    ...
    void testAggregateWithPagination_WithGroupBy_ReturnsPagedResults() {
      ...
      PageResult<Map<String, Object>> page =
          userMybatisSqlRepository.aggregateWithPagination(
