...
/** QueryBuilder测试类 - 使用真实数据库进行测试 基于纯Lambda版本实现，遵循业界最佳实践 */
...
public class QueryBuilderTest {
  ...
  private void initTestData() {
    try {
      ...
      String insertUserSql =
          "INSERT INTO users(id, name, role_id, created_at, created_by, updated_at, updated_by, deleted) VALUES(:id, :name, :roleId, :createdAt, :createdBy, :updatedAt, :updatedBy, :deleted)";
      ...
      String insertSalesSql =
          "INSERT INTO sales_record(id, category, amount, price, status, created_at, region, product_name, quantity, is_deleted) VALUES (:id, :category, :amount, :price, :status, :createdAt, :region, :productName, :quantity, :isDeleted)";
  ...
  private Map<String, Object> createUserParams(
      Long id, String name, Long roleId, LocalDateTime now) {
  ...
  private Map<String, Object> createSalesParams(
      Long id,
      String category,
      String amount,
      String price,
  ...
  void testBasicQueryWithWhere() {
    // 测试简单条件查询
    List<User> users = QueryBuilder.from(User.class).where(User::getName).eq("张三").list();
  ...
  void testSingleResultQuery() {
    // 测试单个结果查询
    Optional<User> userOpt = QueryBuilder.from(User.class).where(User::getId).eq(1L).singleOpt();
  ...
  void testSingleResultNotFound() {
    // 测试查询不存在的单个结果
    Optional<User> userOpt = QueryBuilder.from(User.class).where(User::getId).eq(999L).singleOpt();
  ...
  void testFirstResultQuery() {
    ...
    Optional<User> firstUserOptional =
        QueryBuilder.from(User.class).orderByAsc(User::getId).first();
  ...
  void testCountQuery() {
    ...
    // 按条件计数
    long role1Count = QueryBuilder.from(User.class).where(User::getRoleId).eq(10000L).count();
  ...
  void testExistsQuery() {
    // 测试存在性检查
    boolean exists = QueryBuilder.from(User.class).where(User::getName).eq("张三").exists();

    assertTrue(exists);

    boolean notExists = QueryBuilder.from(User.class).where(User::getName).eq("不存在的用户").exists();
  ...
  void testOrderByAsc() {
    // 测试升序排序
    List<User> users = QueryBuilder.from(User.class).orderByAsc(User::getId).list();
  ...
  void testOrderByDesc() {
    // 测试降序排序
    List<User> users = QueryBuilder.from(User.class).orderByDesc(User::getId).list();
  ...
  void testLimitAndOffset() {
    // 测试限制和偏移
    List<User> users =
        QueryBuilder.from(User.class).orderByAsc(User::getId).limit(2).offset(1).list();
  ...
  void testPaginationQuery() {
    // 测试分页查询
    com.bone.core.model.PageResult<User> pageResult =
        QueryBuilder.from(User.class).orderByAsc(User::getId).page(1, 2);
  ...
  void testInCondition() {
    // 测试IN条件
    List<User> users =
        QueryBuilder.from(User.class).where(User::getId).in(Arrays.asList(1L, 2L, 3L)).list();
  ...
  void testNotInCondition() {
    // 测试NOT IN条件
    List<User> users =
        QueryBuilder.from(User.class).where(User::getId).notIn(Arrays.asList(1L, 2L)).list();
  ...
  void testLikeConditions() {
    // 测试各种LIKE条件
    List<User> usersStartsWith =
        QueryBuilder.from(User.class).where(User::getName).startsWith("张").list();
    assertEquals(1, usersStartsWith.size());

    List<User> usersEndsWith =
        QueryBuilder.from(User.class).where(User::getName).endsWith("四").list();
    assertEquals(1, usersEndsWith.size());

    List<User> usersContains =
        QueryBuilder.from(User.class).where(User::getName).contains("五").list();
  ...
  void testComparisonConditions() {
    // 测试比较条件
    List<User> usersGt = QueryBuilder.from(User.class).where(User::getId).gt(2L).list();
    assertEquals(2, usersGt.size());

    List<User> usersGte = QueryBuilder.from(User.class).where(User::getId).gte(2L).list();
    assertEquals(3, usersGte.size());

    List<User> usersLt = QueryBuilder.from(User.class).where(User::getId).lt(3L).list();
    assertEquals(2, usersLt.size());

    List<User> usersLte = QueryBuilder.from(User.class).where(User::getId).lte(3L).list();
  ...
  void testNullConditions() {
    // 测试空值条件
    List<User> usersNotNull = QueryBuilder.from(User.class).where(User::getName).isNotNull().list();
  ...
  void testBetweenCondition() {
    // 测试BETWEEN条件
    List<User> users = QueryBuilder.from(User.class).where(User::getId).between(2L, 3L).list();
  ...
  void testAggregateFunctions() {
    // 测试聚合函数
    Optional<Long> userCount =
        QueryBuilder.from(User.class).aggregate("COUNT", User::getId, Long.class);

    assertTrue(userCount.isPresent());
    assertEquals(4L, userCount.get());

    // 测试带条件的聚合
    Optional<Long> role1Count =
        QueryBuilder.from(User.class)
            .where(User::getRoleId)
            .eq(10000L)
  ...
  void testSalesRecordBasicQuery() {
    ...
    List<SalesRecord> records =
        QueryBuilder.from(SalesRecord.class).where(SalesRecord::getCategory).eq("电子产品").list();
  ...
  void testSalesRecordBooleanCondition() {
    // 测试布尔条件查询
    List<SalesRecord> activeRecords =
        QueryBuilder.from(SalesRecord.class).where(SalesRecord::getIsDeleted).eq(false).list();

    assertEquals(3, activeRecords.size());

    List<SalesRecord> deletedRecords =
        QueryBuilder.from(SalesRecord.class).where(SalesRecord::getIsDeleted).eq(true).list();
  ...
  void testSalesRecordRangeQuery() {
    // 测试范围查询
    List<SalesRecord> records =
        QueryBuilder.from(SalesRecord.class)
            .where(SalesRecord::getAmount)
            .gt(new BigDecimal("1000"))
            .where(SalesRecord::getAmount)
            .lt(new BigDecimal("6000"))
            .list();
  ...
  void testSalesRecordComplexQuery() {
    ...
    List<SalesRecord> records =
        QueryBuilder.from(SalesRecord.class)
            .where(SalesRecord::getCategory)
            .eq("电子产品")
            .where(SalesRecord::getStatus)
            .eq("已完成")
            .where(SalesRecord::getIsDeleted)
            .eq(false)
  ...
  void testStream() {
    // 测试流式处理
    long count =
        QueryBuilder.from(User.class).stream().filter(user -> user.getRoleId() == 10000L).count();
  ...
  void testNonUniqueResultException() {
    ...
    assertThrows(
        Exception.class,
        () -> {
          QueryBuilder.from(User.class).where(User::getRoleId).eq(10001L).single();
        });
  }
  ...
  void testEmptyResult() {
    // 测试空结果
    List<User> users = QueryBuilder.from(User.class).where(User::getName).eq("不存在的用户").list();
