...
/** Repository接口DSL查询方法测试类 测试Repository接口中新增的query()、where()等DSL查询方法 */
...
public class UserRepositoryDSLTest {
  ...
  void testWhereConsumer() {
    List<User> users =
        userRepository
            .where(
                builder -> {
                  builder.and(User::getRoleId).eq(10001L).and(User::getName).like("%王%");
                })
            .list();
  ...
  void testComplexConditions() {
    // 只测试简单条件查询
    List<User> users = userRepository.query().where(User::getRoleId).eq(10001L).list();
    ...
    if (!users.isEmpty()) {
      ...
      List<User> filteredUsers =
          userRepository.query().where(User::getId).eq(users.get(0).getId()).list();
