...
public class ReservedColumnsHandler implements ExtensionStorageHandler {
    ...
    try {
      ...
      distributedLockUtil.executeWithLock(
          ...
          () -> {
            ...
            String sql =
                ReservedQueryBuilder.buildUpsertSQL(
                    ctx, fields, MetadataSdkContext.getDatabaseType());
  ...
  public Map<String, Object> load(ExtensionContext ctx) {
    ...
    CompiledQuery cq =
        new CompiledQuery(ReservedQueryBuilder.buildSelectSQL(ctx, fields), buildSelectParams(ctx));
    ...
    fields.forEach(
        field ->
            result.put(
                field.getName(),
                convertFieldValue(row.get(field.getColumnName()), field.getDataType())));
    return result;
  }

  private List<FieldMetadata> loadOrCreateFields(ExtensionContext ctx) {
    Set<String> names = ctx.getExtraProperties().keySet();
    List<FieldMetadata> existing =
        metadataService.findExtensionFieldsByNames(
            AllocationContext.from(ctx), new ArrayList<>(names));
