...
public class Criteria<T> {
  ...
  /** 设置实体类，用于解析 @Column 注解的列名映射 */
  public Criteria<T> entityClass(Class<?> entityClass) {
  ...
  /** 解析字段名对应的数据库列名，优先使用 @Column 注解映射，回退到驼峰转下划线。 */
  private String resolveColumnName(String fieldName) {
    if (entityClass != null) {
      try {
        java.lang.reflect.Field field = FieldCache.getFieldByName(entityClass, fieldName);
        if (field != null) {
    ...
    return SqlUtil.toSnakeCase(fieldName);
  }

  private Criteria<T> add(
      List<Condition> target, String fieldName, Operator op, boolean ext, Object... vals) {
    // 智能跳过空的IN条件
    ...
    String column = resolveColumnName(fieldName);
    String paramName = generateParamName(column);
    Condition cond =
        new Condition(SqlUtil.toCamelCase(fieldName), column, paramName, op, ext, vals);
    target.add(cond);
    bind(cond);
    return this;
  }
