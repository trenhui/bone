...
public class ColumnAllocator {
    ...
    public List<String> allocate(AllocationContext ctx, DataType type, int count) {
        ...
        // 最终返回所有已分配的列名
        return recycled.stream()
                .map(ColumnAllocation::getColumnName)
     ...
     */
    private List<ColumnAllocation> createNewAllocations(AllocationContext ctx, DataType type, int count) {
        // 通过数据库行锁方式获取连续索引
        long baseIndex = allocateFromDB(ctx, type, count);

        // 构建待插入的 ColumnAllocation 实例列表
        List<ColumnAllocation> toInsert = IntStream.range(0, count)
