...
public final class ReservedQueryBuilder {
  private static final Cache<String, String> SELECT_CACHE =
      Caffeine.newBuilder()
          .maximumSize(2048)
  ...
  private static final Cache<String, String> UPSERT_CACHE =
      Caffeine.newBuilder()
          .maximumSize(2048)
          .expireAfterWrite(Duration.ofHours(4))
          .recordStats()
  ...
  public static String buildUpsertSQL(
      ExtensionContext ctx, List<FieldMetadata> fields, DatabaseType dbType) {
  ...
  private static String buildCacheKey(
      ExtensionContext ctx, String type, List<FieldMetadata> fields) {
  ...
  private static String generateUpsertSQL(List<FieldMetadata> fields, DatabaseType dbType) {
    List<String> keyColumns =
        Arrays.asList("tenant_id", "app_code", "biz_identity_code", "entity_type", "entity_id");
  ...
  private static String buildMySQLUpsert(List<String> columns, List<String> keys) {
    ...
    return String.format(
        ...
        String.join(", ", columns), getNamedParameters(columns), updates);
  }
  ...
  private static String buildPostgresUpsert(List<String> columns, List<String> keys) {
    ...
    return String.format(
        ...
        String.join(", ", columns), getNamedParameters(columns), String.join(", ", keys), updates);
  }
  ...
  private static String buildH2Upsert(List<String> columns, List<String> keys) {
    return String.format(
        ...
        String.join(", ", columns), String.join(", ", keys), getNamedParameters(columns));
  }
  ...
  private static String buildOracleUpsert(List<String> columns, List<String> keys) {
    ...
    String onClause =
        keys.stream().map(k -> "t." + k + " = s." + k).collect(Collectors.joining(" AND "));
    String updates =
        columns.stream()
            .filter(c -> !keys.contains(c))
            .map(c -> "t." + c + " = s." + c)
    ...
    return "MERGE INTO ext_data_reserved t "
        + usingClause
        + " ON ("
        + onClause
        + ") "
