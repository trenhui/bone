...
/**
 * A thread-safe static utility class for caching SQL fragments. Supports both class-specific and
 * global fragment access. - RepositoryFactoryBean initializes fragments from @SqlFragment
 * annotations. - MyBatisSqlProcessor reads fragments and adds fragments from <sql> tags.
 */
public final class FragmentCache {
  ...
  private static final ConcurrentMap<String, Map<String, String>> byClass =
  ...
  public static void putClassFragments(String className, Map<String, String> fragments) {
    ...
    Map<String, String> immutableFragments =
        Collections.unmodifiableMap(new ConcurrentHashMap<>(fragments));
  ...
  public static void putFragment(String className, String fragmentId, String sql) {
    if (!StringUtils.hasText(className)
        || !StringUtils.hasText(fragmentId)
        || !StringUtils.hasText(sql)) {
      logger.warn(
          "Invalid fragment: className={}, fragmentId={}, sql={}", className, fragmentId, sql);
      return;
    }

    Map<String, String> classFragments =
        byClass.computeIfAbsent(className, k -> new ConcurrentHashMap<>());
