...
/** 关联查询构建器实现类 - 纯Lambda版本 基于业界最佳实践，提供完全类型安全的关联查询API */
public class JoinImpl<T, J> implements Join<T, J> {
  ...
  public JoinImpl(
      QueryContext<T> queryContext, FluentQuery<T> fluentQuery, String joinEntityAlias) {
  ...
  @Override
  public <F, JF> FluentQuery<T> on(
      SFunction<T, F> entityFieldGetter, SFunction<J, JF> joinFieldGetter) {
