...
/** {@link DataSourceManager} 接口的默认实现类。 提供管理多个数据源的核心功能，包括数据源注册、切换、健康检查和指标收集。 */
@Component
public class DefaultDataSourceManager implements DataSourceManager, InitializingBean {
  ...
  /**
   * Thread-safe storage for registered data sources. Uses ConcurrentHashMap to ensure thread safety
   * during concurrent access.
   */
  private final Map<String, DataSource> dataSources = new ConcurrentHashMap<>();

  /**
   * Atomic reference to the currently active data source name. Ensures thread-safe updates and
   * reads of the active data source.
   */
  private final AtomicReference<String> currentDataSourceName = new AtomicReference<>();

  /**
   * Name of the default data source to use when no specific data source is specified. Default value
   * is "master".
   */
  private String defaultDataSourceName = "master";

  /**
   * Flag indicating strict mode behavior. When enabled, operations will throw exceptions if
   * requested data sources are unavailable or non-existent. Default value is false (non-strict
   * mode).
   */
  private boolean strictMode = false;

  /**
   * Maximum number of data sources that can be registered. Used to prevent resource exhaustion.
   * Default value is 50.
   */
  ...
  /**
   * Interval between data source health checks in milliseconds. Default value is 30,000 ms (30
   * seconds).
   */
  ...
  private final Map<String, DataSourceHealthStatus> dataSourceHealthStatus =
  ...
  /**
   * 注册一个具有指定名称的新数据源。 如果已存在相同名称的数据源，将被替换。 在严格模式下，该方法会强制执行最大数据源数量限制。
   *
   * @param name 数据源的注册名称
  ...
  public void registerDataSource(String name, DataSource dataSource) {
    ...
    if (dataSources.size() >= maxDataSourceCount) {
      String errorMessage =
          String.format("Maximum data source count exceeded: %d", maxDataSourceCount);
    ...
    if (existing != null) {
      String warningMessage =
          String.format("Data source already exists, registration failed: %s", name);
  ...
  /**
   * 注销具有指定名称的数据源。 如果该数据源当前处于活动状态，将被停用。
   *
  ...
  public boolean unregisterDataSource(String name) {
    ...
    if (removed != null) {
      ...
      if (name.equals(currentDataSourceName.get())) {
        ...
        logger.info(
            "Removed current active data source[{}], current data source set to null",
            previousDataSource);
      } else {
  ...
  /**
   * 获取当前活动的数据源。 如果当前数据源不可用，尝试使用默认数据源。 在严格模式下，如果没有可用的数据源，则抛出异常。
   *
  ...
  public DataSource getCurrentDataSource() {
    ...
    if (dataSource == null) {
      ...
      if (dataSource != null) {
        ...
        logger.info(
            "Current data source[{}] unavailable, automatically switched to default data source: {}",
            currentName,
  ...
  /**
   * Switches the currently active data source. Updates both the internal state and the
  ...
  public boolean isDataSourceHealthy(String name) {
    ...
    try {
      ...
      try (Connection connection = dataSource.getConnection()) {
        boolean healthy =
            connection != null && !connection.isClosed() && connection.isValid(DEFAULT_TIMEOUT_MS);
  ...
  /**
   * 使用指定的数据源执行给定的供应商操作。 处理自动上下文切换、指标记录，并确保正确清理。 即使发生异常，也保证在执行后恢复原始数据源上下文。
   *
  ...
  public <T> T executeWithDataSource(String dataSourceName, Supplier<T> action) {
    ...
    if (strictMode && !dataSources.containsKey(dataSourceName)) {
      throw new IllegalArgumentException(
          String.format("Data source does not exist: %s", dataSourceName));
    }
  ...
  /**
   * 使用指定的数据源异步执行给定的可运行操作。 委托给DataSourceContextHolder实现核心逻辑，同时保留指标记录功能。
   *
   * @param dataSourceName 用于执行的数据源名称
   ...
   * @throws IllegalArgumentException 如果数据源名称无效或操作为null
   */
  public CompletableFuture<Void> executeAsyncWithDataSource(
      String dataSourceName, Runnable action) {
    Assert.notNull(dataSourceName, "Data source cannot be null");
    Assert.hasText(dataSourceName, "Data source name cannot be empty");
    Assert.notNull(action, "Execution action cannot be null");
  ...
  /**
   * 使用指定的数据源执行给定的可运行操作。 委托给DataSourceContextHolder实现核心逻辑，同时保留指标记录功能。
   *
   * @param dataSourceName 用于执行的数据源名称
   * @param action 要使用指定数据源执行的可运行操作
   * @throws IllegalArgumentException 如果数据源名称无效或操作为null
  ...
  /**
   * 使用指定的数据源异步执行给定的供应商操作。 委托给DataSourceContextHolder实现核心逻辑，同时保留指标记录功能。
   *
   * @param <T> 返回值类型
   * @param dataSourceName 用于执行的数据源名称
   * @param action 要使用指定数据源执行的供应商操作
   ...
   * @throws IllegalArgumentException 如果数据源名称无效或操作为null
   */
  public <T> CompletableFuture<T> executeAsyncWithDataSource(
      String dataSourceName, Supplier<T> action) {
    Assert.notNull(dataSourceName, "Data source cannot be null");
    Assert.hasText(dataSourceName, "Data source name cannot be empty");
    Assert.notNull(action, "Execution action cannot be null");
  ...
  /** 关闭健康检查调度器并释放相关资源。 当不再需要管理器时，应调用此方法以防止资源泄漏。 执行带有超时的优雅关闭，必要时进行强制关闭。 */
  ...
  /**
   * 设置默认数据源的名称。 如果当前未设置活动数据源，这将成为活动数据源。
   *
  ...
  /**
   * 设置严格模式标志。 在严格模式下，如果找不到数据源或数据源不可用，操作将抛出异常。
   *
  ...
  /**
   * 设置允许的最大数据源数量。 尝试注册超过此限制的数据源将被拒绝。
   *
  ...
  public void afterPropertiesSet() {
    logger.info(
        "DefaultDataSourceManager已初始化 - 严格模式: {}, 默认数据源: {}, 最大数据源数量: {}",
        strictMode,
        defaultDataSourceName,
        maxDataSourceCount);
  ...
  /** 使用守护线程初始化健康检查调度器。 为所有已注册的数据源设置定期健康检查。 */
    ...
    healthCheckScheduler =
        Executors.newSingleThreadScheduledExecutor(
            r -> {
              ...
              thread.setDaemon(
                  true); // Set as daemon thread to avoid affecting application shutdown
              return thread;
            });
  ...
  /** 对所有已注册的数据源执行健康检查。 记录健康检查过程的统计信息。 */
  private void performHealthChecks() {
    ...
    for (String dataSourceName : new ArrayList<>(dataSources.keySet())) {
      try {
        ...
        logger.error(
            "Data source[{}] health check exception: {}", dataSourceName, e.getMessage(), e);
      }
    }

    long endedAt = System.currentTimeMillis();
    logger.info(
        "健康检查完成 - 已检查数据源: {}, 健康数据源: {}, 耗时: {}ms",
        checkedCount,
        healthyCount,
        (endedAt - startedAt));
  }

  /** 跟踪数据源健康状态的内部类。 维护健康状态、检查时间戳和失败计数。 */
  private static class DataSourceHealthStatus {
    ...
    /**
     * 更新数据源的健康状态。 此方法是线程安全的，以确保状态更新的一致性。
     *
    ...
    public String toString() {
      return String.format(
          "DataSourceHealthStatus{name='%s', healthy=%s, consecutiveFailures=%d, lastCheckTime=%d}",
          dataSourceName, healthy, consecutiveFailures, lastCheckTime);
    }
  }

  /** 收集数据源使用指标的内部类。 跟踪访问计数、查询时间和性能统计信息。 */
  private static class DataSourceMetrics {
    ...
    /**
     * 记录数据源访问及其执行时间。 此方法是线程安全的，以确保指标收集的准确性。
     *
    ...
    public String toString() {
      return String.format(
          "DataSourceMetrics{name='%s', accessCount=%d, avgQueryTime=%dms, maxQueryTime=%dms, minQueryTime=%dms}",
          dataSourceName,
          totalAccessCount,
          getAverageQueryTime(),
          getMaxQueryTime(),
