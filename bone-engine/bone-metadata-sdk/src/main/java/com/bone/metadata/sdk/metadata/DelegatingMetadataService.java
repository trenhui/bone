...
public class DelegatingMetadataService implements MetadataService, ApplicationContextAware {
  ...
  public DelegatingMetadataService(
  ...
  private synchronized void refreshActiveDelegate() {
    ...
    try {
      MetadataService newDelegate =
          applicationContext.getBean(
              targetMode.equals(DeploymentMode.REMOTE)
  ...
  private void handleDelegateException(DeploymentMode mode, Exception ex) {
    ...
    DeploymentMode fallbackMode =
        (mode == DeploymentMode.REMOTE) ? DeploymentMode.EMBEDDED : DeploymentMode.REMOTE;

    try {
      log.warn("Attempting fallback to {} mode", fallbackMode);
      MetadataService fallbackService =
          applicationContext.getBean("embeddedMetadataService", MetadataService.class);
