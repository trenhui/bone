...
/**
 * 表示 SQL 模板的实体类，包含模板内容、参数、元数据和处理/执行所需的属性。 该类设计为部分不可变（通过 Builder 构建），支持序列化以用于分布式缓存，
...
public class SqlTemplate implements Serializable {
  ...
  // 检测动态 SQL 标签的正则表达式
  public static final Pattern DYNAMIC_TAG_PATTERN =
      Pattern.compile("<(where|if|foreach)(?:\\s+[^>]*)?>", Pattern.CASE_INSENSITIVE);
  ...
  @Builder.Default private SqlTemplateType sqlTemplateType = SqlTemplateType.SQL;

  /** SQL 操作类型，例如 SELECT, INSERT, UPDATE, DELETE, CTE, UNKNOWN。 */
  @Builder.Default private SqlType sqlType = SqlType.UNKNOWN; // "UNKNOWN";
      ...
      parameters.forEach(
          (key, value) -> {
            ...
            if (value instanceof String str
                && (str.contains(";") || str.matches("(?i).*\\b(SELECT|DROP)\\b.*"))) {
