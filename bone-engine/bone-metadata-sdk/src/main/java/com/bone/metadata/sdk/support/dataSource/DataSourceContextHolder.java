...
/**
 * 线程安全的数据源上下文管理器，用于在不同操作之间管理数据源标识符。 使用ThreadLocal<Deque<String>>实现LIFO栈结构，支持嵌套数据源切换场景，
...
public final class DataSourceContextHolder {
  ...
  /**
   * 创建当前线程数据源上下文栈的深拷贝 对于异步操作正确继承父线程数据源上下文至关重要
   *
  ...
  /**
   * 恢复之前保存的数据源上下文快照 用于向新线程传播上下文或恢复之前的状态
   *
  ...
  /**
   * 使用指定数据源安全执行带返回值的supplier操作 确保无论执行结果如何，都能正确设置和清理上下文
   *
   * @param <T> 操作的返回类型
   ...
   */
  private static CompletableFuture<Void> doExecuteAsync(
      String dataSource, Executor executor, Runnable action, boolean withExecutor) {
    // 保存当前线程的数据源上下文快照
   ...
   */
  private static <T> CompletableFuture<T> doExecuteAsyncWithResult(
      String dataSource, Executor executor, Supplier<T> action, boolean withExecutor) {
    // 保存当前线程的数据源上下文快照
    Deque<String> currentContext = createContextSnapshot();
  ...
  /**
   * 使用指定数据源异步执行runnable操作 保留并传播父线程的数据源上下文到异步任务
   *
   * @param dataSource 用于执行的数据源标识符
   ...
   * @throws NullPointerException 如果数据源标识符或操作对象为null
   */
  public static CompletableFuture<Void> executeAsyncInDataSource(
      String dataSource, Runnable action) {
    Objects.requireNonNull(dataSource, "数据源不能为空");
    Objects.requireNonNull(action, "操作不能为空");
  ...
  /**
   * 在指定数据源上异步执行带返回值的操作 确保异步任务继承父线程的数据源上下文
   *
   * @param <T> 返回类型
   * @param dataSource 数据源标识符
   * @param action 要执行的操作
   ...
   * @throws NullPointerException 如果数据源标识符或操作对象为null
   */
  public static <T> CompletableFuture<T> executeAsyncInDataSourceWithResult(
      String dataSource, Supplier<T> action) {
    Objects.requireNonNull(dataSource, "数据源不能为空");
    Objects.requireNonNull(action, "操作不能为空");
   ...
   */
  public static CompletableFuture<Void> executeAsyncInDataSource(
      String dataSource, Executor executor, Runnable action) {
    Objects.requireNonNull(dataSource, "数据源不能为空");
   ...
   */
  public static <T> CompletableFuture<T> executeAsyncInDataSourceWithResult(
      String dataSource, Executor executor, Supplier<T> action) {
    Objects.requireNonNull(dataSource, "数据源不能为空");
    Objects.requireNonNull(executor, "执行器不能为空");
    Objects.requireNonNull(action, "操作不能为空");
   ...
   */
  public static void executeInDataSourceWithFallback(
      String dataSource, Runnable action, Runnable fallback) {
    Objects.requireNonNull(dataSource, "数据源不能为空");
    Objects.requireNonNull(action, "操作不能为空");
   ...
   * @throws NullPointerException 如果数据源标识符或主操作对象为null
   */
  public static <T> T executeInDataSourceWithResultAndFallback(
      String dataSource, Supplier<T> supplier, Supplier<T> fallbackSupplier) {
    Objects.requireNonNull(dataSource, "数据源不能为空");
  ...
  /**
   * 尝试按顺序在多个数据源上执行操作，直到成功为止 实现简单的故障转移机制，当一个数据源失败时尝试下一个
   *
   * @param <T> 操作的返回类型
  ...
  /**
   * 从提供的列表中随机选择一个数据源来执行操作 使用ThreadLocalRandom实现简单的轮询负载均衡方法
   *
   * @param <T> 操作的返回类型
   * @param action 要执行的supplier操作
