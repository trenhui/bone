...
/**
 * 统一SQL模板加载器，支持多源（注解、类路径）加载和回退机制。 支持路径： - sql/<package>/<RepositoryClass>/<methodName>.sql 或
 * sql/<module>/<methodName>.sql - sql-templates/<RepositoryClass>/<methodName>.yaml 或
 * sql-templates/<module>/<methodName>.yaml
 */
@Slf4j
public class UnifiedSqlTemplateLoader implements SqlTemplateLoader {
  ...
  /** 构造函数，初始化缓存。 */
  public UnifiedSqlTemplateLoader(
    ...
    this.templateCache =
        Caffeine.newBuilder()
            .maximumSize(config.getTemplate().getCacheSize())
            .expireAfterWrite(config.getTemplate().getExpireHours(), TimeUnit.HOURS)
            .build();
  ...
  public SqlTemplate loadTemplate(Method method, String templateId) throws TemplateLoadException {
    return templateCache.get(
        templateId,
        id -> {
          List<TemplateDescriptor> descriptors =
              descriptorCache.computeIfAbsent(templateId, this::generateCandidateDescriptors);
          ...
          for (TemplateDescriptor descriptor : descriptors) {
            try {
              ...
              throw new TemplateLoadException(
                  "加载模板失败: id=" + templateId + ", source=" + descriptor.getSourceUri(), e);
            }
          }
  ...
  private SqlTemplate loadTemplateWithFallback(Method method, TemplateDescriptor descriptor)
      throws TemplateLoadException {
    securityValidator.validateDescriptor(descriptor);
    TemplateSourceProvider provider =
        sourceProviders.stream()
            .filter(p -> p.supports(URI.create(descriptor.getSourceUri())))
            .findFirst()
            .orElseThrow(() -> new TemplateLoadException("无支持的源提供器: " + descriptor.getSourceUri()));
    ...
    TemplateContentParser parser =
        contentParsers.stream()
  ...
  /**
   * 生成候选模板描述符列表，支持多源回退。 支持两种模板 ID 格式： - 标准格式：package.ClassName.methodName -
  ...
  private List<TemplateDescriptor> generateCandidateDescriptors(String templateId) {
    ...
    if (!templateId.contains(".") && !templateId.contains("/")) {
      throw new IllegalArgumentException(
          "模板ID格式无效，期望格式 'package.ClassName.methodName' 或 'module/methodName': " + templateId);
    }
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          ...
          templateId,
          descriptors.size(),
          descriptors.stream().map(TemplateDescriptor::getSourceUri).collect(Collectors.toList()));
    }
    return descriptors;
  }
  ...
  private TemplateDescriptor createAnnotationDescriptor(String templateId) {
    return TemplateDescriptor.builder()
        ...
        .sourceUri(ANNOTATION_PREFIX + templateId)
        .format(SqlTemplateType.MYBATIS)
        .tags(Collections.singletonMap("source", "annotation"))
        .version("1.0")
        .build();
  }

  /**
   * 创建类路径源描述符，支持 sql/ 和 sql-templates/ 路径。 支持两种模板 ID 格式： - package.ClassName.methodName（如
   * com.example.UserRepository.findById） - module/methodName（如 user/searchUsersPaged）
   */
  private List<TemplateDescriptor> createClasspathDescriptors(String templateId) {
    ...
    if (templateId.contains("/")) {
      ...
      if (parts.length == 2) {
        ...
        String sqlPath =
            String.format(
                "%s%s/%s%s",
                config.getTemplate().getBasePath(), moduleName, methodName, SQL_EXTENSION);
        descriptors.add(
            TemplateDescriptor.builder()
                .templateId(templateId)
                .sourceUri(sqlPath)
                .format(SqlTemplateType.MYBATIS)
                .tags(Collections.singletonMap("pathStyle", "sql-module"))
                .version("1.0")
                .build());
        ...
        String yamlPath =
            String.format(
                "%s%s/%s%s",
                config.getTemplate().getYamlPath(), moduleName, methodName, YAML_EXTENSION);
        descriptors.add(
            TemplateDescriptor.builder()
                .templateId(templateId)
                .sourceUri(yamlPath)
                .format(SqlTemplateType.YAML_SQL)
                .tags(Collections.singletonMap("pathStyle", "yaml-module"))
                .version("1.0")
                .build());
      }
      return descriptors;
    ...
    String packagePath =
        classDotIndex > 0 ? className.substring(0, classDotIndex).replace('.', '/') : "";
    ...
    String sqlPath =
        String.format(
            "%s%s/%s/%s%s",
            config.getTemplate().getBasePath(),
            packagePath,
            simpleClassName,
            methodName,
            SQL_EXTENSION);
    descriptors.add(
        TemplateDescriptor.builder()
            ...
            .sourceUri(sqlPath)
            .format(SqlTemplateType.MYBATIS)
            .tags(Collections.singletonMap("pathStyle", "sql"))
            .version("1.0")
            .build());
    ...
    String yamlPath =
        String.format(
            "%s%s/%s%s",
            config.getTemplate().getYamlPath(), simpleClassName, methodName, YAML_EXTENSION);
    descriptors.add(
        TemplateDescriptor.builder()
            .templateId(templateId)
            .sourceUri(yamlPath)
            .format(SqlTemplateType.YAML_SQL)
            .tags(Collections.singletonMap("pathStyle", "yaml"))
            .version("1.0")
            .build());
    ...
    if (config.getTemplate().isAsyncPreload() && !config.getTemplate().getPreload().isEmpty()) {
      try {
        ...
        if (log.isInfoEnabled()) {
          log.info(
              "预加载 {} 个模板: {}",
              config.getTemplate().getPreload().size(),
              config.getTemplate().getPreload());
        }
      } catch (Exception e) {
