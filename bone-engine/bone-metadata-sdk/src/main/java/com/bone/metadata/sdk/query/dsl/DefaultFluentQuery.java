...
/** 默认流畅查询实现 - 纯Lambda版本 基于业界最佳实践，提供完全类型安全的API，复用现有组件 */
public class DefaultFluentQuery<T> implements FluentQuery<T> {
  ...
  @Override
  public <J> FluentQuery<T> joinOn(
      Class<J> joinClass,
      String joinAlias,
      Function<T, ?> entityFieldGetter,
      Function<J, ?> joinFieldGetter) {
    validateJoinAlias(joinAlias);
  ...
  @Override
  public <J> FluentQuery<T> leftJoinOn(
      Class<J> joinClass,
      String joinAlias,
      Function<T, ?> entityFieldGetter,
      Function<J, ?> joinFieldGetter) {
  ...
  private <J> Join<T, J> createJoin(
      Class<J> joinClass, String joinAlias, QueryContext.JoinType joinType) {
  ...
  @Override
  public <R> Optional<R> aggregate(
      String function, SFunction<T, ?> fieldGetter, Class<R> resultType) {
    String fieldName = SqlUtil.extractFieldName(fieldGetter);
  ...
  private String buildAggregateSql(String function, String fieldName, SqlBuilder<T> sqlBuilder) {
    ...
    return "SELECT "
        ...
        + fieldName
        + ") FROM "
        + getTableName(queryContext.getEntityClass());
  }
