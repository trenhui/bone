...
/** 动态 SQL 处理类，支持 where、if 和 foreach 等标签的解析和处理。 增强版：支持 ${} 占位符替换 */
@Slf4j
  ...
  private void validatePlaceholderValue(String placeholder, String value) {
    ...
    // 其他占位符的通用验证
      ...
      throw new SqlProcessingException(
          "Potential SQL injection detected in placeholder: " + placeholder);
    }
  }
  ...
  static class WhereHandler implements DirectiveHandler {
    public void handle(TagNode node, SqlContext context) {
      context.pushClosure(
          content -> {
            String trimmed =
                content.trim().replaceAll("(?i)^AND\\s+", "").replaceAll("(?i)^OR\\s+", "");
  ...
  static class IfHandler implements DirectiveHandler {
    ...
    public void handle(TagNode node, SqlContext context) {
      ...
      try {
        ...
        log.warn(
            "SpEL evaluation failed for expression: {}. Treating as false. Error: {}",
            testExpr,
            e.getMessage());
      }
