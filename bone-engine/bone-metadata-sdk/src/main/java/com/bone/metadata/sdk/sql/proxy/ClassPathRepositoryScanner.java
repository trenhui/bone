...
class ClassPathRepositoryScanner extends ClassPathBeanDefinitionScanner {
  ...
  protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
    logger.info(
        "Scanning for Repository interfaces in packages: {}", Arrays.toString(basePackages));
    ...
    if (holders.isEmpty()) {
      logger.warn(
          "No Repository interfaces found with standard scanning. Trying alternative approach...");
  ...
  private Set<BeanDefinitionHolder> alternativeScan(String... basePackages) {
    ...
    for (String basePackage : basePackages) {
      try {
        ...
        Resource[] resources =
            ((ResourcePatternResolver) getResourceLoader()).getResources(packageSearchPath);
  ...
  private void processBeanDefinition(BeanDefinitionHolder holder) {
    ...
    try {
      ...
      if (!isRepository) {
        logger.warn(
            "Interface {} is not a Repository (does not implement Repository interface)",
            repoInterfaceName);
        return;
      }
      ...
      if (genericTypes == null || genericTypes.length != 2) {
        logger.error(
            "Invalid Repository interface: {} (must extend Repository<T, ID>)", repoInterfaceName);
        return;
      }
      ...
      logger.info(
          "Registered Repository: {} (entity: {}, id: {})",
          repoInterfaceName,
          genericTypes[0].getSimpleName(),
          genericTypes[1].getSimpleName());
