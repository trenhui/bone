...
/**
 * High-performance MyBatis-compatible SQL processor. Supports dynamic SQL syntax with multi-level
...
public class MyBatisSqlProcessor implements SqlProcessor {
  ...
  private static final Pattern INCLUDE_PATTERN =
      Pattern.compile(
          "<include\\s+refid\\s*=\\s*['\"]([^'\"]+)['\"]\\s*/>", Pattern.CASE_INSENSITIVE);
  ...
  private void validateSpelExpressions(String sqlTemplate) {
    ...
    if (matcher.find()) {
      throw new SqlProcessingException(
          "Incomplete SpEL expression detected. Found: '"
              + matcher.group()
  ...
  private String generateCacheKey(String templateId, String sqlTemplate) {
    return templateId
        + "_"
        + DigestUtils.md5DigestAsHex(sqlTemplate.getBytes(StandardCharsets.UTF_8));
  }

  private void extractSqlFragments(String templateId, String sqlTemplate) {
    String className =
        (templateId != null && templateId.contains("."))
    ...
    while (matcher.find()) {
      ...
      log.debug(
          "Extracted SQL fragment: class={}, id={}, content={}",
          className,
          fragmentId,
          fragmentContent);
    }
  ...
  private String processIncludeTags(String templateId, String sqlTemplate) {
    ...
    while (matcher.find()) {
      if (depth++ > MAX_DEPTH) {
        throw new SqlProcessingException(
            "Maximum include depth exceeded for template: " + templateId);
      }
    ...
    switch (tagName.toLowerCase()) {
      ...
      case "trim":
        return new TrimNode(
            ...
            content);
      case "bind":
        return new BindNode(attrs.get("name"), attrs.get("value"), content);
      default:
  ...
  private boolean evaluateSpel(String expression, Map<String, Object> params) {
    try {
      ...
      // 设置属性访问器
      evalContext.setPropertyAccessors(
          Arrays.asList(new MapAccessor(), new ReflectivePropertyAccessor()));

      // 注册自定义函数
      try {
        evalContext.registerFunction(
            "isEmpty", SqlFunctions.class.getDeclaredMethod("isEmpty", Object.class));
        evalContext.registerFunction(
            "isNotEmpty", SqlFunctions.class.getDeclaredMethod("isNotEmpty", Object.class));
        evalContext.registerFunction(
            "contains",
          ...
          if (current instanceof Map) {
            ...
            try {
              ...
              throw new AccessException(
                  "Cannot access property '" + parts[i] + "' on object: " + current);
            }
          }
        }
  ...
  private void validateParameters(Map<String, Object> params) {
    ...
    for (Map.Entry<String, Object> entry : params.entrySet()) {
      ...
      if (entry.getValue() instanceof String str) {
        if (str.contains(";")
            || str.matches("(?i).*\\b(SELECT|DROP|INSERT|UPDATE|DELETE|EXEC)\\b.*")) {
      ...
      if ("tableName".equals(entry.getKey())
          && !properties
              .getSecurity()
              .getAllowedTables()
              .contains(String.valueOf(entry.getValue()))) {
  ...
  private void validateSql(String sql) {
    ...
    while (quoteMatcher.find()) {
      ...
      if (content.contains("%")) {
        ...
        if (safeMatcher.find()
            && safeMatcher.regionStart() < quoteMatcher.start()
      ...
      if (content.contains("'")
        ...
        log.warn(
            "Potential SQL injection risk detected, SQL: [{}], risky content: [{}]", sql, matched);
  ...
  static class SqlContext {
    ...
    public SqlContext(
        MyBatisSqlProcessor processor, SqlConfigProperties properties, Map<String, Object> params) {
      ...
      while (matcher.find()) {
        ...
        if (hashParam != null) {
          ...
          if (!ALLOWED_DOLLAR_PATTERN.matcher(valueStr).matches()) {
            throw new SqlInjectionRiskException(
                "Invalid ${" + dollarParam + "} value: " + valueStr);
          }
