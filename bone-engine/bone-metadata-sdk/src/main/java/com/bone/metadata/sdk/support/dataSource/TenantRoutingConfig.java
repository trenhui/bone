...
/** 多租户数据源自动路由配置 提供不依赖Servlet API的多租户数据源切换工具类 用户可以根据自己的框架适配这个工具类 */
public class TenantRoutingConfig {

  /** 多租户数据源工具类 不直接依赖Servlet API，提供通用的租户数据源切换功能 */
  @Component
  ...
  /**
   * Spring Web框架适配器示例（注释掉，由用户根据需要启用） 要使用此适配器，用户需要： 1. 取消注释此类 2. 添加servlet-api依赖 3. 确保Spring
   * Web在类路径中
   */
  /*
  @Configuration
