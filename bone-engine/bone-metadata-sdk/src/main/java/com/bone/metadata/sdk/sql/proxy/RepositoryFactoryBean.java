...
/** Repository 代理工厂 Bean，基于业界最佳实践实现 支持注解和类路径 SQL 模板加载，集成多源回退机制 */
@Slf4j
public class RepositoryFactoryBean<T, E, ID>
  ...
  private final ConcurrentHashMap<Method, MethodHandler> methodHandlers =
      new ConcurrentHashMap<>(64);
  private final ConcurrentHashMap<Method, Class<?>> listElementTypeCache =
      new ConcurrentHashMap<>(32);
  private final ConcurrentHashMap<Method, Class<?>> optionalElementTypeCache =
      new ConcurrentHashMap<>(32);
  private final ConcurrentHashMap<Method, MethodHandle> defaultMethodHandles =
  ...
  public RepositoryFactoryBean(
      Class<T> repositoryInterface, Class<E> entityClass, Class<ID> idClass) {
    ...
    ReflectionUtils.doWithMethods(
        BaseRepository.class, method -> baseRepoMethodSigs.add(MethodSignature.of(method)));
  }
  ...
  private MethodHandler createMethodHandler(Method method, BaseRepository<?, ?> baseRepo)
      throws Exception {
    if (method.isDefault()) {
  ...
  private Method findBaseRepositoryMethod(Method interfaceMethod) throws NoSuchMethodException {
    return BaseRepository.class.getMethod(
  ...
  private final class SqlMethodHandler implements MethodHandler {
    ...
    private IllegalStateException createExecutionException(String sql, RuntimeException ex) {
      ...
      return new IllegalStateException(
          String.format(
              "SQL执行失败: templateId=%s, type=%s, sqlHead=%s",
              sqlTemplate.getId(), sqlTemplate.getSqlType(), sqlHead),
          ex);
    }
  }
  ...
  private void resolveAndCacheElementType(Method method) {
    if (List.class.equals(method.getReturnType())) {
      listElementTypeCache.computeIfAbsent(
          method, m -> GenericTypeResolver.resolveReturnTypeArgument(m, List.class));
    } else if (Optional.class.equals(method.getReturnType())) {
      optionalElementTypeCache.computeIfAbsent(
          method, m -> GenericTypeResolver.resolveReturnTypeArgument(m, Optional.class));
    }
  }
  ...
  private String getParameterName(Parameter parameter, String[] discoveredNames, int index) {
    ...
    return discoveredNames != null
            && discoveredNames.length > index
            && discoveredNames[index] != null
  ...
  private MethodHandle toHandle(Method method, Object target)
    ...
    try {
      ...
      return MethodHandles.lookup()
          .bind(
              new ReflectFallback(target, method),
              ReflectFallback.INVOKE,
              MethodType.methodType(Object.class, Object[].class));
    }
  }

  private MethodHandle lookupDefaultMethodHandle(Method method) {
    return defaultMethodHandles.computeIfAbsent(
        method,
        m -> {
          try {
