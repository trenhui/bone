...
/** 针对 MySQL（8.0+）的 ColumnAllocationDialect 实现， 要求把 LIMIT 放在 FOR UPDATE SKIP LOCKED 之前。 */
public class MySqlColumnAllocationDialect implements ColumnAllocationDialect {
  ...
  /**
   * 因为 LIMIT … OFFSET … 要放在 FOR UPDATE SKIP LOCKED 之前， 所以这里不再返回完整的 “FOR UPDATE SKIP LOCKED”。 而是在
   * repository 里，先拼 LIMIT，再拼下面这个锁定子句。
   */
  @Override
