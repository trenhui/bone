...
public class SqlFragmentLoader {
  ...
  public void cacheSqlFragmentsForInterface(Class<?> repoInterface) {
    ...
    if (!fragmentMap.isEmpty()) {
      ...
      log.info(
          "缓存 {} 个 SQL 片段: interface={}, priority={}",
          fragmentMap.size(),
          interfaceName,
          loadPriority);
    } else {
   ...
   * @return 是否加载到片段
   */
  private boolean loadAnnotationFragmentsWithFallback(
      Class<?> repoInterface, Map<String, String> fragmentMap) {
   ...
   * @return 是否加载到片段
   */
  private boolean loadFileFragmentsWithFallback(
      String interfaceName, Map<String, String> fragmentMap) {
    try {
  ...
  /** 加载注解片段（排除已存在的片段） */
  private boolean loadAnnotationFragmentsWithExclusion(
      Class<?> repoInterface, Map<String, String> fragmentMap) {
    SqlFragment[] fragments = repoInterface.getAnnotationsByType(SqlFragment.class);
    int loadedCount = 0;
  ...
  /** 加载文件片段（排除已存在的片段） */
  private boolean loadFileFragmentsWithExclusion(
      String interfaceName, Map<String, String> fragmentMap) {
    try {
      String packagePath = interfaceName.replace('.', '/');
      String fragmentPath = String.format("classpath:sql/%s/sqlFragment/*.sql", packagePath);
  ...
  /** 从资源加载片段（排除已存在的片段） */
  private boolean loadFragmentFromResourceWithExclusion(
      String interfaceName, Resource resource, Map<String, String> fragmentMap) {
    ...
    if (fileName != null && fileName.endsWith(".sql")) {
      ...
      return loadFragmentFromResource(
          interfaceName, resource, fragmentMap, qualifiedId, fragmentId);
    }
    return false;
  }

  /** 从资源加载片段（核心实现） */
  private boolean loadFragmentFromResource(
      String interfaceName, Resource resource, Map<String, String> fragmentMap) {
    String fileName = resource.getFilename();
    if (fileName != null && fileName.endsWith(".sql")) {
      String fragmentId = fileName.substring(0, fileName.lastIndexOf(".sql"));
  ...
  private boolean loadFragmentFromResource(
    ...
    try (BufferedReader reader =
      ...
      if (StringUtils.hasText(content)) {
        fragmentMap.putIfAbsent(qualifiedId, content);
        log.debug(
            "从文件缓存 SQL 片段: interface={}, id={}, file={}",
            interfaceName,
            qualifiedId,
