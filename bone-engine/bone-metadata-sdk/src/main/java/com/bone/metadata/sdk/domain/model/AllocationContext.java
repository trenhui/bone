...
/** 实体分组／列分配上下文： 既是路由分区的键，也是 ColumnAllocator 的上下文载体。 */
@Getter
@Setter
@AllArgsConstructor
@NoArgsConstructor
public class AllocationContext {
  ...
  public static AllocationContext from(ExtensionContext ctx) {
    return new AllocationContext(
        ctx.getTenantId(), ctx.getAppCode(), ctx.getBizIdentityCode(), ctx.getEntityType());
  }

  public static AllocationContext of(
      Long tenantId, String appCode, String bizIdentityCode, String entityType) {
    return new AllocationContext(tenantId, appCode, bizIdentityCode, entityType);
  }

  // todo more
  public static AllocationContext of(String appCode, String entityType) {
    return new AllocationContext(null, appCode, null, entityType);
  }

  /** 分布式锁用的唯一 key */
  public String lockKey() {
    return String.format("alloc:%d:%s:%s:%s", tenantId, appCode, bizIdentityCode, entityType);
  }
