...
/** 参数转换工具类：高效将JavaBean转换为查询参数Map，过滤null值和空白字符串 线程安全，高性能设计，适合高频调用场景 */
@Slf4j
public final class ParamConvertUtil {
  ...
  /** ✅ 新增：线程安全的类加载缓存：类名 -> Class 对象 使用 ConcurrentHashMap 保证高并发性能 */
  ...
  /**
   * 根据类名高效加载并缓存 Class 对象。 线程安全，首次加载后缓存，后续直接返回。
   *
  ...
  /**
   * 根据类名加载 Class，但允许调用者处理 ClassNotFoundException。 适用于需要精确控制异常处理的场景。
   *
   ...
   */
  @SuppressWarnings("unchecked")
  public static <T> Class<T> loadClassWithException(String className)
  ...
  /**
   * 将JavaBean转换为参数Map 规则：过滤null值和空白字符串，键为字段名，值为字段原值
   *
  ...
  private static boolean processField(
      Object fieldValue, Field field, Map<String, Object> paramMap) {
    // 字符串类型需检查空白
