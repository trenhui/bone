 ...
 * 数据源切换拦截器实现
 *
 * <p>该拦截器根据方法或类上的{@link DataSourceSwitch}注解自动切换数据源。 遵循方法级注解优先于类级注解的原则。
 */
public class DataSourceAnnotationInterceptor implements MethodInterceptor {

  private static final Logger logger =
      LoggerFactory.getLogger(DataSourceAnnotationInterceptor.class);
  ...
  public Object invoke(MethodInvocation invocation) throws Throwable {
    ...
    // 解析方法和类上的注解
    DataSourceSwitch methodAnnotation =
        RepositoryClassUtils.findAnnotation(method, DataSourceSwitch.class);
    DataSourceSwitch classAnnotation =
        RepositoryClassUtils.findAnnotation(targetClass, DataSourceSwitch.class);
    ...
    if (StringUtils.hasText(targetDataSourceName)) {
      ...
      try {
        ...
        logger.debug("已恢复方法: {}.{} 的数据源上下文", targetClass.getSimpleName(), method.getName());
      }
    } else {
      // 不需要切换数据源，直接执行
      return invocation.proceed();
   ...
   * 基于注解解析最终使用的数据源名称
   *
   * <p>方法级注解优先于类级注解。如果没有注解提供有效的数据源名称， 则返回null，表示应使用当前数据源。
   *
   * @param methodAnnotation 方法上的注解
   * @param classAnnotation 类上的注解
   * @return 解析后的数据源名称，如未指定则返回null
   */
  private String resolveDataSourceName(
      DataSourceSwitch methodAnnotation, DataSourceSwitch classAnnotation) {
    // 方法级注解优先级最高
   ...
   * 方法调用后清理数据源上下文
   *
   * <p>此方法确保上下文栈正确恢复到原始深度，防止内存泄漏并确保嵌套方法调用 的正确行为。如果检测到意外的上下文元素，它会执行额外的清理以维护系统稳定性。
   *
