 ...
 *
 * <p>Provides functionality for advanced data source management including:
 *
 * <ul>
 *   <li>Master-slave separation configuration
 *   <li>Connection pool management
 *   <li>Health checking mechanisms
 *   <li>Dynamic data source switching
 *   <li>Routing strategy configuration
 * </ul>
 *
 * <p>This class is designed with flexibility in mind while maintaining a clear separation of
 * concerns between different configuration aspects. It provides validation capabilities to ensure
...
public class DataSourceProperties {
  ...
  /**
   * Whether strict mode is enabled. When strict mode is on, operations will throw exceptions if
  ...
  /**
   * Mapping of data source configurations, where keys are data source names and values are the
  ...
  /** 缓存已创建的数据源实例，避免重复创建 */
  private final Map<String, javax.sql.DataSource> dataSourceCache =
      ...
      if (config.getType() != null) {
        ...
        try {
          ...
          Class<? extends javax.sql.DataSource> type =
              (Class<? extends javax.sql.DataSource>) Class.forName(typeName);
          builder.type(type);
  ...
  /**
   * Validates the configuration properties. This method checks for common configuration errors and
   * consistency issues, and also validates all nested configuration objects.
   *
    ...
    if (dataSources != null) {
      ...
      if (dataSources.size() > maxDataSourceCount) {
        throw new IllegalArgumentException(
            String.format(
                "Number of data sources (%d) exceeds maximum allowed (%d)",
      ...
      for (Map.Entry<String, DataSourceConfig> entry : dataSources.entrySet()) {
        ...
        try {
          ...
          throw new IllegalArgumentException(
              String.format(
                  "Invalid configuration for data source '%s': %s", dataSourceName, e.getMessage()),
              e);
        }
      }
    }
  ...
  /**
   * Individual data source configuration class. Contains settings for a specific data source
   * including connection details and type. This class provides configuration for a single database
   * connection with appropriate validation to ensure required properties are set correctly.
   */
  ...
  /**
   * Read-write separation configuration class. Provides settings for implementing read-write
   * separation patterns across multiple data sources. This class configures how read and write
   * operations are directed to appropriate data sources.
   */
  public static class ReadWriteConfig {
    ...
    public void setSlaveDataSourceNames(String[] slaveDataSourceNames) {
      this.slaveDataSourceNames =
          slaveDataSourceNames != null ? slaveDataSourceNames : new String[0];
    }
    ...
    public void validate() {
      if (enabled) {
        if (masterDataSourceName == null || masterDataSourceName.trim().isEmpty()) {
          throw new IllegalArgumentException(
              "Master data source name must be configured when read-write separation is enabled");
        }
        if (slaveLoadBalancingStrategy == null) {
          throw new IllegalArgumentException(
              "Load balancing strategy must be configured when read-write separation is enabled");
        }
        if (slaveDataSourceNames != null
            && Arrays.stream(slaveDataSourceNames)
  ...
  /**
   * Connection pool configuration class. Provides settings for managing database connection pools
   * including sizing, validation, and idle connection handling. This class configures how database
   * connections are pooled, validated, and managed.
   */
  public static class PoolConfig {
    ...
    /**
     * Minimum amount of time a connection may sit idle in the pool before it is eligible for
     * eviction.
     *
    ...
    public void validate() {
      ...
      if (minIdle > maxActive) {
        throw new IllegalArgumentException(
            "Minimum idle connections cannot exceed maximum active connections");
      }
      if (maxWait < 0) {
  ...
  /**
   * Data source type enumeration. Defines whether a data source is intended for write operations
   * (MASTER) or read operations (SLAVE). This enumeration categorizes data sources by their
  ...
  /**
   * Data source routing strategy enumeration. Defines how data source selection is determined for
   * operations. This enumeration specifies different approaches for choosing which data source to
   * use for a particular operation or request.
   */
  ...
  /**
   * Load balancing strategy enumeration. Defines how read operations are distributed across
   * multiple slave data sources. This enumeration provides different algorithms for distributing
   * read traffic among available slave data sources to optimize performance and availability.
   */
