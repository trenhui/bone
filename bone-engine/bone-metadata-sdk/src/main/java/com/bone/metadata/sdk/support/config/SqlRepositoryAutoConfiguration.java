...
public class SqlRepositoryAutoConfiguration {
  ...
  @Bean
  @ConditionalOnMissingBean
  public ColumnAllocator columnAllocator(
      ColumnAllocationRepository repo, ColumnNamingStrategy namingStrategy) {
  ...
  public ColumnAllocationDialectFactory columnAllocationDialectFactory(
      List<ColumnAllocationDialect> columnAllocationDialects) {
  ...
  @Bean
  @ConditionalOnMissingBean
  public SqlBuilder SqlBuilder(MetadataService metadataService, DatabaseDialect dialect) {
  ...
  public SqlExecutor sqlExecutor(NamedParameterJdbcOperations jdbcOperations) {
    SqlExecutor executor = new SqlExecutor(jdbcOperations, properties);
    // 初始化 QueryBuilder
    QueryBuilder.initialize(executor);
    log.info("QueryBuilder initialized successfully");
    return executor;
  }
  ...
  public ReservedColumnsHandler reservedColumnsHandler(
  ...
  public List<TemplateContentParser> templateContentParsers() {
    return Arrays.asList(
        new SqlTemplateParser(), new MyBatisTemplateParser(), new YamlTemplateParser());
  }

  /** 注册 SQL 模板内容解析器。 */
  @Bean
  public List<TemplateSourceProvider> templateSourceProviders(
      ResourceLoader resourceLoader, SqlConfigProperties config) {
    return Arrays.asList(
  ...
  /** 注册 SQL 模板加载器。 增加 Cache 和 ConcurrentHashMap 参数 */
  @Bean
