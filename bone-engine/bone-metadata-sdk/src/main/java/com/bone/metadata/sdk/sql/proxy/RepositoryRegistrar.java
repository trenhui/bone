...
public class RepositoryRegistrar implements ImportBeanDefinitionRegistrar, ResourceLoaderAware {
  ...
  public void registerBeanDefinitions(
    ...
    logger.info(
        "Scanned {} Repository interfaces in {}ms", scanCount, System.currentTimeMillis() - start);
  ...
  private List<String> getBasePackages(AnnotationAttributes attrs, AnnotationMetadata metadata) {
    ...
    Arrays.stream(attrs.getStringArray("value"))
        .filter(StringUtils::hasText)
        .forEach(basePackages::add);
    Arrays.stream(attrs.getStringArray("basePackages"))
        .filter(StringUtils::hasText)
        .forEach(basePackages::add);
  ...
  /** 手动注册Repository接口 */
  private void manuallyRegisterRepositories(
      List<String> basePackages, BeanDefinitionRegistry registry) {
    for (String basePackage : basePackages) {
      try {
        String packageSearchPath = "classpath*:" + basePackage.replace('.', '/') + "/**/*.class";
        Resource[] resources =
            ((ResourcePatternResolver) resourceLoader).getResources(packageSearchPath);
          ...
          if (resource.isReadable()) {
            try {
              MetadataReader metadataReader =
                  new SimpleMetadataReaderFactory().getMetadataReader(resource);
              ClassMetadata classMetadata = metadataReader.getClassMetadata();

              if (classMetadata.isInterface()
                  && classMetadata.getClassName().endsWith("Repository")) {
                ...
                if (implementsRepository) {
                  ...
                  logger.warn(
                      "Interface {} does not implement Repository interface",
  ...
  /** 注册Repository Bean */
  private void registerRepositoryBean(
      Class<?> repositoryInterface, BeanDefinitionRegistry registry) {
    try {
      // 解析泛型参数
