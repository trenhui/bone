...
/** 切面：拦截 NamedParameterJdbcOperations 的执行， 记录 SQL、参数、耗时，并对超过阈值的慢查询打 WARN。 */
@Aspect
@Slf4j
@Order()
public class SqlExecutionInterceptor {
  ...
  @Pointcut(
      "execution(* org.springframework.jdbc.core.namedparam.NamedParameterJdbcOperations.query*(..))"
  ...
  public Object aroundJdbcExecution(ProceedingJoinPoint pjp) throws Throwable {
    ...
    try {
      ...
      if (elapsedMs > slowQueryThreshold && !log.isDebugEnabled()) {
        log.warn(
            "[SLOW-SQL][{}ms > {}ms] {} | params={}", elapsedMs, slowQueryThreshold, sql, params);
      }
    }
  }
  ...
  private Map<String, Object> extractBeanPropertySourceParams(
      BeanPropertySqlParameterSource beanSource) {
    try {
