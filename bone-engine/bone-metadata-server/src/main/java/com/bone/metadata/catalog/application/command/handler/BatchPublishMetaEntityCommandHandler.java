 ...
 * 批量发布元数据实体命令处理器。
 *
 * <p>类级 {@code @Transactional} 满足 DDD 写事务边界约定；内部以 {@code REQUIRES_NEW} 隔离每个单条发布，单个失败被捕获并计入 {@link
 * BatchOperateResult}，不影响其余实体， 实现部分成功语义。
 */
@Component
@RequiredArgsConstructor
@Transactional
public class BatchPublishMetaEntityCommandHandler {

  private final PublishMetaEntityHandler publishMetaEntityHandler;
  private final PlatformTransactionManager transactionManager;

  public BatchOperateResult handle(BatchPublishMetaEntityCommand cmd) {
      ...
      try {
        txTemplate.execute(
            (TransactionStatus status) -> {
              publishMetaEntityHandler.handle(id, null);
              return null;
            });
        success++;
      } catch (Exception e) {
