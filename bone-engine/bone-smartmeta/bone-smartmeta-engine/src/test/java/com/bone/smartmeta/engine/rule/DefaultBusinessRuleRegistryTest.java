...
/**
 * DefaultBusinessRuleRegistry测试类
 * 提供全面的测试覆盖，确保业务规则注册表功能正确
 */
class DefaultBusinessRuleRegistryTest {
    ...
    void testRegisterRule_NullRule() {
        // 测试注册null规则，应该优雅处理
        assertDoesNotThrow(() -> ruleRegistry.registerRule(null), 
                "注册null规则不应该抛出异常");
    }
    ...
    void testDeleteRule_NonExistent() {
        // 删除不存在的规则，应该优雅处理
        assertDoesNotThrow(() -> ruleRegistry.deleteRule("non-existent-rule"), 
                "删除不存在的规则不应该抛出异常");
    }
    ...
    void testRegisterAndRetrieveRulesWithDifferentParams(String ruleId, String entityType, String ruleType) {
