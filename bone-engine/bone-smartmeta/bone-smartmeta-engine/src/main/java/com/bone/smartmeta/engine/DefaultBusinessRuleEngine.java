...
public class DefaultBusinessRuleEngine implements BusinessRuleEngine {
    ...
    @Autowired
    public DefaultBusinessRuleEngine(BusinessRuleRegistry businessRuleRegistry, ExpressionEngine expressionEngine) {
                    ...
                    if (evaluateCondition(rule.getCondition(), context)) {
                        ...
                        try {
                            ...
                            if (validationResult instanceof Boolean && !(Boolean) validationResult) {
                                result.addError(rule.getName(), rule.getErrorMessage() != null ? 
                                        rule.getErrorMessage() : "Validation rule failed: " + rule.getName());
                            }
                        } catch (Exception e) {
    ...
    public ValidationResult evaluateHighValueOrderRule(Map<String, Object> orderData) {
        ...
        try {
            ...
            if (amount != null && amount.doubleValue() > 10000) {
                // 高价值订单需要额外验证
                if (!orderData.containsKey("approvalStatus") || !"APPROVED".equals(orderData.get("approvalStatus"))) {
    ...
    public ValidationResult executeRules(Object entity, String ruleType) {
        ...
        try {
            ...
            LOGGER.debug("Executing rules of type: {} for entity type: {}", ruleType, entity.getClass().getName());
    ...
    private void analyzeExpressionDependencies(String expression, List<String> dependencies) {
        ...
        // 简化的依赖分析
        String[] potentialFields = expression.split("[\\s\\+\\-\\*\\/\\(\\)\\{\\}\\[\\],;:\\=\\!\\<\\>\\&\\|\\\\\\'\"]+");
        for (String field : potentialFields) {
            // 过滤掉关键字和数字
            if (!field.isEmpty() && !field.matches("^\\d+") && 
                !field.equals("true") && !field.equals("false") && 
                !field.equals("null")) {
