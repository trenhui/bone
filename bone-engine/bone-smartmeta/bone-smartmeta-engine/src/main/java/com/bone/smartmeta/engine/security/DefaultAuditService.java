...
public class DefaultAuditService implements AuditService {
    ...
    public void logAccessDenied(String username, String resourceType, String resourceId, String operation) {
        ...
        // 异步保存到数据库
        auditExecutor.submit(() -> saveAuditLog(username, "ACCESS_DENIED", resourceType, resourceId, operation, message));
    }
    ...
    public void logFieldAccess(String username, String entityName, String fieldName, String operation, boolean isMasked) {
        ...
        // 异步保存到数据库
        auditExecutor.submit(() -> saveAuditLog(username, "FIELD_ACCESS", entityName, fieldName, operation, message));
    }
    ...
    public void logRecordAccess(String username, String entityName, String recordId, String operation) {
        ...
        // 异步保存到数据库
        auditExecutor.submit(() -> saveAuditLog(username, "RECORD_ACCESS", entityName, recordId, operation, message));
    }
    ...
    public void logSensitiveOperation(String username, String operation, String details) {
        ...
        // 异步保存到数据库
        auditExecutor.submit(() -> saveAuditLog(username, "SENSITIVE_OPERATION", "SYSTEM", "", operation, message));
    }
    ...
    public void logBatchOperation(String username, String entityName, String operation, int recordCount, int successCount) {
        ...
        // 异步保存到数据库
        String details = String.format("总数: %d, 成功: %d, 失败: %d", recordCount, successCount, recordCount - successCount);
        auditExecutor.submit(() -> saveAuditLog(username, "BATCH_OPERATION", entityName, "", operation, message));
    }

    @Override
    public void logAuthentication(String username, String ipAddress, boolean success, String reason) {
        String message = String.format("[认证] 时间: %s, 用户: %s, IP: %s, 结果: %s, 原因: %s",
        ...
        // 异步保存到数据库
        auditExecutor.submit(() -> saveAuditLog(username, "AUTHENTICATION", "SYSTEM", ipAddress, success ? "SUCCESS" : "FAILURE", message));
    }
    ...
    public void logAuthorization(String username, String resource, String operation, boolean success) {
        ...
        // 异步保存到数据库
        auditExecutor.submit(() -> saveAuditLog(username, "AUTHORIZATION", resource, "", operation, message));
    }
     ...
     * 实际项目中应该保存到数据库中
     */
    private void saveAuditLog(String username, String logType, String resourceType, String resourceId, String operation, String message) {
        try {
            // 这里应该是保存到数据库的逻辑
            // 暂时只是打印日志
            logger.debug("保存审计日志: {}, {}, {}, {}, {}, {}",
                    username, logType, resourceType, resourceId, operation, message);
