            ...
            try {
                ...
                if (relationship != null && entityData.containsKey(fieldName) && entityData.get(fieldName) != null) {
                    ...
                    if (targetEntity != null && !targetEntity.isEmpty()) {
                        // 验证目标实体是否存在
                        if (entityMetadataProvider != null && entityMetadataProvider.getEntityMetadata(targetEntity) == null) {
                            ValidationResult.ValidationWarning validationWarning = 
                                ValidationResult.ValidationWarning.builder()
                                    .fieldPath(fieldName)
                                    .message(String.format("字段 '%s' 关联的实体类型 '%s' 未定义", 
                                            fieldLabel, targetEntity))
                                    .build();
