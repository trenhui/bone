    ...
    public QueryAst parse(String smartql) {
        ...
        try {
            ...
            if (normalizedQuery.contains("FROM")) {
                ...
                int endIndex = whereIndex > 0 ? whereIndex : (nextSpace > 0 ? nextSpace : normalizedQuery.length());
            ...
            while (paramIndex >= 0) {
                ...
                int nextOperator = Math.min(
                    Math.min(nextSpace > 0 ? nextSpace : Integer.MAX_VALUE,
