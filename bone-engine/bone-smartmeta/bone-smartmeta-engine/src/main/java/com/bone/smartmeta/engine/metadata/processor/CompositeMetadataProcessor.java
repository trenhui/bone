...
import com.bone.smartmeta.engine.metadata.CalculatedFieldMetadata;
import com.bone.smartmeta.engine.metadata.VirtualFieldMetadata;
import com.bone.smartmeta.engine.metadata.MetadataChangeEvent;
import com.bone.smartmeta.engine.metadata.MetadataChangeListener;
...
public class CompositeMetadataProcessor implements MetadataProcessor {
    ...
    public List<EntityMetadata> hotReloadMetadata() {
        ...
        if (hasSourceChanged("yaml")) {
            processMetadataSource("yaml", "classpath*:com/bone/smartmeta/packages/**/objects/*.yaml", updatedMetadataGroups);
            hasChanges = true;
        ...
        if (hasSourceChanged("groovy")) {
            processMetadataSource("groovy", "classpath*:com/bone/smartmeta/packages/**/groovy/*.groovy", updatedMetadataGroups);
            hasChanges = true;
     ...
     * 发布元数据变更事件
     */
    private void publishMetadataChangeEvent(String entityApiName, EntityMetadata oldMetadata, EntityMetadata newMetadata) {
     ...
     * 注册计算字段
     */
    private void registerCalculatedField(EntityMetadata entityMetadata, CalculatedFieldMetadata field) {
        // 实现计算字段的注册逻辑
        log.debug("为实体 {} 注册计算字段: {}", 
    ...
    private void mergeFieldMetadata(SmartFieldMetadata target, SmartFieldMetadata source) {
        ...
        // if (source.isAiAutoFillEnabled() != null) target.setAiAutoFillEnabled(source.isAiAutoFillEnabled());
        // if (source.getAiPrompt() != null) target.setAiPrompt(source.getAiPrompt());
        // if (source.getSensitivityLevel() != null) target.setSensitivityLevel(source.getSensitivityLevel());
        // if (source.getCalculationExpression() != null) target.setCalculationExpression(source.getCalculationExpression());
