...
/**
 * 字段约束验证策略
 * 验证字段值是否符合定义的约束条件（长度、范围、正则表达式等）
 */
public class FieldConstraintValidationStrategy implements ValidationStrategy {
    
    private static final Logger log = LoggerFactory.getLogger(FieldConstraintValidationStrategy.class);
     ...
     * 验证字符串约束
     */
    private void validateStringConstraints(SmartFieldMetadata field, String fieldName, String fieldLabel, String value, ValidationResult result) {
        // 验证字符串长度
        ...
        if (maxLength != null && value.length() > maxLength) {
            ValidationResult.ValidationError validationError = 
                ValidationResult.ValidationError.builder()
                    .fieldPath(fieldName)
                    .message(String.format("字段 '%s' 的长度不能超过 %d 个字符", 
        ...
        if (minLength != null && value.length() < minLength) {
            ValidationResult.ValidationError validationError = 
                ValidationResult.ValidationError.builder()
                    .fieldPath(fieldName)
                    .message(String.format("字段 '%s' 的长度不能少于 %d 个字符", 
                            fieldLabel, minLength))
                    .build();
            result.addError(validationError);
        }
        ...
        if (regexPattern != null) {
            if (!Pattern.matches(regexPattern, value)) {
                ValidationResult.ValidationError validationError = 
                    ValidationResult.ValidationError.builder()
                        .fieldPath(fieldName)
                        .message(String.format("字段 '%s' 的值不符合要求的格式", 
                                fieldLabel))
                        .build();
                result.addError(validationError);
            }
     ...
     * 验证数值约束
     */
    private void validateNumberConstraints(SmartFieldMetadata field, String fieldName, String fieldLabel, Number value, ValidationResult result) {
        ...
        if (maxValue != null && numValue > maxValue) {
            ValidationResult.ValidationError validationError = 
                ValidationResult.ValidationError.builder()
                    .fieldPath(fieldName)
                    .message(String.format("字段 '%s' 的值不能大于 %s", 
                            fieldLabel, maxValue))
                    .build();
            result.addError(validationError);
        }
        ...
        if (minValue != null && numValue < minValue) {
            ValidationResult.ValidationError validationError = 
                ValidationResult.ValidationError.builder()
                    .fieldPath(fieldName)
                    .message(String.format("字段 '%s' 的值不能小于 %s", 
                            fieldLabel, minValue))
                    .build();
            result.addError(validationError);
        }
     ...
     * 验证选择列表约束
     */
    private void validatePicklistConstraints(SmartFieldMetadata field, String fieldName, String fieldLabel, Object value, ValidationResult result) {
        ...
        if (picklistValues != null && !picklistValues.isEmpty()) {
            ...
            if (!picklistValues.contains(stringValue)) {
                ValidationResult.ValidationError validationError = 
                    ValidationResult.ValidationError.builder()
                        .fieldPath(fieldName)
                        .message(String.format("字段 '%s' 的值必须是以下之一: %s", 
                                fieldLabel, String.join(", ", picklistValues)))
                        .build();
                result.addError(validationError);
            }
        }
     ...
     * 验证数组约束
     */
    private void validateArrayConstraints(SmartFieldMetadata field, String fieldName, String fieldLabel, List<?> value, ValidationResult result) {
        try {
            // 尝试获取数组最大/最小元素数量
            ...
            if (maxItems != null && value.size() > maxItems) {
                ValidationResult.ValidationError validationError = 
                    ValidationResult.ValidationError.builder()
                        .fieldPath(fieldName)
                        .message(String.format("字段 '%s' 的数组元素数量不能超过 %d 个", 
                                fieldLabel, maxItems))
                        .build();
                result.addError(validationError);
            }
            ...
            if (minItems != null && value.size() < minItems) {
                ValidationResult.ValidationError validationError = 
                    ValidationResult.ValidationError.builder()
                        .fieldPath(fieldName)
                        .message(String.format("字段 '%s' 的数组元素数量不能少于 %d 个", 
                                fieldLabel, minItems))
                        .build();
                result.addError(validationError);
            }
