...
public class FieldLevelSecurityManager {
    ...
    @Autowired
    public FieldLevelSecurityManager(PermissionEvaluator permissionEvaluator, 
    ...
    public <T> T processDataAccess(CustomAuthentication authentication, String entityName, T data, String operation) {
        if (data == null) {
            return null;
        }
     ...
     * 处理Map类型的数据
     */
    private Map<String, Object> processMapData(CustomAuthentication authentication, 
        ...
        for (Map.Entry<String, Object> entry : dataMap.entrySet()) {
            ...
            if (!hasFieldPermission(authentication, metadata, field, operation)) {
                // 记录访问拒绝日志
                auditService.logAccessDenied(authentication, "FIELD_ACCESS", 
                                           metadata.getApiName() + "." + fieldName, operation);
     ...
     * 检查用户是否有字段访问权限
     */
    private boolean hasFieldPermission(CustomAuthentication authentication, 
        ...
        // 检查操作类型对应的权限
        if ("READ".equals(operation)) {
            return securityMetadata.getReadableRoles().stream()
                    .anyMatch(userRoles::contains);
        } else if ("WRITE".equals(operation) || "UPDATE".equals(operation)) {
            return securityMetadata.getEditableRoles().stream()
                    .anyMatch(userRoles::contains);
        }
     ...
     * 应用数据脱敏
     */
    private Object applyDataMasking(CustomAuthentication authentication, 
                                  EntityMetadata entity, 
                                  FieldMetadata field, 
                                  Object value) {
        if (value == null) {
        ...
        // 获取脱敏规则
        List<DataMaskingRule> maskingRules = permissionEvaluator.getDataMaskingRules(entity.getApiName(), field.getName());
        ...
        for (DataMaskingRule rule : maskingRules) {
            // 如果用户角色在例外列表中，则不应用脱敏
            if (rule.getExceptions().stream().anyMatch(userRoles::contains)) {
                return value;
            }
    ...
    private Object decryptValue(FieldMetadata field, Object encryptedValue) {
        try {
            // 实际项目中应该使用加密服务进行解密
            // 这里简化处理，实际应该调用加密服务
            String algorithm = field.getEncryptionAlgorithm() != null ? 
                              field.getEncryptionAlgorithm() : "AES/GCM/NoPadding";
     ...
     * 验证批量操作的权限
     */
    public void validateBatchOperation(CustomAuthentication authentication, 
                                     String operation, 
                                     List<?> recordIds) {
        // 验证操作权限
        if (!permissionEvaluator.hasOperationPermission(authentication, operation)) {
            auditService.logAccessDenied(authentication, 
                                       "OPERATION_ACCESS", operation, "");
     ...
     * 验证记录访问权限
     */
    private void validateRecordAccess(CustomAuthentication authentication, 
                                    String entityName, 
                                    Object recordId) {
        if (!permissionEvaluator.hasRecordAccessPermission(authentication, entityName, recordId)) {
            auditService.logAccessDenied(authentication, 
                                       "RECORD_ACCESS", entityName, recordId.toString());
    ...
    private String getEntityFromOperation(String operation) {
        ...
        return operation.replaceAll("([A-Z])", " $1")
                .trim()
                .split(" ")[0]
                .toLowerCase();
    }
    ...
    // 注意：DataMaskingRule类已移至独立文件中
    // 导入: com.bone.smartmeta.engine.security.DataMaskingRule
