...
| lineAmountCalculationRule | PurchaseOrderItem | `${amount} == ${quantity} * ${unitPrice}` | 金额必须等于数量乘以单价 | ERROR |
| receivedQuantityValidationRule | PurchaseOrderItem | `${actualQuantity} <= ${quantity}` | 已收货数量不能大于订单数量 | ERROR |
...
**业务应用层**：
- 采购业务服务：负责采购订单的创建、审核、管理等核心业务流程
- 审批流程服务：处理采购订单的多级审批逻辑
- 报表分析服务：基于元数据生成采购业务报表和数据分析

**元数据引擎核心**：
- EntityMetadata：实体元数据管理，包含实体定义、字段映射等
- FieldMetadata：字段元数据管理，包含字段类型、验证规则等
- BusinessRuleEngine：业务规则引擎，执行验证规则和计算规则
- FieldCalculationEngine：字段计算引擎，处理字段间的计算关系
- ValidationEngine：验证引擎，执行数据验证逻辑
- MetadataCacheManager：元数据缓存管理，提高元数据访问性能
- MetadataVersionController：版本控制器，管理元数据的版本变更
- CodeGenerationEngine：代码生成引擎，基于元数据生成实体类和服务类
- ExpressionEngine：表达式引擎，解析和执行动态表达式

**数据持久层**：
- MetadataRepository：元数据仓库，提供元数据的CRUD操作
- RedisCache：分布式缓存，缓存热点元数据，提高访问性能
- 关系数据库：持久化存储元数据信息
...
public class DefaultCodeGenerationEngine implements CodeGenerationEngine {
    ...
    public String generateCode(EntityMetadata metadata, String templateType) {
        try {
            Template template = templates.get(templateType);
            if (template == null) {
    ...
    private void loadBuiltInTemplates() {
        // 加载内置模板
        try {
            templates.put("entity", configuration.getTemplate("entity.ftl"));
            templates.put("service", configuration.getTemplate("service.ftl"));
            templates.put("controller", configuration.getTemplate("controller.ftl"));
            templates.put("dto", configuration.getTemplate("dto.ftl"));
            templates.put("repository", configuration.getTemplate("repository.ftl"));
        } catch (IOException e) {
            throw new RuntimeException("Failed to load built-in templates", e);
        }
    }
}```
...
import com.bone.smartmeta.core.metadata.MetadataEngine;
...
public class ${entity.name}Service {
    ...
    public ${entity.name} create(${entity.name} entity) {
        // 验证实体
        metadataEngine.validateEntity("${entity.name}", entity);
        // 计算字段
        metadataEngine.calculateFields("${entity.name}", entity);
        // 保存实体
