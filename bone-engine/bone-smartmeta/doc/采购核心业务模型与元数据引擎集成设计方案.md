...
以下是基于注解的实体定义和YAML格式的元数据定义两种方式：

采购订单是采购业务的核心实体，代表企业与供应商之间的正式采购协议，包含订单基本信息、供应商信息、金额信息、时间信息等完整业务数据。

```java
/**
 * 采购订单实体 - 企业采购业务的核心数据模型
 * <p>符合JPA规范的企业级实体设计，支持审计跟踪、业务规则验证和元数据驱动</p>
 * <p><strong>最佳实践要点：</strong></p>
 * <ul>
 *   <li>使用@Audited确保数据变更可追溯性</li>
 *   <li>采用组合模式设计复杂业务实体</li>
 *   <li>实现字段级权限控制和敏感数据保护</li>
 *   <li>支持AI辅助的数据生成功能</li>
 * </ul>
 */
@SmartEntity(
    ...
    indexes = {"orderCode", "supplierId,createTime"}
)
@Data
@Audited // 审计跟踪支持 - 记录所有数据变更
@EntityListeners({ValidationEntityListener.class, AuditEntityListener.class})
public class PurchaseOrder extends Entity<Long> {
    ...
    /**
     * 订单编号 - 系统唯一标识符
     * <p>格式规范：PO+8位日期+4位序号，例如：PO202401150001</p>
     * <p>业务意义：作为订单的主要检索标识，贯穿整个采购流程</p>
     */
    @SmartField(
        name = "orderCode", 
        label = "订单编号", 
        ...
        aiPrompt = "生成符合PO+8位日期+4位序号格式的订单编号",
        searchable = true, // 支持快速检索
        sortable = true,   // 支持排序操作
        exportable = true, // 支持数据导出
        displayOrder = 1   // UI显示顺序
    )
    @BusinessRule(
        name = "orderCodeRule", 
        expression = "${orderCode} != null && ${orderCode}.matches('PO\\d{8}\\d{4}')", 
        errorMessage = "订单编号格式必须为PO+8位日期+4位序号"
    )
...
#### 2.1.2 YAML格式采购订单元数据定义

```yaml
# metadata/purchase-order.yaml
entity:
  name: "PurchaseOrder"
  description: "采购订单"
  ...
  fields:
    ...
    - name: "orderCode"
      ...
      aiPrompt: "生成符合PO+8位日期+4位序号格式的订单编号"
      validateRules:
        - expression: "${orderCode}.matches('PO\\d{8}\\d{4}')"
          message: "订单编号格式必须为PO+8位日期+4位序号"
          severity: "ERROR"
    ...
    - name: "orderType"
      ...
      description: "订单类型"
      required: true
      options: ["STANDARD", "EMERGENCY", "ROUTINE", "CONTRACT"]
      validateRules:
        - expression: "${orderType} != null && (${orderType}.equals('STANDARD') || ${orderType}.equals('EMERGENCY') || ${orderType}.equals('ROUTINE') || ${orderType}.equals('CONTRACT'))"
          message: "订单类型必须为标准采购、紧急采购、日常采购或合同采购"
          severity: "ERROR"
    ...
    - name: "totalAmount"
      ...
      scale: 2
      validateRules:
        - expression: "${totalAmount}.compareTo(java.math.BigDecimal.ZERO) > 0"
          message: "总金额必须大于0"
          severity: "ERROR"
        - expression: "${totalAmount}.scale() <= 2"
          message: "总金额精度不能超过2位小数"
          severity: "ERROR"
    ...
    - name: "taxRate"
      ...
      defaultValue: "13"
      validateRules:
        - expression: "${taxRate} >= 0 && ${taxRate} <= 100"
    ...
    - name: "taxAmount"
      ...
      precision: 19
      scale: 2
      expression: "${totalAmountExclTax} * ${taxRate} / 100"
    ...
    - name: "totalAmountExclTax"
      ...
      precision: 19
      scale: 2
      expression: "${totalAmount} / (1 + ${taxRate}/100)"
    ...
    - name: "status"
      ...
      required: true
      defaultValue: "DRAFT"
      options: ["DRAFT", "PENDING_APPROVAL", "APPROVED", "REJECTED", "PLACED", "RECEIVED", "PAID", "CLOSED", "CANCELLED"]
    
    - name: "items"
      type: "LIST"
      elementType: "PurchaseOrderItem"
      description: "订单明细"
      minItems: 1
      maxItems: 1000
      validateRules:
        - expression: "${items} != null && !${items}.isEmpty() && ${items}.size() <= 1000"
          message: "订单项不能为空且不能超过1000个"
          severity: "ERROR"
...
业务规则是企业业务逻辑的核心体现，通过统一的规则管理框架实现业务逻辑的集中化、可视化和可配置化管理，提高系统的灵活性和可维护性。

#### 2.2.1 验证规则

| 规则名称 | 适用实体 | 规则表达式 | 错误消息 | 严重程度 | 执行时机 | 实现方式 |
|---------|---------|-----------|---------|--------|---------|--------|
| orderCodeRule | PurchaseOrder | `${orderCode} != null && ${orderCode}.matches('PO\\d{8}\\d{4}')` | 订单编号格式必须为PO+8位日期+4位序号 | ERROR | 保存前 | 正则表达式 |
| totalAmountRule | PurchaseOrder | `${totalAmount} != null && ${totalAmount}.compareTo(java.math.BigDecimal.ZERO) > 0` | 总金额必须大于0 | ERROR | 保存前 | 条件表达式 |
| orderItemsRule | PurchaseOrder | `${items} != null && !${items}.isEmpty() && ${items}.size() <= 1000` | 订单项不能为空且不能超过1000个 | ERROR | 保存前 | 集合验证 |
| emergencyPurchaseRule | PurchaseOrder | `${orderType} == 'EMERGENCY' && ${expectedDeliveryDate} != null && ${expectedDeliveryDate}.isBefore(LocalDateTime.now().plusDays(7))` | 紧急采购的期望交货日期必须在7天内 | WARNING | 提交前 | 日期比较 |
| lineAmountCalculationRule | PurchaseOrderItem | `${amount} == ${quantity} * ${unitPrice}` | 金额必须等于数量乘以单价 | ERROR | 保存前 | 计算验证 |
| receivedQuantityValidationRule | PurchaseOrderItem | `${actualQuantity} <= ${quantity}` | 已收货数量不能大于订单数量 | ERROR | 更新前 | 数值比较 |

#### 2.2.2 审批规则

| 审批级别 | 适用条件 | 审批人角色 | 流程定义 | 超时设置 | 通知策略 |
|---------|---------|-----------|--------|---------|--------|
| LEVEL_1 | 金额 ≤ 5000元 | 部门经理 | SIMPLE_PURCHASE_FLOW | 24小时 | 邮件+站内信 |
| LEVEL_2 | 5000元 < 金额 ≤ 20000元 | 总监 | STANDARD_PURCHASE_FLOW | 48小时 | 邮件+站内信+短信 |
| LEVEL_3 | 20000元 < 金额 ≤ 100000元 | 副总经理 | COMPLEX_PURCHASE_FLOW | 72小时 | 邮件+站内信+短信+应用推送 |
| LEVEL_4 | 金额 > 100000元 | 总经理 | VIP_PURCHASE_FLOW | 96小时 | 邮件+站内信+短信+应用推送+电话提醒 |
| EMERGENCY | 紧急采购 | 快速审批通道 | EMERGENCY_PURCHASE_FLOW | 4小时 | 全渠道通知 |

#### 2.2.3 业务规则最佳实践

##### 2.2.3.1 规则设计原则

1. **分离关注点**：将业务规则与数据模型分离，通过注解或配置文件定义规则，便于独立管理和维护
2. **可组合性**：支持规则的组合和优先级管理，确保规则执行的正确性和有序性
3. **可测试性**：每个业务规则应独立可测试，支持单元测试和集成测试
4. **性能优化**：对频繁执行的规则进行性能优化，考虑缓存和异步执行
5. **可扩展性**：支持自定义规则函数和规则处理器，满足特殊业务场景需求
6. **版本管理**：对业务规则进行版本控制，支持规则的回溯和变更记录
...
/**
 * 业务规则引擎接口 - 采用责任链模式设计
 * <p>支持规则的注册、优先级排序和有序执行</p>
 */
public interface RuleEngine {
    
    /**
     * 执行规则验证
     * @param context 规则执行上下文，包含实体数据和执行环境
     * @return 规则执行结果，包含验证状态和错误信息
     */
     ...
     * @param rule 业务规则定义
     */
    void registerRule(BusinessRuleDefinition rule);
     ...
     * @return 按优先级排序的规则列表
     */
    List<BusinessRuleDefinition> getApplicableRules(Class<?> entityType, String eventType);
}
...
public class RuleExecutionResult {
    ...
    // 快捷方法
    public boolean hasErrors() { /* 实现 */ }
    public List<String> getErrorMessages() { /* 实现 */ }
    // getters, setters...
}

/**
 * 默认规则引擎实现
 * <p>基于责任链模式和命令模式的规则执行器</p>
 */
@Service
public class DefaultRuleEngine implements RuleEngine {
    ...
    public RuleExecutionResult executeRules(RuleExecutionContext context) {
        // 1. 获取适用规则
        List<BusinessRuleDefinition> applicableRules = getApplicableRules(
            context.getEntityType(), context.getEventType());
        ...
        for (BusinessRuleDefinition rule : applicableRules) {
            try {
                ...
                boolean conditionMet = expressionEvaluator.evaluate(
                    rule.getCondition(), context);
                    
                if (conditionMet) {
                    // 执行动作
                    executeRuleActions(rule, context, result);
                }
                
                // 记录规则执行历史
                recordRuleExecution(rule, context, conditionMet);
            } catch (Exception e) {
                // 处理规则执行异常
                result.addViolation(new RuleViolation(
                    rule.getName(), e.getMessage(), RuleSeverity.ERROR));
            }
        }
...
#### 2.3.1 YAML格式采购审批流程定义

```yaml
# metadata/purchase-approval-workflow.yaml
workflow:
  ...
  tasks:
    - id: "department_approval"
      name: "部门经理审批"
      type: "user_task"
      assigneeExpression: "${getDepartmentManager(creatorId)}"
      priorityExpression: "${totalAmount > 10000 ? 'HIGH' : 'NORMAL'}"
      timeoutExpression: "PT24H"
      
    - id: "financial_approval"
      name: "财务审批"
      type: "user_task"
      assigneeExpression: "${getFinancialApprover(departmentId)}"
      priorityExpression: "${totalAmount > 50000 ? 'HIGH' : 'NORMAL'}"
      conditionExpression: "${totalAmount > 20000}"
      
    - id: "executive_approval"
      name: "高管审批"
      type: "user_task"
      assigneeExpression: "${getExecutiveApprover()}"
      priorityExpression: "HIGH"
      conditionExpression: "${totalAmount > 100000}"
      
    - id: "emergency_approval"
      name: "紧急审批"
      type: "user_task"
      assigneeExpression: "${getEmergencyApprover()}"
      priorityExpression: "URGENT"
      timeoutExpression: "PT2H"
      conditionExpression: "${orderType == 'EMERGENCY'}"
  ...
  transitions:
    - source: "start_approval"
      target: "type_gateway"
      
    - source: "type_gateway"
      target: "emergency_approval"
      conditionExpression: "${orderType == 'EMERGENCY'}"
    ...
    - source: "amount_gateway"
      target: "end_approval"
      conditionExpression: "${totalAmount <= 20000}"
      
    - source: "amount_gateway"
      target: "financial_approval"
      conditionExpression: "${totalAmount > 20000 && totalAmount <= 100000}"
      
    - source: "amount_gateway"
      target: "executive_approval"
      conditionExpression: "${totalAmount > 100000}"
      
    - source: "financial_approval"
      target: "end_approval"
      
    - source: "executive_approval"
      target: "end_approval"
      
    - source: "emergency_approval"
      target: "end_approval"
      
  listeners:
    - eventType: "TASK_COMPLETED"
      listenerClass: "com.bone.smartmeta.listener.PurchaseApprovalTaskListener"
      
    - eventType: "WORKFLOW_COMPLETED"
      listenerClass: "com.bone.smartmeta.listener.PurchaseApprovalCompleteListener"
      
    - eventType: "WORKFLOW_CANCELLED"
      listenerClass: "com.bone.smartmeta.listener.PurchaseApprovalCancelListener"
      
  businessRules:
    - name: "approval_limits"
      rules:
        - conditionExpression: "${totalAmount <= 5000}"
          actionExpression: "${setApprovalLevel('LEVEL_1')}"
          
        - conditionExpression: "${totalAmount > 5000 && totalAmount <= 20000}"
          actionExpression: "${setApprovalLevel('LEVEL_2')}"
          
        - conditionExpression: "${totalAmount > 20000 && totalAmount <= 100000}"
          actionExpression: "${setApprovalLevel('LEVEL_3')}"
          
        - conditionExpression: "${totalAmount > 100000}"
          actionExpression: "${setApprovalLevel('LEVEL_4')}"
          
        - conditionExpression: "${orderType == 'EMERGENCY'}"
          actionExpression: "${setApprovalLevel('EMERGENCY')}"
      
  notifications:
    - eventType: "TASK_ASSIGNED"
      template: "purchase_task_assigned"
      channels: ["EMAIL", "SMS", "APP"]
      
    - eventType: "APPROVAL_REJECTED"
      template: "purchase_approval_rejected"
      channels: ["EMAIL", "APP"]
      
    - eventType: "APPROVAL_APPROVED"
      template: "purchase_approval_approved"
      channels: ["EMAIL", "APP"]
...
元数据引擎采用**分层架构**结合**微服务设计理念**，确保系统的可扩展性、可维护性和性能优化。整体架构遵循**高内聚低耦合**原则，通过清晰的接口定义和组件边界，实现灵活的扩展和集成能力。
...
graph TB
    ...
    subgraph "元数据引擎核心"
        ...
        B -->|使用| P[CodeGenerationEngine]
        B -->|使用| Q[ExpressionEngine]
        B -->|使用| R[MetadataNotificationCenter]
        B -->|使用| S[CustomExtensionRegistry]
    end
...
**业务应用层**：
- **采购业务服务**：负责采购订单的创建、审核、管理等核心业务流程，通过元数据引擎实现业务逻辑的可配置化
- **审批流程服务**：处理采购订单的多级审批逻辑，利用元数据驱动的工作流定义
- **报表分析服务**：基于元数据生成采购业务报表和数据分析，支持动态报表定义
- **供应商管理服务**：管理供应商信息，与采购订单业务集成
- **预算管理服务**：进行预算控制和管理，与采购业务规则集成

**元数据引擎核心**：
- **EntityMetadata**：实体元数据管理，包含实体定义、字段映射、索引配置等
- **FieldMetadata**：字段元数据管理，包含字段类型、验证规则、计算表达式等
- **BusinessRuleEngine**：业务规则引擎，采用责任链模式实现规则的组合执行
- **FieldCalculationEngine**：字段计算引擎，处理字段间的计算关系，支持复杂表达式
- **ValidationEngine**：验证引擎，执行数据验证逻辑，支持自定义验证器扩展
- **MetadataCacheManager**：元数据缓存管理器，采用多级缓存策略提高性能
- **MetadataVersionController**：版本控制器，管理元数据的版本变更和回滚
- **CodeGenerationEngine**：代码生成引擎，基于元数据生成实体类和服务类代码
- **ExpressionEngine**：表达式引擎，解析和执行动态表达式，支持自定义函数
- **MetadataNotificationCenter**：元数据变更通知中心，采用发布-订阅模式
- **CustomExtensionRegistry**：自定义扩展注册表，支持SPI机制的插件扩展

**数据持久层**：
- **MetadataRepository**：元数据仓库，提供元数据的CRUD操作，支持多租户隔离
- **RedisCache**：分布式缓存，缓存热点元数据，提高访问性能，支持缓存预热和刷新
- **关系数据库**：持久化存储元数据信息，支持事务和数据一致性
- **Elasticsearch**：提供元数据的全文检索和复杂查询能力

#### 3.1.2 设计模式应用

元数据引擎设计中应用了多种企业级设计模式，确保系统的灵活性和可扩展性：

1. **代理模式**：SmartMetadataEngine作为代理，统一管理元数据访问，提供缓存、日志等横切关注点
2. **责任链模式**：业务规则引擎和验证引擎采用责任链设计，支持规则的动态组合和优先级管理
3. **命令模式**：将规则执行封装为命令对象，支持规则的序列化和异步执行
4. **观察者模式**：元数据变更通知中心采用观察者模式，实现松耦合的事件通知
5. **策略模式**：表达式引擎支持多种策略实现，适应不同的表达式语法
6. **工厂模式**：元数据对象的创建采用工厂方法模式，支持扩展和定制
7. **装饰器模式**：为核心组件添加额外功能，如缓存、审计、日志等
8. **适配器模式**：支持不同数据源和系统的集成，提供统一的访问接口
9. **模板方法模式**：定义算法骨架，允许子类覆盖特定步骤
10. **外观模式**：为复杂的子系统提供简单的统一接口

#### 3.1.3 高可用与可伸缩性设计

- **无状态设计**：元数据引擎核心组件采用无状态设计，支持水平扩展
- **集群部署**：支持集群部署，通过Redis缓存实现集群间元数据同步
- **熔断降级**：集成熔断机制，当元数据服务不可用时提供降级方案
- **限流保护**：实现请求限流，防止系统过载
- **健康检查**：提供组件健康状态检查，支持自动恢复和故障转移
...
```mermaid
sequenceDiagram
    participant Client as 采购业务客户端
    participant PurchaseService as 采购业务服务
    participant MetaEngine as 元数据引擎
    participant RuleEngine as 业务规则引擎
    participant ValidationEngine as 验证引擎
    participant Cache as 元数据缓存
    participant DB as 数据库
    
    Client->>PurchaseService: 创建采购订单
    PurchaseService->>MetaEngine: 验证订单数据
    MetaEngine->>Cache: 查询订单实体元数据
    alt 缓存命中
        Cache-->>MetaEngine: 返回元数据
    else 缓存未命中
        MetaEngine->>DB: 查询元数据
        DB-->>MetaEngine: 返回元数据
        MetaEngine->>Cache: 更新缓存
    end
    MetaEngine->>ValidationEngine: 执行验证规则
    ValidationEngine->>RuleEngine: 解析并执行表达式
    RuleEngine-->>ValidationEngine: 返回验证结果
    alt 验证通过
        ValidationEngine-->>MetaEngine: 验证成功
        MetaEngine->>RuleEngine: 执行计算规则
        RuleEngine-->>MetaEngine: 返回计算结果
        MetaEngine-->>PurchaseService: 返回验证和计算结果
        PurchaseService->>DB: 保存订单数据
        PurchaseService-->>Client: 返回成功响应
    else 验证失败
        ValidationEngine-->>MetaEngine: 验证失败，返回错误信息
        MetaEngine-->>PurchaseService: 返回验证失败结果
        PurchaseService-->>Client: 返回错误信息
    end
```
...
**MetadataEngine**是元数据引擎的核心接口，采用**外观模式**设计，为上层应用提供统一的元数据访问入口。它封装了元数据管理的复杂逻辑，提供简洁、一致的API，同时集成了缓存、事件、版本控制等企业级特性。
...
```java
/**
 * 元数据引擎核心接口 - 企业级元数据管理的统一入口
 * <p>采用外观模式设计，为上层应用提供简洁一致的API</p>
 */
public interface MetadataEngine {
     ...
     * @throws MetadataAlreadyExistsException 当元数据已存在时抛出
     */
     ...
     * @param metadata 实体元数据对象
     * @return 更新后的实体元数据
     * @throws MetadataNotFoundException 当元数据不存在时抛出
     * @throws MetadataValidationException 当元数据验证失败时抛出
     */
     ...
     * @throws MetadataNotFoundException 当元数据不存在时抛出
     */
    EntityMetadata getEntityMetadata(String entityName, String tenantId);
     ...
     * @return 分页查询结果
     */
    Page<EntityMetadata> queryEntityMetadata(MetadataQuery query, Pageable pageable);
     ...
     * @throws MetadataNotFoundException 当元数据不存在时抛出
     */
    void deleteEntityMetadata(String entityName);
     ...
     * @return 批量验证结果
     */
    List<ValidationResult> validateEntities(String entityName, List<Map<String, Object>> entityDataList);
     ...
     * @return 规则执行结果
     */
    RuleExecutionResult executeBusinessRule(String entityName, String ruleName, Map<String, Object> context);
     ...
     * @return 版本历史列表
     */
    List<MetadataVersion> getMetadataVersions(String entityName);
     ...
     * @return 回滚后的实体元数据
     */
    EntityMetadata rollbackToVersion(String entityName, String versionId);
     ...
     */
    void registerCustomValidator(String entityName, EntityValidator validator);
     ...
     * @return 生成的代码内容
     */
    String generateCode(String entityName, String templateType);
}
```
...
#### 3.2.3 CodeGenerationEngine

**CodeGenerationEngine**是企业级代码自动化生成的核心组件，采用**模板方法模式**和**策略模式**设计，支持多种编程语言和代码类型的灵活生成。它与元数据引擎紧密集成，能够将元数据定义转换为高质量、符合企业规范的代码资产。

- **职责**：基于元数据生成实体类、服务类、控制器等代码
- **主要功能**：
  - 模板管理与缓存
  - 多语言代码生成
  - 自定义模板扩展
...
**设计亮点**：
- 采用**工厂模式**和**构建者模式**组合设计，支持复杂代码生成流程
- 集成**缓存机制**提升模板加载和代码生成性能
- 支持**插件化架构**，允许第三方扩展代码生成能力
- 内置**代码质量检查**，确保生成代码符合企业规范

代码生成引擎基于FreeMarker模板引擎实现，支持多种代码模板，可以根据元数据定义自动生成符合规范的代码，显著提高开发效率。

```java
/**
 * 代码生成引擎接口 - 企业级代码自动生成的统一抽象
 * <p>支持多种编程语言和代码类型的灵活生成</p>
 */
public interface CodeGenerationEngine {
     ...
     * @param templateType 模板类型
     * @return 生成的代码内容
     * @throws CodeGenerationException 代码生成失败时抛出
     * @throws TemplateNotFoundException 模板不存在时抛出
     */
    String generateCode(EntityMetadata metadata, String templateType);
    
    /**
     * 批量生成实体代码
     * @param metadataList 实体元数据列表
     * @param templateType 模板类型
     * @return 生成的代码映射，key为实体名称，value为生成的代码内容
     * @throws CodeGenerationException 代码生成失败时抛出
     */
    Map<String, String> batchGenerateCode(List<EntityMetadata> metadataList, String templateType);
    
    /**
     * 生成包含依赖关系的代码包
     * @param metadata 实体元数据
     * @param templateTypes 需要生成的模板类型列表
     * @return 代码包，包含所有生成的代码文件
     * @throws CodeGenerationException 代码生成失败时抛出
     */
    CodePackage generateCodePackage(EntityMetadata metadata, List<String> templateTypes);
    
    /**
     * 注册自定义模板
     * @param templateType 模板类型
     * @param templateContent 模板内容
     * @throws TemplateRegistrationException 模板注册失败时抛出
     */
    void registerTemplate(String templateType, String templateContent);
     ...
     * @throws TemplateRegistrationException 模板注册失败时抛出
     */
    void registerFileTemplate(String templateType, String templatePath, Charset charset);
     ...
     * @throws TemplateNotFoundException 模板不存在时抛出
     */
    String getTemplateContent(String templateType);
    
    /**
     * 设置代码格式化器
     * @param formatter 代码格式化器
     */
    void setCodeFormatter(CodeFormatter formatter);
     ...
     * @return 统计信息
     */
    CodeGenerationStatistics getStatistics();
}

/**
 * 默认代码生成引擎实现 - 基于FreeMarker的高性能代码生成器
 * <p>集成多级缓存机制和插件化架构，支持企业级大规模代码生成需求</p>
 */
public class DefaultCodeGenerationEngine implements CodeGenerationEngine {
    // 模板缓存 - 使用LoadingCache实现高性能模板加载
    private final LoadingCache<String, Template> templateCache;
    // FreeMarker配置
    private final Configuration configuration;
    // 代码格式化器
    private volatile CodeFormatter codeFormatter;
    // 代码生成统计信息
    private final AtomicLong codeGenerationCount = new AtomicLong(0);
    private final AtomicLong templateLoadTime = new AtomicLong(0);
    private final AtomicLong codeGenerationTime = new AtomicLong(0);
    // 多租户隔离键
    ...
    @Override
    public String generateCode(EntityMetadata metadata, String templateType) {
        ...
        try {
            ...
            // 获取模板
            Template template = templateCache.get(templateType);
            
            // 准备模板数据
            Map<String, Object> dataModel = prepareDataModel(metadata);
            ...
            // 代码格式化
            String formattedCode = formatCode(rawCode, templateType);
            
            // 更新统计信息
            codeGenerationCount.incrementAndGet();
            codeGenerationTime.addAndGet(System.currentTimeMillis() - startTime);
            
            return formattedCode;
        } catch (Exception e) {
            if (e instanceof TemplateNotFoundException) {
                throw (TemplateNotFoundException) e;
            }
            throw new CodeGenerationException("Failed to generate code for " + metadata.getName() + 
                                             " with template " + templateType, e);
        }
    }
    
    @Override
    public Map<String, String> batchGenerateCode(List<EntityMetadata> metadataList, String templateType) {
        if (metadataList == null || metadataList.isEmpty()) {
            return Collections.emptyMap();
        }
        
        Map<String, String> result = new HashMap<>(metadataList.size());
        for (EntityMetadata metadata : metadataList) {
            try {
                String code = generateCode(metadata, templateType);
                result.put(metadata.getName(), code);
            } catch (Exception e) {
                throw new CodeGenerationException("Failed to batch generate code for " + metadata.getName(), e);
            }
        }
    ...
    @Override
    public CodePackage generateCodePackage(EntityMetadata metadata, List<String> templateTypes) {
        CodePackage codePackage = new CodePackage(metadata.getName());
        
        for (String templateType : templateTypes) {
            try {
                String code = generateCode(metadata, templateType);
                String fileName = generateFileName(metadata, templateType);
                codePackage.addFile(fileName, code);
            } catch (Exception e) {
                throw new CodeGenerationException("Failed to generate code package for " + metadata.getName(), e);
            }
        }
    ...
    @Override
    public void registerTemplate(String templateType, String templateContent) {
        try {
            Template template = new Template(templateType, new StringReader(templateContent), configuration);
            templateCache.put(templateType, template);
        } catch (IOException e) {
            throw new TemplateRegistrationException("Failed to register template: " + templateType, e);
        }
    }
    
    @Override
    public void registerFileTemplate(String templateType, String templatePath, Charset charset) {
        try {
            ...
            String templateContent = Files.readString(templateFile.toPath(), charset);
            registerTemplate(templateType, templateContent);
        } catch (IOException e) {
            throw new TemplateRegistrationException("Failed to register file template: " + templatePath, e);
        }
    }
    
    @Override
    public List<String> getSupportedTemplateTypes() {
        return new ArrayList<>(templateCache.asMap().keySet());
    }
    
    @Override
    public String getTemplateContent(String templateType) {
        try {
            Template template = templateCache.get(templateType);
            StringWriter writer = new StringWriter();
            ...
            throw new TemplateNotFoundException("Template not found: " + templateType, e);
        }
    }
    ...
    public CodeGenerationStatistics getStatistics() {
        ...
        return new CodeGenerationStatistics.Builder()
                .setCodeGenerationCount(codeGenerationCount.get())
                .setTotalGenerationTime(codeGenerationTime.get())
                .setTemplateCacheHitRate(stats.hitRate())
                .setTemplateCacheEvictionCount(stats.evictionCount())
                .setTemplateLoadTime(templateLoadTime.get())
                .build();
    }
    ...
    public void setCurrentTenantId(String tenantId) {
        this.tenantIdHolder.set(tenantId);
    }
    ...
    public void clearCurrentTenantId() {
        this.tenantIdHolder.remove();
    }
    
    // 加载模板
    private Template loadTemplate(String templateType) {
        long startTime = System.currentTimeMillis();
        try {
            return configuration.getTemplate(templateType + ".ftl");
        } catch (IOException e) {
            throw new TemplateNotFoundException("Failed to load template: " + templateType, e);
        } finally {
            templateLoadTime.addAndGet(System.currentTimeMillis() - startTime);
        }
    }
    
    // 准备模板数据
    private Map<String, Object> prepareDataModel(EntityMetadata metadata) {
        Map<String, Object> dataModel = new HashMap<>();
        dataModel.put("entity", metadata);
        dataModel.put("fields", metadata.getFields());
        dataModel.put("now", new Date());
        dataModel.put("tenantId", tenantIdHolder.get());
        dataModel.put("packageName", metadata.getPackageName());
        dataModel.put("baseClassName", metadata.getBaseClassName());
        dataModel.put("auditEnabled", metadata.isAuditEnabled());
        dataModel.put("relations", metadata.getRelationships());
        dataModel.put("rules", metadata.getRules());
        dataModel.put("author", "SmartMeta Engine");
        dataModel.put("generatedDate", DateTimeFormatter.ISO_LOCAL_DATE.format(LocalDate.now()));
        return dataModel;
    }
    
    // 格式化代码
    private String formatCode(String rawCode, String templateType) {
        if (codeFormatter != null) {
            try {
                return codeFormatter.format(rawCode, templateType);
            } catch (Exception e) {
                // 格式化失败，返回原始代码
                log.warn("Code formatting failed, using raw code: {}", e.getMessage());
            }
        }
        return rawCode;
    }
    ...
    // 生成文件名
    private String generateFileName(EntityMetadata metadata, String templateType) {
        switch (templateType) {
            case "entity":
                return metadata.getName() + ".java";
            case "service":
                return metadata.getName() + "Service.java";
            case "controller":
                return metadata.getName() + "Controller.java";
            case "repository":
                return metadata.getName() + "Repository.java";
            default:
                return metadata.getName() + "-" + templateType + ".java";
        }
    }
    ...
    private void loadBuiltInTemplates() {
        try {
            // 加载内置模板文件
            templateCache.put("entity", configuration.getTemplate("entity.ftl"));
            templateCache.put("service", configuration.getTemplate("service.ftl"));
            templateCache.put("controller", configuration.getTemplate("controller.ftl"));
            templateCache.put("repository", configuration.getTemplate("repository.ftl"));
            templateCache.put("dto", configuration.getTemplate("dto.ftl"));
            templateCache.put("mapper", configuration.getTemplate("mapper.ftl"));
            templateCache.put("validator", configuration.getTemplate("validator.ftl"));
            templateCache.put("enum", configuration.getTemplate("enum.ftl"));
            templateCache.put("test", configuration.getTemplate("test.ftl"));
            templateCache.put("config", configuration.getTemplate("config.ftl"));
        } catch (IOException e) {
            throw new TemplateInitializationException("Failed to load built-in templates", e);
        }
    }
}
...
public class TemplateRegistrationException extends RuntimeException {
    ...
    public TemplateRegistrationException(String message, Throwable cause) {
        super(message, cause);
    }
}
...
public class CodeFormattingException extends RuntimeException {
    public CodeFormattingException(String message) {
        super(message);
    }
    
    public CodeFormattingException(String message, Throwable cause) {
        super(message, cause);
    }
}
...
public interface CodeFormatter {
     ...
     * @param templateType 模板类型
     * @return 格式化后的代码
     * @throws CodeFormattingException 代码格式化失败时抛出
     */
    String format(String code, String templateType);
}
...
public class CodePackage {
    ...
    // 导出为ZIP文件
    public byte[] exportAsZip() throws IOException {
...
public class CodeGenerationStatistics {
    private final long codeGenerationCount;
    private final long totalGenerationTime;
    private final double templateCacheHitRate;
    private final long templateCacheEvictionCount;
    private final long templateLoadTime;
    private final long startTime;
    private final long endTime;
    
    private CodeGenerationStatistics(Builder builder) {
        this.codeGenerationCount = builder.codeGenerationCount;
        this.totalGenerationTime = builder.totalGenerationTime;
        this.templateCacheHitRate = builder.templateCacheHitRate;
        this.templateCacheEvictionCount = builder.templateCacheEvictionCount;
        this.templateLoadTime = builder.templateLoadTime;
        this.startTime = builder.startTime;
        this.endTime = builder.endTime;
    }
    
    public long getCodeGenerationCount() {
    ...
    public double getAverageGenerationTime() {
        return codeGenerationCount > 0 ? (
            (double) totalGenerationTime / codeGenerationCount
        ) : 0;
    }
    
    public double getTemplateCacheHitRate() {
        return templateCacheHitRate;
    }
    
    public long getTemplateCacheEvictionCount() {
        return templateCacheEvictionCount;
    }
    
    public long getTemplateLoadTime() {
        return templateLoadTime;
    }
    ...
    public String toString() {
        return String.format(
            ...
            "codeGenerationCount=%d, " +
            "totalGenerationTime=%dms, " +
            "averageGenerationTime=%.2fms, " +
            "templateCacheHitRate=%.2f%%, " +
            "templateCacheEvictionCount=%d, " +
            "templateLoadTime=%dms" +
            "}",
            codeGenerationCount,
            totalGenerationTime,
            getAverageGenerationTime(),
            templateCacheHitRate * 100,
            templateCacheEvictionCount,
            templateLoadTime
        );
    }
    ...
    public static class Builder {
        ...
        private double templateCacheHitRate;
        private long templateCacheEvictionCount;
        private long templateLoadTime;
        private long startTime;
        private long endTime;
        
        public Builder() {
            this.startTime = System.currentTimeMillis();
            this.templateCacheHitRate = 0.0;
        }
        
        public Builder setCodeGenerationCount(long codeGenerationCount) {
            this.codeGenerationCount = codeGenerationCount;
            return this;
        }
        
        public Builder setTotalGenerationTime(long totalGenerationTime) {
            this.totalGenerationTime = totalGenerationTime;
            return this;
        }
        
        public Builder setTemplateCacheHitRate(double templateCacheHitRate) {
            this.templateCacheHitRate = templateCacheHitRate;
            return this;
        }
        
        public Builder setTemplateCacheEvictionCount(long templateCacheEvictionCount) {
            this.templateCacheEvictionCount = templateCacheEvictionCount;
            return this;
        }
        
        public Builder setTemplateLoadTime(long templateLoadTime) {
            this.templateLoadTime = templateLoadTime;
            return this;
        }
        ...
        public CodeGenerationStatistics build() {
            if (this.endTime == 0) {
                this.endTime = System.currentTimeMillis();
            }
            return new CodeGenerationStatistics(this);
        }
    }
}
...
@Data
public class ${entity.name} extends ${entity.baseClassName} {
...
import ${entity.packageName}.repository.${entity.name}Repository;
...
public class ${entity.name}Service {
    ...
    public Optional<${entity.name}> findById(Long id) {
        return ${entity.name?lower_case}Repository.findById(id);
    }
    ...
    public List<${entity.name}> findAll() {
        return ${entity.name?lower_case}Repository.findAll();
    }
     ...
     */
    @Transactional
    public ${entity.name} save(${entity.name} ${entity.name?lower_case}) {
        log.info("Saving ${entity.name}: {}", ${entity.name?lower_case}.getId());
        return ${entity.name?lower_case}Repository.save(${entity.name?lower_case});
    }
    ...
    public void deleteById(Long id) {
        log.info("Deleting ${entity.name} by id: {}", id);
        ${entity.name?lower_case}Repository.deleteById(id);
    }
...
@RestController
@RequestMapping("/api/${entity.name?lower_case}")
public class ${entity.name}Controller {
    ...
    @PostMapping
    public ResponseEntity<${entity.name}> create(@RequestBody ${entity.name} entity) {
...
UI组件引擎负责根据实体元数据自动生成前端UI界面，支持表单、表格、详情页等多种展示形式，实现前后端一体化的元数据驱动开发。
...
```java
public interface UiComponentEngine {
    /**
     * 生成表单配置
     * @param entityName 实体名称
     * @param options 配置选项
     * @return 表单配置
     */
    FormConfig generateFormConfig(String entityName, Map<String, Object> options);
     ...
     * @return 表格配置
     */
    TableConfig generateTableConfig(String entityName, Map<String, Object> options);
     ...
     * @return 详情页配置
     */
    DetailConfig generateDetailConfig(String entityName, Map<String, Object> options);
     ...
     */
    void registerFieldComponent(String fieldType, ComponentInfo componentInfo);
     ...
     * @return UI配置
     */
    UiConfig getEntityUiConfig(String entityName);
}

public class DefaultUiComponentEngine implements UiComponentEngine {
    private final MetadataEngine metadataEngine;
    private final Map<String, ComponentInfo> fieldComponentMap = new HashMap<>();
    private final Map<String, UiConfig> entityUiConfigCache = new ConcurrentHashMap<>();
    
    public DefaultUiComponentEngine(MetadataEngine metadataEngine) {
        this.metadataEngine = metadataEngine;
        initializeDefaultComponents();
    }
    
    @Override
    public FormConfig generateFormConfig(String entityName, Map<String, Object> options) {
        EntityMetadata metadata = metadataEngine.getEntityMetadata(entityName);
        FormConfig formConfig = new FormConfig();
        formConfig.setTitle(metadata.getDisplayName());
        formConfig.setFields(convertToFormFields(metadata.getFields(), options));
        formConfig.setLayout(metadata.getUiConfig().getFormLayout());
        return formConfig;
    }
    
    @Override
    public TableConfig generateTableConfig(String entityName, Map<String, Object> options) {
        EntityMetadata metadata = metadataEngine.getEntityMetadata(entityName);
        TableConfig tableConfig = new TableConfig();
        tableConfig.setTitle(metadata.getDisplayName());
        tableConfig.setColumns(convertToTableColumns(metadata.getFields(), options));
        tableConfig.setPagination(true);
        tableConfig.setSearchable(true);
        return tableConfig;
    }
    
    @Override
    public DetailConfig generateDetailConfig(String entityName, Map<String, Object> options) {
        EntityMetadata metadata = metadataEngine.getEntityMetadata(entityName);
        DetailConfig detailConfig = new DetailConfig();
        detailConfig.setTitle(metadata.getDisplayName());
        detailConfig.setFields(convertToDetailFields(metadata.getFields(), options));
        return detailConfig;
    }
    
    @Override
    public void registerFieldComponent(String fieldType, ComponentInfo componentInfo) {
        fieldComponentMap.put(fieldType, componentInfo);
    }
    
    @Override
    public UiConfig getEntityUiConfig(String entityName) {
        return entityUiConfigCache.computeIfAbsent(entityName, name -> {
            EntityMetadata metadata = metadataEngine.getEntityMetadata(name);
            return metadata.getUiConfig();
        });
    }
    
    private List<FormField> convertToFormFields(List<FieldMetadata> fields, Map<String, Object> options) {
        List<FormField> formFields = new ArrayList<>();
        for (FieldMetadata field : fields) {
            FormField formField = new FormField();
            formField.setName(field.getName());
            formField.setLabel(field.getDisplayName());
            formField.setRequired(field.isRequired());
            formField.setComponent(getComponentForField(field));
            formField.setRules(convertToFormRules(field.getValidationRules()));
            formFields.add(formField);
        }
        return formFields;
    }
    
    private ComponentInfo getComponentForField(FieldMetadata field) {
        return fieldComponentMap.getOrDefault(field.getType(), getDefaultComponent(field.getType()));
    }
    
    private void initializeDefaultComponents() {
        // 初始化默认组件映射
        registerFieldComponent("STRING", new ComponentInfo("Input", new HashMap<>()));
        registerFieldComponent("INTEGER", new ComponentInfo("InputNumber", Map.of("precision", 0)));
        registerFieldComponent("DECIMAL", new ComponentInfo("InputNumber", Map.of("precision", 2)));
        registerFieldComponent("BOOLEAN", new ComponentInfo("Switch", new HashMap<>()));
        registerFieldComponent("DATE", new ComponentInfo("DatePicker", new HashMap<>()));
        registerFieldComponent("DATETIME", new ComponentInfo("DateTimePicker", new HashMap<>()));
        registerFieldComponent("ENUM", new ComponentInfo("Select", new HashMap<>()));
        registerFieldComponent("REFERENCE", new ComponentInfo("Select", new HashMap<>()));
    }
    
    private ComponentInfo getDefaultComponent(String fieldType) {
        return new ComponentInfo("Input", new HashMap<>());
    }
    
    private List<FormRule> convertToFormRules(List<ValidationRule> validationRules) {
        List<FormRule> formRules = new ArrayList<>();
        for (ValidationRule rule : validationRules) {
            FormRule formRule = new FormRule();
            formRule.setType(rule.getType());
            formRule.setMessage(rule.getErrorMessage());
            formRule.setOptions(rule.getOptions());
            formRules.add(formRule);
        }
        return formRules;
    }
    
    private List<TableColumn> convertToTableColumns(List<FieldMetadata> fields, Map<String, Object> options) {
        List<TableColumn> columns = new ArrayList<>();
        for (FieldMetadata field : fields) {
            if (!field.isHiddenInList()) {
                TableColumn column = new TableColumn();
                column.setTitle(field.getDisplayName());
                column.setDataIndex(field.getName());
                column.setKey(field.getName());
                column.setWidth(field.getWidth());
                column.setSortable(field.isSortable());
                column.setFilterable(field.isFilterable());
                columns.add(column);
            }
        }
        return columns;
    ...
    private List<DetailField> convertToDetailFields(List<FieldMetadata> fields, Map<String, Object> options) {
        List<DetailField> detailFields = new ArrayList<>();
        for (FieldMetadata field : fields) {
            DetailField detailField = new DetailField();
            detailField.setName(field.getName());
            detailField.setLabel(field.getDisplayName());
            detailField.setFormatter(field.getFormatter());
...
const PurchaseOrderForm = ({ entityId, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchFormConfig = async () => {
      try {
        const response = await axios.get('/api/meta/form-config/PurchaseOrder', {
          params: { mode: entityId ? 'edit' : 'create' }
        });
        setFormConfig(response.data);
        
        // 如果是编辑模式，加载实体数据
        if (entityId) {
          const entityResponse = await axios.get(`/api/purchase-orders/${entityId}`);
          form.setFieldsValue(entityResponse.data);
        }
      } catch (error) {
  ...
  const renderField = (field) => {
    const { name, label, required, component, rules, options = {} } = field;
      ...
      case 'DatePicker':
        return (
          <Form.Item 
            ...
            key={name}
          >
            <DatePicker placeholder={`请选择${label}`} />
          </Form.Item>
        );
      ...
      case 'DateTimePicker':
        return (
          <Form.Item 
            ...
            key={name}
          >
            <DatePicker showTime placeholder={`请选择${label}`} />
          </Form.Item>
        );
        ...
        return (
          ...
          >
            <InputNumber 
              ...
              precision={component.options?.precision || 2}
            />
          </Form.Item>
        );
  ...
  const onSubmit = async (values) => {
    ...
    try {
      let response;
      if (entityId) {
        response = await axios.put(`/api/purchase-orders/${entityId}`, values);
      } else {
        response = await axios.post('/api/purchase-orders', values);
      }
      ...
      message.error(entityId ? '更新失败' : '创建失败');
...
1. **元数据版本控制**：对元数据进行版本管理，支持回滚和变更历史追踪
2. **元数据缓存策略**：使用多级缓存机制提高元数据访问性能
3. **元数据校验**：在元数据注册时进行完整性和一致性校验
4. **模块化设计**：将元数据按业务领域进行模块化管理
5. **渐进式采用**：从核心实体开始，逐步扩展到整个业务领域
...
1. **数据验证**：前后端双重验证，防止恶意输入
2. **权限控制**：基于元数据的细粒度权限控制
3. **敏感信息保护**：敏感字段加密存储和传输
4. **审计日志**：记录所有元数据变更和重要操作

#### 4.4 扩展性设计

1. **插件机制**：支持通过插件扩展元数据引擎功能
2. **事件驱动**：基于事件的架构，支持自定义事件处理器
3. **SPI机制**：使用Java SPI实现服务发现和扩展
4. **自定义组件**：支持注册和使用自定义UI组件

### 5. 总结与展望

本文档详细介绍了采购核心业务模型与元数据引擎的集成设计方案，涵盖了实体定义、业务规则、状态流转、元数据引擎架构、代码生成和UI组件引擎等多个方面。通过元数据驱动的方式，可以极大地提高开发效率，减少重复代码，提升系统的灵活性和可维护性。

未来的发展方向包括：

1. **AI辅助设计**：利用人工智能技术辅助元数据设计和优化
2. **低代码平台**：基于元数据引擎构建低代码开发平台
3. **跨平台支持**：扩展到移动端和其他前端框架
4. **实时协作**：支持多用户实时协作编辑元数据
5. **云原生适配**：优化设计以适应云原生环境的部署和扩展

通过持续改进和创新，元数据驱动的开发模式将成为企业级应用开发的重要趋势，为企业数字化转型提供强大的技术支撑。
        try {
