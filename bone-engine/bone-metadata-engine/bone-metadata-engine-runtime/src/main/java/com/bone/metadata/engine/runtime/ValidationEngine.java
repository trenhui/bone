...
import com.bone.metadata.engine.runtime.validation.strategy.RelationshipValidationStrategy;
import com.bone.metadata.engine.runtime.validation.strategy.StrategyValidationExecutor;
 ...
 *
 * <p>Core component responsible for validating entity data against metadata-defined rules.
 * Implements the strategy pattern for flexible validation approaches. Provides caching, batch
 * processing, and performance monitoring capabilities.
 */
@Component
public class ValidationEngine
    implements InitializingBean, RelationshipValidationStrategy.EntityMetadataProvider {
  ...
  @Setter private boolean cacheEnabled = true;
  @Setter private boolean failFast = false;

  // Batch processing configuration
  @Getter @Setter private int batchThreshold = DEFAULT_BATCH_THRESHOLD;
  public static final int DEFAULT_BATCH_THRESHOLD = 100;

  // Cache configuration
  @Getter @Setter private long cacheExpirationTime = TimeUnit.MINUTES.toMillis(30);
  @Getter @Setter private int maxCacheSize = 1000;

  // Caches
  private final ConcurrentHashMap<String, CachedEntityMetadata> entityMetadataCache =
   ...
   */
  public ValidationEngine(
      MetadataRepository metadataRepository, ExpressionEngine expressionEngine) {
  ...
  public void afterPropertiesSet() throws Exception {
    LOGGER.info(
        "Initializing validation engine - cache enabled: {}, fail fast mode: {}, batch threshold: {}, "
  ...
  /**
   * Schedules periodic cache cleanup task to prevent memory leaks. Uses a dedicated daemon thread
  ...
  /**
   * Cleans up expired and excessive cache entries to maintain memory efficiency. Removes entries
   * that have expired based on the configured expiration time. If cache size exceeds maximum limit,
  ...
  private void cleanupCache() {
    try {
      ...
      entityMetadataCache
          .entrySet()
          .removeIf(entry -> entry.getValue().isExpired(cacheExpirationTime));

      // If cache still exceeds maximum size, remove oldest entries
      if (entityMetadataCache.size() > maxCacheSize) {
        List<Map.Entry<String, CachedEntityMetadata>> entries =
            new ArrayList<>(entityMetadataCache.entrySet());
      ...
      if (beforeSize > afterSize) {
        LOGGER.debug(
            "Cache cleanup completed, removed {} expired entries, current cache size: {}",
  ...
  /**
   * Cleans up the method reflection cache when it exceeds size limits. Maintains memory efficiency
   * by removing oldest entries when cache grows too large.
   */
  private void cleanupMethodReflectionCache() {
    ...
    if (methodReflectionCache.size() > maxCacheSize * 2) {
      ...
      LOGGER.debug(
          "Method reflection cache cleaned up, current size: {}", methodReflectionCache.size());
    }
  }
   ...
   */
  public ValidationResult validateEntity(
      EntityMetadata entityMetadata, Map<String, Object> entityData) {
    ...
    try {
      ...
      LOGGER.debug(
          "实体数据验证完成，实体类型: {}, 是否有效: {}, 错误数量: {}, 警告数量: {}, 耗时: {}ms",
      ...
      // 记录慢验证
      if (duration > 100) {
        LOGGER.warn("验证性能警告: 实体类型 {} 的验证耗时超过100ms ({})", entityMetadata.getApiName(), duration);
      }
    } catch (Exception e) {
   ...
   */
  private void validateEntityWithDirectApproach(
   ...
   * @return the validation result
   */
  public ValidationResult validateEntityByTypeName(
  ...
  public CompletableFuture<List<ValidationResult>> validateBatchAsync(
      EntityMetadata entityMetadata, List<Map<String, Object>> entityDataList) {
    Assert.notNull(entityMetadata, "实体元数据不能为空");
    ...
    LOGGER.debug("开始批量异步验证，实体类型: {}, 记录数量: {}", entityMetadata.getApiName(), recordCount);

    // 使用策略验证执行器执行批量验证
    if (validationExecutor != null) {
      try {
        return validationExecutor
            .executeBatchStrategiesAsync(entityMetadata, entityDataList, failFast)
            .whenComplete(
                (results, ex) -> {
                  if (ex != null) {
                  ...
                  logPerformanceMetrics(
                      entityMetadata.getApiName(), startTime, recordCount, results);
                });
      } catch (Exception e) {
              ...
              try {
                          ...
                          data -> {
                            try {
                              ...
                              return createErrorValidationResult(
                                  "Record validation exception: " + e.getMessage());
                            }
                          })
                ...
                ValidationResult errorResult =
                    createErrorValidationResult(
                        "Batch asynchronous validation exception: " + e.getMessage());
   ...
   */
  private List<ValidationResult> processLargeBatch(
      EntityMetadata entityMetadata, List<Map<String, Object>> entityDataList) {
    List<ValidationResult> results = new ArrayList<>(entityDataList.size());
    int batchSize =
        Math.min(500, entityDataList.size() / Runtime.getRuntime().availableProcessors());
    ...
    for (int i = 0; i < entityDataList.size(); i += batchSize) {
      ...
      // 对每个子批次使用并行流
      List<ValidationResult> batchResults =
  ...
  private void logPerformanceMetrics(
      String entityType, long startTime, int recordCount, List<ValidationResult> results) {
    long duration = System.currentTimeMillis() - startTime;
    ...
    if (results != null) {
      ...
      LOGGER.info(
          "批量验证完成，实体类型: {}, 总记录数: {}, 成功数: {}, 错误总数: {}, 警告总数: {}, 总耗时: {}ms, 平均每条耗时: {}ms",
          entityType,
          recordCount,
          successCount,
          errorCount,
    ...
    if (recordCount > 0 && duration / recordCount > 50) {
      LOGGER.warn("批量验证性能警告: 实体类型 {} 的平均验证耗时超过50ms ({})", entityType, duration / recordCount);
    }
  }
  ...
  public List<ValidationResult> validateBatch(
    ...
    LOGGER.debug(
        "开始批量验证实体数据，实体类型: {}, 数据量: {}",
        entityMetadata.getApiName(),
        entityDataList != null ? entityDataList.size() : 0);
   ...
   * @return list of validation results
   */
  private List<ValidationResult> performFallbackBatchValidation(
      EntityMetadata entityMetadata, List<Map<String, Object>> entityDataList) {
    // 根据数据量选择合适的验证策略
    if (entityDataList.size() <= batchThreshold) {
      ...
      for (int i = 0; i < entityDataList.size(); i++) {
        try {
          ...
          results.add(
              createErrorValidationResult("Record validation exception: " + e.getMessage()));
        }
      }
      return results;
    } else {
              ...
              if (!entityData.containsKey(fieldName)) {
                ...
                result.addError(
                    ValidationResult.ValidationError.builder()
                        .fieldPath(fieldName)
                        .message(
                            String.format("Field '%s' cannot be empty string", field.getLabel()))
                        .build());
              }
            });
  }
              ...
              if (!isValidType) {
                result.addError(
                    ValidationResult.ValidationError.builder()
                        ...
                        .message(
                            String.format(
                                "Field '%s' requires %s type, actual type: %s",
  ...
  private boolean validateFieldType(
      String fieldType, Object value, String fieldName, ValidationResult result) {
        ...
        if (!SUPPORTED_TYPES.containsKey(fieldType.toLowerCase())) {
          result.addWarning(
              ValidationResult.ValidationWarning.builder()
                  .message(
                      String.format("Field '%s' uses unknown data type: %s", fieldName, fieldType))
                  .build());
        }
      ...
      try {
        ...
        result.addError(
            ValidationResult.ValidationError.builder()
                ...
                .message(
                    String.format(
                        "Field value is not a valid date format, please use %s format",
      ...
      try {
        ...
        result.addError(
            ValidationResult.ValidationError.builder()
                ...
                .message(
                    String.format(
                        "Field value is not a valid date-time format, please use %s format",
  ...
  private String getFieldTypeName(SmartFieldMetadata field) {
    ...
    try {
      ...
      // Ignore exceptions
      LOGGER.debug(
          "Failed to get field type, returning default type: {}.{}",
   ...
   */
  private Object getFieldPropertyWithCache(
      SmartFieldMetadata field, String propertyName, Class<?> propertyType, String cacheKey) {
    try {
      // 尝试从方法缓存获取方法
      String methodName =
  ...
  private void validateStringConstraints(
      SmartFieldMetadata field, String value, ValidationResult result) {
    String fieldName = field.getApiName();
    ...
    if (field.getMaxLength() != null && value.length() > field.getMaxLength()) {
      result.addError(
          ValidationResult.ValidationError.builder()
              ...
              .message(
                  String.format(
                      "Field '%s' length cannot exceed %d characters",
    ...
    if (field.getMinLength() != null && value.length() < field.getMinLength()) {
      result.addError(
          ValidationResult.ValidationError.builder()
              ...
              .message(
                  String.format(
                      "Field '%s' length cannot be less than %d characters",
    ...
    if (field.getRegexPattern() != null) {
      ...
      Pattern pattern =
          regexPatternCache.computeIfAbsent(field.getRegexPattern(), Pattern::compile);
      if (!pattern.matcher(value).matches()) {
        result.addError(
            ValidationResult.ValidationError.builder()
                ...
                .message(
                    String.format(
                        "Field '%s' value does not match required format", field.getLabel()))
                .build());
      }
    }
  }
  ...
  private void validateNumericConstraints(
      SmartFieldMetadata field, double value, ValidationResult result) {
    String fieldName = field.getApiName();
    ...
    if (field.getMaxValue() != null && value > field.getMaxValue()) {
      result.addError(
          ValidationResult.ValidationError.builder()
              ...
              .message(
                  String.format(
                      "Field '%s' value cannot be greater than %s",
    ...
    if (field.getMinValue() != null && value < field.getMinValue()) {
      result.addError(
          ValidationResult.ValidationError.builder()
              ...
              .message(
                  String.format(
                      "Field '%s' value cannot be less than %s",
   ...
   * @param result the validation result to accumulate errors
   */
  private void validatePicklistValue(
    ...
    if (!picklistValues.contains(value)) {
      result.addError(
          ValidationResult.ValidationError.builder()
              ...
              .message(
                  String.format(
                      "Field '%s' value must be one of: %s",
  ...
  private void validateArrayConstraints(
      SmartFieldMetadata field, List<?> value, ValidationResult result) {
    String fieldName = field.getApiName();
      ...
      if (maxItems != null && value.size() > maxItems) {
        result.addError(
            ValidationResult.ValidationError.builder()
                ...
                .message(
                    String.format(
                        "Field '%s' array cannot have more than %d elements",
      ...
      if (minItems != null && value.size() < minItems) {
        result.addError(
            ValidationResult.ValidationError.builder()
                ...
                .message(
                    String.format(
                        "Field '%s' array must have at least %d elements",
   ...
   */
  @SuppressWarnings("unchecked")
  private <T> T getFieldProperty(
      SmartFieldMetadata field, String propertyName, Class<T> propertyType) {
    try {
      // 尝试从缓存获取方法
      ...
      // Ignore exceptions
      LOGGER.debug(
          "Failed to get field property: {}.{}", field.getClass().getSimpleName(), propertyName);
    }
    return null;
  }
                ...
                if (relationship != null
                  ...
                  if (targetEntity != null && !targetEntity.isEmpty()) {
                    ...
                    if (getEntityMetadata(targetEntity) == null) {
                      LOGGER.warn(
                          "Entity type '{}' referenced by field '{}' is not defined",
                          targetEntity,
                          field.getLabel());
                    }
                  }
      ...
      if (metadataRepository != null) {
        try {
          ...
          Method getMetadataMethod =
              findMethod(metadataRepository.getClass(), "getEntityMetadata", String.class);
