...
/** 基于Spring表达式语言的业务规则引擎实现 支持复杂条件表达式、计算字段和业务规则验证 */
@Service
public class SpelBusinessRuleEngine implements BusinessRuleEngine {
  ...
  @Override
  public RuleResult executeRules(
  ...
  @Override
  public Object calculateField(
      String entityName, Map<String, Object> entityData, String fieldName) {
    try {
      // 获取实体元数据
      EntityMetadata metadata = (EntityMetadata) metadataEngine.getEntityMetadata(entityName);
      if (metadata == null) {
  ...
  private void executeFieldRules(
    ...
    if (fieldMetadata.getValidationExpression() != null) {
      try {
        boolean isValid =
            evaluateExpression(fieldMetadata.getValidationExpression(), context, Boolean.class);
        if (!isValid) {
          String errorMessage =
              fieldMetadata.getErrorMessage() != null
                  ? fieldMetadata.getErrorMessage()
                  : "Field validation failed";
          result.addError("FieldValidation", errorMessage);
        }
      } catch (Exception e) {
        logger.warn(
            "Failed to evaluate validation expression for field: {}", fieldMetadata.getName(), e);
        result.addError(
            "ValidationError",
            "Invalid validation expression for field: " + fieldMetadata.getName());
      }
    }
  }

  private void executeEntityRules(
  ...
  private void executeNamedRules(
      EntityMetadata metadata,
      Map<String, Object> entityData,
  ...
  private EvaluationContext buildEvaluationContext(Map<String, Object> data) {
    ...
    try {
      ...
      context.registerFunction(
          "isNotEmpty", getClass().getDeclaredMethod("isNotEmpty", Object.class));
      context.registerFunction(
          "contains", getClass().getDeclaredMethod("contains", Collection.class, Object.class));
  ...
  private <T> T evaluateExpression(
      String expressionString, EvaluationContext context, Class<T> expectedType) {
  ...
  private Expression getOrParseExpression(String expressionString) {
    ...
    Map<String, Expression> expressions =
        expressionCache.computeIfAbsent("global", k -> new ConcurrentHashMap<>());
