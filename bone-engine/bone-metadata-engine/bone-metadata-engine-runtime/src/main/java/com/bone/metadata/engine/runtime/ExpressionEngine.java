...
/** 表达式引擎，用于计算虚拟字段和公式字段 支持SpEL风格的表达式计算 */
@Component
public class ExpressionEngine implements ExpressionEvaluator {
  ...
  /** 评估模板表达式 */
  private String evaluateTemplateExpression(
      String templateExpression, Map<String, Object> context) {
    try {
      // 预编译或从缓存获取表达式
      ...
      // 当SpEL解析失败时，回退到原始实现
      LOGGER.warn(
          "SpEL template parsing failed, falling back to original implementation: {}",
          e.getMessage());
  ...
  private Expression getOrCompileExpression(String expression) {
    return compiledExpressionCache.computeIfAbsent(
        expression,
        expr -> {
          try {
    ...
    public String formatDate(Date date, String pattern) {
      ...
      try {
        ...
        SimpleDateFormat sdf =
            DATE_FORMAT_CACHE.get().computeIfAbsent(pattern, SimpleDateFormat::new);
   ...
   * @return 评估结果映射
   */
  public Map<String, String> evaluateExpressions(
      Map<String, String> expressions, Map<String, Object> context) {
  ...
  public boolean evaluateBooleanExpression(String expression, Map<String, Object> context) {
    ...
    try {
      ...
      String booleanExpression =
          expression.contains("$") ? evaluateExpression(expression, context) : expression;
      ...
      try {
        ...
        // SpEL评估失败时，回退到原始实现
        LOGGER.warn(
            "SpEL boolean expression evaluation failed, falling back to original implementation: {}",
            e.getMessage());
  ...
  /** 原始布尔表达式评估实现（作为回退方案） */
  private boolean evaluateBooleanExpressionFallback(
    ...
    try {
      ...
      if (getterMethod == null) {
        ...
        if (cacheEnabled && getterMethod != null) {
          propertyAccessorCache.put(
              cacheKey, new MethodCacheEntry(getterMethod, cacheExpirationTime));
        }
      }
      ...
      LOGGER.debug(
          "Failed to get property value: {}.{}", obj.getClass().getName(), propertyName, e);
      return null;
    }
  }
  ...
  /** 计算虚拟字段 */
  private <T extends SmartBaseEntity> void calculateVirtualFields(
      T entity, EntityMetadata entityMetadata, Map<String, Object> context) {
  ...
  /** 计算字段并进行错误处理 */
  private <T extends SmartBaseEntity> void calculateFieldWithErrorHandling(
      T entity, SmartFieldMetadata field, Map<String, Object> context) {
    try {
      LOGGER.debug(
          "Calculating virtual field: {}, expression: {}",
          field.getApiName(),
          field.getCalculationExpression());
      ...
      fieldCalculationFailures.incrementAndGet();
      LOGGER.error(
          "Failed to calculate virtual field {}: {}", field.getApiName(), e.getMessage(), e);
    }
  }

  /** 计算公式字段 */
  private <T extends SmartBaseEntity> void calculateFormulaFields(
      T entity, EntityMetadata entityMetadata, Map<String, Object> context) {
    for (SmartFieldMetadata field : entityMetadata.getFields().values()) {
      // 暂时注释掉isFormulaField()和getFormula()调用，因为SmartFieldMetadata类中似乎没有这些方法
      // if (field.isFormulaField() && field.getFormula() != null) {
      //     try {
      //         LOGGER.debug("Calculating formula field: {}, formula: {}", field.getApiName(),
      // field.getFormula());
      //
      //         // 处理简单的数学公式
      ...
      //     } catch (Exception e) {
      //         fieldCalculationFailures.incrementAndGet();
      //         LOGGER.error("Error calculating formula field '{}': {}", field.getApiName(),
      // e.getMessage());
      //     }
      // }
    }
  ...
  private Object evaluateMathExpression(String formula, Map<String, Object> context) {
    ...
    try {
      ...
      // SpEL计算失败时，回退到原始实现
      LOGGER.warn(
          "SpEL mathematical expression calculation failed, falling back to original implementation: {}",
          e.getMessage());
      try {
        return evaluateSimpleMath(processedFormula);
      } catch (Exception ex) {
        LOGGER.warn(
            "Simple mathematical expression calculation failed, returning original value: {}",
            ex.getMessage());
  ...
  private double evaluateExpressionRecursive(String expression) {
    ...
    while (openParen != -1) {
      ...
      expression =
          expression.substring(0, openParen) + result + expression.substring(closeParen + 1);
  ...
  /** 使用反射设置字段值 */
  private <T extends SmartBaseEntity> void setFieldValueUsingReflection(
      T entity, String fieldName, Object value) {
  ...
  private Object convertValueToType(Object value, Class<?> targetType) {
    ...
    if (objectMapper != null) {
      try {
        ...
        LOGGER.debug(
            "Type conversion failed: {} -> {}", value.getClass().getName(), targetType.getName());
      }
    }
  ...
  public Object eval(String expression, Map<String, Object> context) {
    ...
    try {
      ...
      if (context != null
          && expression.contains("/")
          && expression.trim().split("\\s*/\\s*").length == 2) {
      ...
      StandardEvaluationContext evaluationContext =
          (StandardEvaluationContext) createEvaluationContext(context);
