...
public class RuleEngine {
   ...
   * @return 计算后的实体数据
   */
  public Map<String, Object> calculateFields(
      String entityName, Map<String, Object> entityData, boolean incremental) {
    // 获取实体元数据
    Object metadataObj = metadataEngine.getEntityMetadata(entityName);
    EntityMetadata metadata =
        (metadataObj instanceof EntityMetadata) ? (EntityMetadata) metadataObj : null;
  ...
  private void handleFieldCalculationError(
      SmartFieldMetadata field, String entityName, Exception e, Map<String, Object> result) {
  ...
  private List<SmartFieldMetadata> sortCalculatedFieldsByDependency(
      List<SmartFieldMetadata> fields, EntityMetadata metadata) {
    if (fields.isEmpty()) {
      return fields;
    }
  ...
  private List<SmartFieldMetadata> optimizeCalculatedFields(
      List<SmartFieldMetadata> fields, Map<String, Object> entityData, EntityMetadata metadata) {
    // 检查是否有字段值发生变化
  ...
  public Map<String, Object> validateRules(
      String entityName, Map<String, Object> entityData, List<String> triggerEvents) {
    ValidationResult validationResult =
        validateRulesWithResult(entityName, entityData, triggerEvents);
   ...
   * @return 验证结果
   */
  public ValidationResult validateRulesWithResult(
      String entityName, Map<String, Object> entityData, List<String> triggerEvents) {
   ...
   * @param result 验证结果对象
   */
  private void validateEntityRules(
      EntityMetadata metadata,
      Map<String, Object> entityData,
  ...
  private ValidationResult.ValidationError createGeneralError(String message) {
    return ValidationResult.ValidationError.builder().fieldPath("general").message(message).build();
  }
  ...
  private void validateRuleWithReflection(
    ...
    try {
      ...
      result.addError(
          createGeneralError("Reflection-based rule validation exception: " + e.getMessage()));
    }
  }
