...
/** ExpressionEvaluator测试类 提供全面的测试覆盖，确保表达式引擎功能正确 */
class ExpressionEvaluatorTest {
  ...
  @ParameterizedTest
  @ValueSource(strings = {"name", "age", "salary", "isActive", "department"})
  ...
  void testEvaluateComplexExpression() {
    ...
    boolean complexResult =
        (Boolean) evaluator.eval("age > 18 && isActive && department == '技术部'", context);
  ...
  void testEvaluateMethodInvocation() {
    ...
    assertTrue(
        (Boolean) evaluator.eval("department.startsWith('技术')", context), "startsWith方法调用失败");
  }
}
