...
public class FieldTypeValidationStrategy implements ValidationStrategy {
          ...
          if (!SUPPORTED_TYPES.contains(fieldType.toLowerCase())) {
            ValidationResult.ValidationWarning validationWarning =
                ValidationResult.ValidationWarning.builder()
                    .fieldPath(fieldName)
                    .message(String.format("字段 '%s' 使用了未知的数据类型: %s", fieldLabel, fieldType))
                    .build();
      ...
      if (!isValidType) {
        ValidationResult.ValidationError validationError =
            ValidationResult.ValidationError.builder()
                .fieldPath(fieldName)
                .message(String.format("字段 '%s' 需要 %s 类型的值", fieldLabel, fieldType))
                .build();
  ...
  /** 验证日期类型 */
  private boolean validateDateType(
      Object value, String fieldName, ValidationResult result, String fieldLabel) {
      ...
      try {
        ...
        ValidationResult.ValidationError validationError =
            ValidationResult.ValidationError.builder()
                ...
                .message(
                    String.format(
                        "字段 '%s' 不是有效的日期格式，请使用 %s 格式", fieldLabel, DATE_FORMATTER.toString()))
                .build();
        result.addError(validationError);
        return false;
      }
  ...
  /** 验证日期时间类型 */
  private boolean validateDateTimeType(
      Object value, String fieldName, ValidationResult result, String fieldLabel) {
    if (value instanceof java.util.Date) {
      return true;
    } else if (value instanceof String) {
