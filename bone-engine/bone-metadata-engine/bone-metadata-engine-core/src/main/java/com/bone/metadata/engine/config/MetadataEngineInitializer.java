...
/** 元数据引擎初始化器 负责初始化元数据引擎，加载默认元数据和配置 */
@Component
public class MetadataEngineInitializer implements ApplicationRunner {
  ...
  private final MetadataEngineProperties properties;
  private final ResourceLoader resourceLoader;

  public MetadataEngineInitializer(
      MetadataEngine metadataEngine,
      MetadataEngineProperties properties,
  ...
  public void run(ApplicationArguments args) throws Exception {
    log.info("开始初始化Metadata Engine引擎...");
    ...
    log.info("Metadata Engine引擎初始化完成！");
  }
  ...
  private void initEngine() {
    try {
      ...
      log.error("引擎初始化失败", e);
      throw new RuntimeException("Metadata Engine引擎初始化失败", e);
    }
  }
  ...
  private EntityMetadata createUserEntity() {
    ...
    // 创建时间字段
    fields.add(
        createField("createdAt", "创建时间", "DATETIME", true, null, null, "创建时间", "#date()", null));

    // 虚拟字段：全名
    SmartFieldMetadata fullNameField =
        createField("fullName", "全名", "STRING", false, null, null, "用户全名", null, null);
    ...
    rules.add(
        createValidationRule(
            "uniqueUsername", "用户名必须唯一", "VALIDATION", "#root.username != null", null));
    rules.add(
        createValidationRule(
            "validEmail",
            "邮箱格式必须正确",
            "VALIDATION",
            "#root.email matches '^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$'",
            "email"));
  ...
  private EntityMetadata createProductEntity() {
    ...
    // 虚拟字段：折扣价格
    SmartFieldMetadata discountPriceField =
        createField("discountPrice", "折扣价格", "DOUBLE", false, null, null, "产品折扣价格", null, null);
  ...
  private EntityMetadata createOrderEntity() {
    ...
    // 虚拟字段：订单描述
    SmartFieldMetadata descriptionField =
        createField("description", "订单描述", "STRING", false, null, null, "订单描述", null, null);
  ...
  /** 创建字段元数据 */
  private SmartFieldMetadata createField(
      String apiName,
      String label,
      String type,
      boolean required,
  ...
  /** 创建验证规则 */
  private ValidationRuleMetadata createValidationRule(
      String name, String message, String type, String expression, String fieldName) {
