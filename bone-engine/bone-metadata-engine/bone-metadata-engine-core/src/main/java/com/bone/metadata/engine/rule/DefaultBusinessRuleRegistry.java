...
public class DefaultBusinessRuleRegistry implements BusinessRuleRegistry {
  ...
  public void registerRules(List<BusinessRuleMetadata> rules) {
    ...
    log.info(
        "Registered {} rules ({} skipped)", validRules.size(), rules.size() - validRules.size());
  ...
  public boolean unregisterRule(String ruleId) {
    ...
    log.info(
        "Unregistered rule: {}, Entity: {}",
        ruleId,
        entityApiName != null ? entityApiName : "unknown");
  ...
  public List<BusinessRuleMetadata> searchRules(Predicate<BusinessRuleMetadata> predicate) {
    ...
    return rulesById.values().stream().filter(predicate).collect(Collectors.toList());
  }
  ...
  private void updateEntityIndex(BusinessRuleMetadata rule) {
    ...
    if (entityApiName != null) {
      ...
      rulesByEntity
          .computeIfAbsent(entityApiName, k -> new CopyOnWriteArrayList<>())
          .removeIf(
              r -> {
  ...
  private void updateTypeIndexOnRemove(BusinessRuleMetadata rule) {
    try {
      ...
      if (log != null) {
        log.error(
            "Error updating type index on remove for rule {}: {}", rule.getId(), e.getMessage(), e);
      }
    }
  }
