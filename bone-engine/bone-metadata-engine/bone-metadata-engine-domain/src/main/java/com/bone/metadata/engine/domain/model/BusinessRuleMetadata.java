...
public class BusinessRuleMetadata {
  ...
  public boolean isValidationRule() {
    return hasTriggerEvent(BEFORE_VALIDATE)
        || hasTriggerEvent(AFTER_VALIDATE)
        || executionPhase == null
        || executionPhase.equals(VALIDATION);
  }

  /** 检查规则是否是前置规则 */
  public boolean isPreRule() {
    return hasTriggerEvent(BEFORE_CREATE)
        || hasTriggerEvent(BEFORE_UPDATE)
        || hasTriggerEvent(BEFORE_DELETE)
        || hasTriggerEvent(BEFORE_VALIDATE)
        || PRE_VALIDATION.equals(executionPhase)
        || PRE_PERSIST.equals(executionPhase);
  }

  /** 检查规则是否是后置规则 */
  public boolean isPostRule() {
    return hasTriggerEvent(AFTER_CREATE)
        || hasTriggerEvent(AFTER_UPDATE)
        || hasTriggerEvent(AFTER_DELETE)
        || hasTriggerEvent(AFTER_VALIDATE)
        || POST_VALIDATION.equals(executionPhase)
        || POST_PERSIST.equals(executionPhase);
  }
  ...
  public BusinessRuleMetadata cloneForVersion() {
    BusinessRuleMetadata clone =
        BusinessRuleMetadata.builder()
            .name(this.name)
