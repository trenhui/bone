...
public class DefaultMetadataRegistry implements MetadataRegistry {
        ...
        .filter(
            entity ->
                (entity.getName() != null && entity.getName().toLowerCase().contains(lowerQuery))
                    || (entity.getApiName() != null
                        && entity.getApiName().toLowerCase().contains(lowerQuery))
                    || (entity.getDescription() != null
                        && entity.getDescription().toLowerCase().contains(lowerQuery))
                    || (entity.getDomain() != null
                        && entity.getDomain().toLowerCase().contains(lowerQuery)))
  ...
  public RelationshipMetadata getRelationshipMetadata(
      String entityApiName, String relationshipName) {
    EntityMetadata entityMetadata = getEntityMetadata(entityApiName);
    ...
    return entityMetadata.getRelationships().stream()
        .filter(
            rel ->
                relationshipName.equals(rel.getName()) || relationshipName.equals(rel.getApiName()))
        .findFirst()
        .orElse(null);
  }
  ...
  private void buildIndexes(EntityMetadata entityMetadata) {
    ...
    if (entityMetadata.getEntityType() != null) {
      entityTypeIndex
          .computeIfAbsent(entityMetadata.getEntityType(), k -> new CopyOnWriteArrayList<>())
          .add(entityMetadata);
    }
    ...
    if (entityMetadata.getDomain() != null) {
      domainIndex
          .computeIfAbsent(entityMetadata.getDomain(), k -> new CopyOnWriteArrayList<>())
          .add(entityMetadata);
    }
  ...
  // 通知关系更新事件
  private void notifyRelationshipUpdated(
      String entityApiName, RelationshipMetadata relationshipMetadata) {
    listeners.forEach(
        listener -> {
          try {
