...
class ResourceManager {
   ...
   * 构造函数
   * @param {Object} options 配置选项
   * @param {boolean} [options.loggerEnabled] 是否启用日志记录，默认在开发环境启用
   * @param {number} [options.leakDetectionInterval] 资源泄漏检测间隔（毫秒），默认60000毫秒
   * @param {boolean} [options.enableLeakDetection] 是否启用资源泄漏检测，默认在开发环境启用
   */
  constructor(options = {}) {
    /**
     * 存储自定义资源的映射表
     * @type {Map<string, { cleanupFn: Function, category: string, priority: number, createdTime: number, description: string }>}
     */
     ...
     * @type {number}
     */
    this.leakDetectionInterval = options.leakDetectionInterval ?? 60000; // 默认1分钟检查一次
    
    // 启动资源泄漏检测（开发环境）
    if (ENVIRONMENT.IS_DEVELOPMENT && options.enableLeakDetection !== false) {
  ...
  static PRIORITY_LEVELS = {
    CRITICAL: 10,   // 最高优先级，最先清理
    HIGH: 20,
    MEDIUM: 30,    // 默认优先级
    LOW: 40,
    LOWEST: 50      // 最低优先级，最后清理
  };

  /**
   * 添加自定义资源
   * @param {Object|Function} resource 资源对象或清理函数
   * @param {Object} options 资源选项
   * @param {string} [options.category=DEFAULT] 资源类别
   * @param {number} [options.priority=MEDIUM] 资源优先级
   * @param {string} [options.description=''] 资源描述
   * @returns {string|null} 资源ID，可用于后续移除，无效参数返回null
   */
  addResource(resource, options = {}) {
    // 验证参数
    if (!resource && typeof resource !== 'function') {
      console.warn(`[${APP_CONFIG.ID}] 无效的资源类型添加到资源管理器`);
      return null;
    }
    
    // 检查管理器是否已销毁
    if (this.isDestroyed) {
      console.warn(`[${APP_CONFIG.ID}] 资源管理器已销毁，无法添加新资源`);
      return null;
    }
    
    // 生成资源ID
    const resourceId = `${APP_CONFIG.ID}-resource-${Date.now()}-${this.resourceCounter++}`;
    const category = options.category || ResourceManager.RESOURCE_CATEGORIES.DEFAULT;
    const priority = options.priority ?? ResourceManager.PRIORITY_LEVELS.MEDIUM;
    
    // 标准化清理函数
    const cleanupFn = this._normalizeCleanupFunction(resource);
    ...
    this.customResources.set(resourceId, resourceObj);
    this.categories.add(category);
    
    if (this.loggerEnabled && ENVIRONMENT.LOG_LEVEL === 'verbose') {
      console.log(`[${APP_CONFIG.ID}] 添加资源: ${resourceId} (${category})`);
    }
  ...
  removeResource(resourceId) {
    ...
    if (this.customResources.has(resourceId)) {
      ...
      try {
        resource.cleanupFn();
        if (this.loggerEnabled && ENVIRONMENT.LOG_LEVEL === 'verbose') {
          console.log(`[${APP_CONFIG.ID}] 资源已清理: ${resourceId} (${resource.category})`);
        }
      } catch (error) {
        console.error(`[${APP_CONFIG.ID}] 清理资源 ${resourceId} 失败:`, error);
        // 即使发生错误，也要删除资源引用，避免重复清理
      }
      
      this.customResources.delete(resourceId);
      
      // 清理空类别
      this._cleanupEmptyCategories();
   ...
   */
  clearResourcesByCategory(category) {
    // 参数验证
    if (!category || typeof category !== 'string' || !this.categories.has(category)) {
      if (this.loggerEnabled && ENVIRONMENT.LOG_LEVEL === 'verbose') {
        console.warn(`[${APP_CONFIG.ID}] 无效的资源类别: ${String(category)}`);
      }
      return 0;
    }
    ...
    // 收集需要移除的资源ID
    this.customResources.forEach((resource, resourceId) => {
    ...
    if (this.loggerEnabled && clearedCount > 0) {
      console.log(`[${APP_CONFIG.ID}] 已清理 ${clearedCount} 个 ${category} 类别的资源`);
    }
  ...
  clearResources(options = {}) {
    if (this.loggerEnabled) {
      const categoryText = options.category ? ` 类别: ${options.category}` : '';
      console.log(`[${APP_CONFIG.ID}] 开始资源清理${categoryText}`);
    }
    
    // 准备资源列表
    const resources = Array.from(this.customResources.entries()).map(([id, resource]) => ({
      id,
      ...resource
    }));
    ...
    if (options.category) {
      filteredResources = resources.filter(r => r.category === options.category);
    }
    
    // 按优先级排序，优先级高的先清理
    filteredResources.sort((a, b) => a.priority - b.priority);
    
    let clearedCount = 0;
    let errorCount = 0;
    
    // 清理资源
    filteredResources.forEach(({ id, category }) => {
      try {
        if (this.removeResource(id)) {
          clearedCount++;
        }
      } catch (error) {
        console.error(`[${APP_CONFIG.ID}] 清理资源 ${id} (${category}) 失败:`, error);
        errorCount++;
      }
    });
    
    // 更新类别集合
    if (options.clearCategories !== false) {
      this._cleanupEmptyCategories();
    }
    
    // 记录日志
    if (this.loggerEnabled) {
      let logMessage = `[${APP_CONFIG.ID}] 资源清理完成: 共清理 ${clearedCount} 个资源`;
      if (errorCount > 0) {
        logMessage += `, 发生 ${errorCount} 个错误`;
      }
      console.log(logMessage);
    }
    
    return { clearedCount, errorCount };
  }

  /**
   * 获取资源统计信息
   * @returns {Object} 资源统计数据
   */
  getResourceStatistics() {
    const stats = {
      total: this.customResources.size,
      categories: {},
      oldestResource: null,
      newestResource: null,
      createdAt: this.constructor.name === 'ResourceManager' ? 'Instance created' : 'Unknown'
    };
    ...
    this.customResources.forEach((resource, resourceId) => {
      stats.categories[resource.category]++;
      
      // 跟踪最老和最新的资源
      if (resource.createdTime < oldestTime) {
        oldestTime = resource.createdTime;
        oldestId = resourceId;
      }
      
      if (resource.createdTime > newestTime) {
        newestTime = resource.createdTime;
        newestId = resourceId;
      }
    });
    ...
    // 添加最老和最新资源信息
    if (oldestId) {
      const oldestResource = this.customResources.get(oldestId);
      stats.oldestResource = {
        id: oldestId,
        category: oldestResource.category,
        age: Date.now() - oldestTime,
        createdTime: oldestTime,
        createdTimeFormatted: new Date(oldestTime).toLocaleString()
      };
    }
    
    if (newestId) {
      const newestResource = this.customResources.get(newestId);
      stats.newestResource = {
        id: newestId,
        category: newestResource.category,
        age: Date.now() - newestTime,
        createdTime: newestTime,
        createdTimeFormatted: new Date(newestTime).toLocaleString()
      };
    }
   ...
   */
  startLeakDetection(interval = this.leakDetectionInterval) {
    // 先停止之前的检测
    this.stopLeakDetection();
    
    this.leakDetectionTimer = setInterval(() => {
      this._reportPotentialLeaks();
    }, interval);
    
    if (this.loggerEnabled) {
      console.log(`[${APP_CONFIG.ID}] 资源泄漏检测已启动，间隔: ${interval / 1000}秒`);
    }
  }
  ...
  stopLeakDetection() {
    if (this.leakDetectionTimer) {
      ...
      if (this.loggerEnabled) {
        console.log(`[${APP_CONFIG.ID}] 资源泄漏检测已停止`);
      }
    }
  }
  ...
  destroy() {
    ...
    // 清理所有资源
    const result = this.clearResources();
    
    // 标记为已销毁
    this.isDestroyed = true;
    
    if (this.loggerEnabled) {
      console.log(`[${APP_CONFIG.ID}] 资源管理器已销毁，共清理 ${result.clearedCount} 个资源`);
    }
  }
   ...
   * 检查资源泄漏
   * @private
   * @returns {Array} 潜在泄漏的资源列表
   */
  _reportPotentialLeaks() {
    const now = Date.now();
    // 检查创建超过1小时的资源
    ...
    this.customResources.forEach((resource, resourceId) => {
      if (resource.createdTime < oneHourAgo) {
        leakedResources.push({
          id: resourceId,
          category: resource.category,
          age: now - resource.createdTime,
    ...
    if (leakedResources.length > 0) {
      // 报告潜在的资源泄漏
      console.warn(`[${APP_CONFIG.ID}] 检测到 ${leakedResources.length} 个可能泄漏的资源:`);
      leakedResources.forEach(res => {
        const ageInMinutes = Math.floor(res.age / 60000);
        console.warn(`  - ${res.id} (${res.category}): ${ageInMinutes} 分钟` + 
                     (res.description ? `, 描述: ${res.description}` : ''));
      });
      
      // 通过通信管理器通知主应用
      if (window.__bone_communicator__) {
        try {
          window.__bone_communicator__.sendMessage('main', 'warning:resource_leak', {
            appId: APP_CONFIG.ID,
            count: leakedResources.length,
            timestamp: Date.now(),
            details: leakedResources.map(r => ({ id: r.id, category: r.category, age: r.age }))
          });
        } catch (error) {
          console.error(`[${APP_CONFIG.ID}] 发送资源泄漏警告失败:`, error);
        }
      }
    }
   ...
   * @private
   * @param {Object|Function} resource 资源对象或清理函数
   * @returns {Function} 标准化后的清理函数
   */
  _normalizeCleanupFunction(resource) {
    if (typeof resource === 'function') {
      ...
      console.warn(`[${APP_CONFIG.ID}] 添加到资源管理器的资源没有标准清理方法`);
      return () => {};
    }
  }
   ...
   * @private
   */
  _cleanupEmptyCategories() {
    const activeCategories = new Set();
    
    // 收集当前仍有资源的类别
    this.customResources.forEach(resource => {
      activeCategories.add(resource.category);
    });
