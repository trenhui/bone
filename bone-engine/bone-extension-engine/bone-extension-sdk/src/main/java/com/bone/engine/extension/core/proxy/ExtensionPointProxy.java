 ...
 * 扩展点动态代理（终极优化版）
 * <p>
 * 核心组件：负责拦截扩展点接口方法的调用，识别业务上下文，
 * 并通过 Router 路由到具体的扩展实现进行调用转发。
 * * 核心优化：
 * 1. 使用 ObjectProvider 替代 volatile/synchronized，保证线程安全和延迟加载。
 * 2. 使用 cachedRouter 字段，在第一次获取后缓存结果，后续调用性能极致。
 */
@Slf4j
public class ExtensionPointProxy<T> implements InvocationHandler, Serializable {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        if (bizContext == null) {
            throw new IllegalStateException(
                    String.format("BizContext 未找到！请通过 ExtensionContextManager.setCurrentContext() 或方法参数传入 | 接口: %s | 方法: %s",
        ...
        if (extension == null) {
            throw new IllegalStateException(
                    String.format("路由失败：未找到匹配的扩展实现 | 扩展点: %s | 上下文: %s",
                            extensionPoint.getName(), bizContext.toString()) // 使用 toString 简化，或调用 bizContext.buildSummary()
            );
        }
    ...
    /**
     * 获取 Router 实例（无锁缓存逻辑）
     * 仅在第一次调用时通过 ObjectProvider 触发 Spring 容器查找。
     */
    ...
    /**
     * 序列化安全处理：防止在分布式场景下，代理对象被序列化传输后
     * 由于 transient 字段丢失，导致无法获取 Spring Bean。
     */
    ...
    private static class DeserializedProxyError<T> implements InvocationHandler, Serializable {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) {
            throw new IllegalStateException(String.format(
                    "扩展点代理（%s）不支持序列化后使用。请在 Spring 容器内通过 Factory 重新获取代理。",
