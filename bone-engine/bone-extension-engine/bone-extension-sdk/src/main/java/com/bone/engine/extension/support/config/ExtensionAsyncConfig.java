 ...
 * 扩展点异步执行配置
 *
 * <p>提供可配置的线程池，用于扩展点的异步执行和事件处理， 支持通过配置文件调整线程池参数
 */
@Configuration
public class ExtensionAsyncConfig {
  ...
  /**
   * 创建扩展点事件专用的异步任务执行器 配置合理的线程池参数，适合事件处理的特性
   *
   * @return 配置好的异步任务执行器
   */
   ...
   * 扩展点异步执行线程池
   *
   * <p>核心配置： 1. 核心线程数：默认10，可通过extension.async.core-pool-size配置 2.
   * 最大线程数：默认50，可通过extension.async.max-pool-size配置 3.
   * 队列容量：默认1000，可通过extension.async.queue-capacity配置 4.
   * 线程存活时间：默认60秒，可通过extension.async.keep-alive-seconds配置 5. 线程名称前缀：extension-async- 6.
   * 拒绝策略：CallerRunsPolicy，当队列满时由调用者线程执行
   */
  @Bean(name = EXTENSION_ASYNC_EXECUTOR_BEAN_NAME)
  @ConditionalOnProperty(
      prefix = "extension.async",
      name = "enabled",
      havingValue = "true",
      matchIfMissing = true)
