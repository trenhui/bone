...
public class RedisExtensionMetadataStore implements ExtensionMetadataStore {
    ...
    public RedisExtensionMetadataStore(
            RedisTemplate<String, ExtensionRoutingMetadata> valueTemplate, StringRedisTemplate indexTemplate) {
    ...
    public RedisExtensionMetadataStore(
        ...
        this.refreshChannel =
                StringUtils.hasText(refreshChannel) ? refreshChannel : ExtensionMetadataKeys.REFRESH_CHANNEL;
    }

    @Override
    public void save(@NonNull ExtensionRoutingMetadata metadata) {
        String key = ExtensionMetadataKeys.metadataKey(metadata.getExtensionPoint(), metadata.getCode());
        valueTemplate.opsForValue().set(key, metadata);
        indexTemplate.opsForSet().add(ExtensionMetadataKeys.indexKey(metadata.getExtensionPoint()), metadata.getCode());
    ...
    public Map<String, ExtensionRoutingMetadata> getByExtensionPoint(@NonNull String extensionPoint) {
        Set<String> codes = indexTemplate.opsForSet().members(ExtensionMetadataKeys.indexKey(extensionPoint));
    ...
    public void publishRefresh(@NonNull String extensionPoint) {
        ...
        log.info("Published extension metadata refresh on channel {} for {}", refreshChannel, extensionPoint);
    }
