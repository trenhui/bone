...
public class ExtensionPointDefinition implements Serializable {
  ...
  /** 所有已注册的扩展实现（线程安全） 使用 volatile + 双重检查锁定确保线程安全 */
  ...
  public Collection<ExtensionDefinition> getEnabledExtensions() {
    return getExtensions().values().stream().filter(ExtensionDefinition::isEnabled).toList();
  }
  ...
  public String toString() {
    return String.format(
        "ExtensionPointDefinition[code=%s, extensions=%d, singleton=%b]",
