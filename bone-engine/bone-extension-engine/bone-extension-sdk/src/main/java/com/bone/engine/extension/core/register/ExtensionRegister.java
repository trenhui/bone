 ...
 * 扩展点注册中心 - 修复版
 *
 * <p>修复问题： 1. ✅ 匹配ExtensionRepository接口方法名 2. ✅ 修复方法调用错误 3. ✅ 统一注册逻辑 4. ✅ 保持统计功能
 */
@Component
@Slf4j
public class ExtensionRegister implements ApplicationContextAware, SmartInitializingSingleton {
  ...
  public ExtensionRegister(
      ExtensionRepository extensionRepository, ExtensionProperties extensionProperties) {
    this.extensionRepository =
        Objects.requireNonNull(extensionRepository, "ExtensionRepository cannot be null");
    this.extensionProperties =
        Objects.requireNonNull(extensionProperties, "ExtensionProperties cannot be null");
  }
  ...
  public void initialize() {
    log.info(
        "Extension Register initialized with repository: {}",
  ...
  public void registerExtensionBean(@NonNull Object extensionBean) {
    ...
    try {
      ...
      throw new ExtensionRegistrationException(
          "Extension registration failed for: " + extensionBean.getClass().getName(), e);
    }
  }
  ...
  public synchronized boolean unregisterExtension(
    ...
    // 从仓库中注销 - 使用新接口方法
    ExtensionDefinition removed =
        extensionRepository.unregisterExtension(extensionPoint, extensionCode);
    if (removed != null) {
  ...
  public Optional<ExtensionDefinition> findExtension(
      @NonNull String extensionPoint, @NonNull String extensionCode) {
    // 使用新接口方法 - 返回Optional
  ...
  public Collection<ExtensionDefinition> findEnabledExtensionsByPoint(
      @NonNull String extensionPoint) {
    // 使用新接口方法 - 直接获取启用扩展
  ...
  private Class<?> findExtensionPointInterface(Class<?> implementationClass) {
    ...
    List<Class<?>> extensionPointInterfaces =
        candidateInterfaces.stream()
            .filter(
                interfaceClass ->
                    AnnotatedElementUtils.hasAnnotation(interfaceClass, ExtensionPoint.class))
            .toList();

    if (extensionPointInterfaces.isEmpty()) {
      throw new ExtensionRegistrationException(
          "No @ExtensionPoint interface found for implementation: "
              + implementationClass.getName());
    }
    if (extensionPointInterfaces.size() > 1) {
      log.warn(
          "Multiple @ExtensionPoint interfaces found for {}, using first one: {}",
          implementationClass.getName(),
          extensionPointInterfaces.get(0).getName());
    }
  ...
  private ExtensionDefinition buildExtensionDefinition(
      Object instance, Class<?> implClass, Extension annotation, Class<?> extensionPointInterface) {
    ...
    // 加一行调试日志，永不踩坑！
    log.debug(
        "扩展注册成功: {} | 维度: tenant={}, bizCode={}, useCase={}, scenario={}, userGroup={}",
        def.getCode(),
        def.getTenant(),
        def.getBizCode(),
        def.getUseCase(),
  ...
  private void preprocessExtensionDefinition(ExtensionDefinition definition) {
    ...
    if (StringUtils.hasText(definition.getCondition())) {
      try {
        ...
        log.warn(
            "Failed to compile condition expression for extension {}: {}",
            definition.getCode(),
  ...
  /** 调整扩展的启用状态，基于当前时间和扩展的时间范围 使用公共的isExtensionEffective方法避免重复代码 */
  private void adjustExtensionStatus(ExtensionDefinition definition) {
    ...
    if (!isEffective) {
      ...
      if (StringUtils.hasText(definition.getStartTime())) {
        log.debug(
            "Extension {} disabled until {}", definition.getCode(), definition.getStartTime());
  ...
  /** 检查扩展是否在有效时间范围内 核心优化：提取公共时间范围检查逻辑，避免重复代码 */
  private boolean isExtensionEffective(ExtensionDefinition definition) {
    ...
    if (StringUtils.hasText(definition.getStartTime())) {
      try {
        LocalDateTime startTime =
            LocalDateTime.parse(definition.getStartTime(), DATE_TIME_FORMATTER);
        if (now.isBefore(startTime)) {
          return false;
        }
      } catch (DateTimeParseException e) {
        log.warn(
            "Invalid startTime format for extension {}: {}",
            definition.getCode(),
    ...
    if (StringUtils.hasText(definition.getEndTime())) {
      try {
        ...
        log.warn(
            "Invalid endTime format for extension {}: {}",
            definition.getCode(),
            definition.getEndTime());
      }
    }
  ...
  private void validateExtensionDefinition(ExtensionDefinition definition) {
    ...
    // 检查重复注册 - 使用新接口方法
    Optional<ExtensionDefinition> existing =
        extensionRepository.getExtensionByCode(
            definition.getExtensionPoint(), definition.getCode());
    if (existing.isPresent()) {
      throw new ExtensionRegistrationException(
          "Duplicate extension code: "
              + definition.getCode()
              + " for point: "
              + definition.getExtensionPoint());
    }

    // 验证时间范围
    if (StringUtils.hasText(definition.getStartTime())
        && StringUtils.hasText(definition.getEndTime())) {
      try {
        ...
        if (start.isAfter(end)) {
          log.warn(
              "Extension {} has invalid time range: startTime after endTime", definition.getCode());
        }
      } catch (DateTimeParseException e) {
  ...
  private void executeRegistration(String extensionPoint, ExtensionDefinition definition) {
    ...
    ExtensionDefinition previous =
        extensionRepository.registerExtension(extensionPoint, definition);
    if (previous != null) {
    ...
    log.info(
        "Extension registered successfully: {} -> {} [tenant={}, biz={}, priority={}, weight={}]",
        extensionPoint,
        definition.getCode(),
        definition.getTenant(),
        definition.getBizCode(),
  ...
  private void logRegistrationReport(long startTime) {
    ...
    log.info(
        ...
        stats.getSuccessfulRegistrations(),
        stats.getFailedRegistrations(),
        stats.getExtensionPointCount(),
        stats.getTotalExtensions(),
        stats.getEnabledExtensions(),
        stats.getUniqueExtensionCodes(),
