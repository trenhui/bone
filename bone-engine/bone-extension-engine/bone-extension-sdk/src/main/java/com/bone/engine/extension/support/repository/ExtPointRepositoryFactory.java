...
public class ExtPointRepositoryFactory {
  ...
  // 缓存已创建的仓库实例
  private static final Map<String, ExtensionRepository> REPOSITORY_CACHE =
      new ConcurrentHashMap<>();
  // 默认仓库类型
  private static final Class<? extends ExtensionRepository> DEFAULT_REPOSITORY_CLASS =
      InMemoryExtensionRepository.class;
        ...
        key -> {
          try {
            ...
            if (ExtensionRepository.class.isAssignableFrom(repositoryClass)) {
              ...
              log.warn(
                  "Repository class {} does not implement ExtensionRepository interface",
                  className);
              // 回退到硬编码的创建方式
            ...
            log.error(
                "Failed to create repository of type {} using reflection, falling back to default",
                className,
                e);
            // 反射失败时回退到硬编码方式
            return createRepositoryByType(className);
  ...
  private static ExtensionRepository createRepositoryByType(String className) {
    switch (className) {
      ...
      default:
        log.warn(
            "Unsupported repository type: {}, using MemExtensionRepository as default", className);
        return new InMemoryExtensionRepository();
    }
  }
