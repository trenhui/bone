...
public class ConfigurationChangeListener implements ApplicationListener<EnvironmentChangeEvent> {
    ...
    @Autowired
    public ConfigurationChangeListener(ExtensionConfigManager configManager, ConfigurableEnvironment environment, ApplicationContext applicationContext) {
    ...
    public void init() {
        ...
        Map<String, ConfigChangeListener> beans = applicationContext.getBeansOfType(ConfigChangeListener.class);
        if (!beans.isEmpty()) {
    ...
    public void onApplicationEvent(EnvironmentChangeEvent event) {
        ...
        // 筛选出扩展点相关的配置变更
        List<String> extensionChangedKeys = changedKeys.stream()
     ...
     * 通知配置变更监听器
     */
    private void notifyConfigChangeListeners(Iterable<org.springframework.core.env.PropertySource<?>> propertySources) {
        ...
        for (ConfigChangeListener listener : configChangeListeners) {
            try {
                ...
                if (prefixes != null && prefixes.length > 0) {
                    // 筛选出监听器关注的配置键
                    Set<String> relevantKeys = activeKeys.stream()
                            .filter(key -> Arrays.stream(prefixes).anyMatch(key::startsWith))
                            .collect(Collectors.toSet());
                    
                    if (!relevantKeys.isEmpty()) {
                        log.debug("Notifying listener {} with {} relevant config changes", 
                                listener.getClass().getSimpleName(), relevantKeys.size());
                        listener.onConfigChanged(relevantKeys);
                    }
                }
            } catch (Exception e) {
                log.error("Failed to notify config change listener: {}", 
    ...
    public void refreshExtensionConfig() {
        try {
            ...
            ExtensionProperties properties = binder.bind(EXTENSION_CONFIG_PREFIX, ExtensionProperties.class)
