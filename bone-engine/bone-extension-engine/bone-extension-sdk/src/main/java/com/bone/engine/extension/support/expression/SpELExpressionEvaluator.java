 ...
 * 高性能 SpEL 表达式求值器
 *
 * <p>设计原则： 1. 简洁高效：专注核心功能，避免过度设计 2. 性能优先：合理的缓存策略 3. 健壮性：完善的异常处理 4. 通用性：支持任意 Object 上下文
 */
public class SpELExpressionEvaluator implements ExpressionEvaluator {
  ...
  private Cache<String, Expression> buildExpressionCache(int maxSize, Duration expire) {
    return Caffeine.newBuilder().maximumSize(maxSize).expireAfterAccess(expire).build();
  }
  ...
  public void logCacheStats() {
    log.debug(
        "SpEL cache stats: expressions={}, predicates={}",
        expressionCache.estimatedSize(),
        predicateCache.estimatedSize());
  }
}
