 ...
 *
 * 该类负责在 Spring Bean 定义阶段执行两个主要的注册任务:
 * 1. 扫描被 @ExtensionPoint 注解的接口，并为每个接口注册一个代理 FactoryBean
 * (ExtensionPointFactoryBean)。
 * 2. 扫描被 @Extension 注解的实现类，并注册它们的 Bean 定义。
 *
...
public final class ExtensionPointRegister implements ImportBeanDefinitionRegistrar, ResourceLoaderAware, EnvironmentAware {
    ...
    /**
     * 注册被 @Extension 注解的类（扩展实现）的 Bean 定义。
     * 这些定义将被 Spring 实例化，其实例将在稍后由一个独立的组件（如实现 SmartInitializingSingleton 的 ExtensionRegister）收集和激活。
     */
     ...
     */
    private void registerFactoryBeanDefinition(AnnotationMetadata metadata, BeanDefinitionRegistry registry, Map<String, Object> attrs, LinkedHashSet<BeanDefinition> candidateComponents) {
        ...
        for (BeanDefinition candidateComponent : candidateComponents) {
            ...
            if (candidateComponent instanceof AnnotatedBeanDefinition annotatedBeanDefinition) {
                ...
                try {
                    ...
                    factoryBeanDefinition.getConstructorArgumentValues().addIndexedArgumentValue(0, extPointInterface);
