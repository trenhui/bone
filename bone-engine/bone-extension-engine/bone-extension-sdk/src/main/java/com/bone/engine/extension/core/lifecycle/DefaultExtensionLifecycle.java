 ...
 * 扩展点生命周期默认实现
 *
 * <p>为扩展点生命周期接口提供默认实现，扩展点实现类可以选择性地覆盖需要的方法 提供全面的日志记录、性能监控、异常处理和降级支持 <strong>默认行为：</strong>
 *
 * <ul>
 *   <li>预初始化：记录调试日志
 *   <li>初始化：记录信息日志，执行基础初始化
 *   <li>后初始化：记录调试日志，检查依赖
 *   <li>路由前处理：记录调试日志，允许路由属性修改
 *   <li>前置处理：记录调试日志，执行参数校验
 *   <li>环绕处理：支持同步/异步执行，性能监控
 *   <li>后置处理：记录调试日志，包含执行时间
 *   <li>异常处理：记录错误日志，提供详细诊断信息
 *   <li>降级处理：记录警告日志，抛出运行时异常
 *   <li>销毁前：记录信息日志，准备资源清理
 *   <li>销毁：记录信息日志，释放资源
 * </ul>
 *
...
public class DefaultExtensionLifecycle implements ExtensionLifecycle {
  ...
  public void postInitialize() {
    ...
    if (!initialized) {
      logger.warn(
          "Extension {} was post-initialized but not properly initialized",
          this.getClass().getName());
    }
  }
  ...
  public void beforeRouting(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Extension before routing: extensionPoint={}, context={}, routeAttributes={}",
  ...
  public void beforeInvoke(BizContext<?> context, String methodName, Object[] args) {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Extension before invoke: method={}, context={}, args count={}",
          methodName,
          context,
  ...
  @Override
  public CompletableFuture<Object> aroundInvoke(
    ...
    try {
      ...
      long executionTime = System.currentTimeMillis() - startTime;
      logger.error("Error in aroundInvoke: method={}, time={}ms", methodName, executionTime, e);
  ...
  public void afterInvoke(
      BizContext<?> context, String methodName, Object result, long executionTimeMs) {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Extension after invoke: method={}, context={}, executionTime={}ms",
          methodName,
          context,
          executionTimeMs);
      ...
      if (executionTimeMs > 500) {
        logger.warn(
            "Extension performance warning: method={} exceeded 500ms threshold, took {}ms",
            methodName,
            executionTimeMs);
      }
    }
  ...
  public void onException(
    ...
    if (logger.isErrorEnabled()) {
      ...
      logger.error(
          "Extension invoke error: method={}, context={}, errorType={}, executionTime={}ms, message={}",
          methodName,
          context,
          errorType,
          executionTimeMs,
  ...
  public Object onFallback(
    ...
    if (logger.isWarnEnabled()) {
      ...
      logger.warn(
          "Extension fallback triggered: method={}, context={}, cause={}",
          methodName,
          context,
          causeMessage);
    }
    ...
    throw new RuntimeException(
        "Extension execution failed and fallback was triggered: " + methodName, cause);
  }
  ...
  public void destroy() {
    ...
    if (logger.isInfoEnabled()) {
      ...
      logger.info("Extension destroyed: {}, uptime={}ms", this.getClass().getName(), uptime);
    }
  }
}
