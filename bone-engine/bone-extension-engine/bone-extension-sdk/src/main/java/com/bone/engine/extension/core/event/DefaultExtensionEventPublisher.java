 ...
 *
 * <p>集成Spring的事件机制，支持同步和异步事件发布，可配置的事件过滤 提供完整的事件生命周期管理和监控能力
 *
 * @author renhui.trh
 * @since 1.0.0
 */
public class DefaultExtensionEventPublisher implements ExtensionEventPublisher, InitializingBean {
  ...
  // 启用的事件类型集合
  private final Set<ExtensionEvent.EventType> enabledEventTypes =
      new CopyOnWriteArraySet<>(EnumSet.allOf(ExtensionEvent.EventType.class));

  // 是否异步发布事件
  private boolean asyncPublish = true; // 默认启用异步发布

  // 异步任务执行器名称限定符
  private static final String EXTENSION_EVENT_EXECUTOR_BEAN_NAME =
      ExtensionAsyncConfig.EXTENSION_EVENT_EXECUTOR_BEAN_NAME;

  // 失败事件回退队列
  private final ConcurrentLinkedQueue<ExtensionEvent<?>> fallbackEventQueue =
  ...
  public DefaultExtensionEventPublisher(
      ApplicationEventPublisher applicationEventPublisher, AsyncTaskExecutor taskExecutor) {
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    if (taskExecutor != null) {
      log.info(
          "ExtensionEventPublisher initialized with async capability. Task executor: {}",
      ...
      if (taskExecutor instanceof ThreadPoolExecutor) {
        ...
        log.info(
            "Async task executor configuration - Core pool size: {}, Max pool size: {}, Queue capacity: {}",
      ...
      log.warn(
          "ExtensionEventPublisher initialized in sync mode because AsyncTaskExecutor is not available");
  ...
  private void asyncPublishEvent(ExtensionEvent<?> event) {
    try {
      ...
      log.warn(
          "Async task rejected, event queue may be full. Switching to fallback mode for event: {}",
          event.getEventType(),
          e);
      // 任务被拒绝时添加到回退队列
      addToFallbackQueue(event);
  ...
  private void addToFallbackQueue(ExtensionEvent<?> event) {
    ...
    fallbackEventQueue.offer(event);
    log.debug(
        "Added event to fallback queue: {}, queue size: {}",
        event.getEventType(),
  ...
  @Override
  public void publishBeforeRegister(
      Object source, String extensionPointName, String extensionImplName) {
    ExtensionEvent<?> event =
        ExtensionEvent.builder(source)
  ...
  @Override
  public void publishAfterRegister(
      Object source, String extensionPointName, String extensionImplName) {
    ExtensionEvent<?> event =
        ExtensionEvent.builder(source)
  ...
  @Override
  public <T> void publishBeforeInvoke(
      Object source,
      String extensionPointName,
      String extensionImplName,
      BizContext<T> bizContext) {
    ExtensionEvent<T> event =
        ExtensionEvent.<T>builder(source)
  ...
  @Override
  public <T> void publishAfterInvokeSuccess(
      Object source,
      String extensionPointName,
      String extensionImplName,
      BizContext<T> bizContext,
  ...
  @Override
  public <T> void publishAfterInvokeFailure(
      Object source,
      String extensionPointName,
      String extensionImplName,
      BizContext<T> bizContext,
  ...
  @Override
  public <T> void publishExtensionNotFound(
      Object source, String extensionPointName, BizContext<T> bizContext) {
    ExtensionEvent<T> event =
        ExtensionEvent.<T>builder(source)
  ...
  @Override
  public <T> void publishRoutingDecision(
      Object source, String extensionPointName, String selectedImpl, BizContext<T> bizContext) {
    ExtensionEvent<T> event =
        ExtensionEvent.<T>builder(source)
  ...
  @Override
  public <T> void publishCacheMiss(
      Object source, String extensionPointName, BizContext<T> bizContext) {
    ExtensionEvent<T> event =
        ExtensionEvent.<T>builder(source)
  ...
  @Override
  public void publishConfigurationChanged(
      Object source, String extensionPointName, String changeType) {
    ExtensionEvent<?> event =
        ExtensionEvent.builder(source)
  ...
  @Override
  public void publishVersionRegister(
      Object source, String extensionPointName, String version, String extensionImplName) {
  ...
  @Override
  public <T> void publishVersionSwitch(
      Object source,
      String extensionPointName,
      String oldVersion,
      String newVersion,
  ...
  @Override
  public <T> void publishVersionCompatibilityCheck(
      Object source,
      String extensionPointName,
  ...
  @Override
  public <T> void publishDeprecatedVersionUsed(
      Object source,
      String extensionPointName,
  ...
  public String getEventStats() {
    ...
    return String.format("Fallback Queue Size: %d", fallbackEventQueue.size());
  }
