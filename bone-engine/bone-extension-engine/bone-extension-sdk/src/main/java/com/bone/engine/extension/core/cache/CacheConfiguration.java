...
public class CacheConfiguration {
   ...
   * 缓存管理器Bean
   *
   * <p>当分布式缓存启用且RedisTemplate存在时，使用分布式缓存
   */
  @Bean
  @ConditionalOnBean(RedisTemplate.class)
  @ConditionalOnProperty(name = "bone.extension.cache.distributed-enabled", havingValue = "true")
  public CacheManager cacheManager(
      ExtensionProperties properties, RedisTemplate<String, Object> redisTemplate) {
   ...
   * 缓存管理器Bean（仅本地缓存）
   *
   * <p>当分布式缓存未启用或RedisTemplate不存在时，仅使用本地缓存
   */
  @Bean
  @ConditionalOnProperty(
      name = "bone.extension.cache.distributed-enabled",
      havingValue = "false",
      matchIfMissing = true)
