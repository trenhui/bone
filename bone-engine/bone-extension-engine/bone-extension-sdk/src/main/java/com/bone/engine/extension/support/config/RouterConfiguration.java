...
public class RouterConfiguration {
    ...
    public synchronized void initialize(Properties properties) {
        ...
        if (properties != null) {
            // 从Properties加载配置
            this.cacheExpireTime = getIntProperty(properties, "cache.expireTime", DEFAULT_CACHE_EXPIRE_TIME);
            this.cacheMaxSize = getIntProperty(properties, "cache.maxSize", DEFAULT_CACHE_MAX_SIZE);
            this.slowRouteThresholdMs = getIntProperty(properties, "route.slowThresholdMs", DEFAULT_SLOW_ROUTE_THRESHOLD_MS);
            ...
            this.optimizeIntervalMs = getIntProperty(properties, "cache.optimizeIntervalMs", DEFAULT_OPTIMIZE_INTERVAL_MS);
        }
        
        initialized.set(true);
        logger.info("RouterConfiguration initialized: cacheExpireTime={}min, cacheMaxSize={}, cacheEnabled={}",
                cacheExpireTime, cacheMaxSize, cacheEnabled);
    }
    ...
    // Getter方法
    public int getCacheExpireTime() { return cacheExpireTime; }
    public int getCacheMaxSize() { return cacheMaxSize; }
    public int getSlowRouteThresholdMs() { return slowRouteThresholdMs; }
    public boolean isCacheEnabled() { return cacheEnabled; }
    public boolean isStatsEnabled() { return statsEnabled; }
    public int getOptimizeIntervalMs() { return optimizeIntervalMs; }
    public boolean isInitialized() { return initialized.get(); }
    
    // 缓存优化相关方法
    public boolean isCacheOptimizationEnabled() { return cacheEnabled; }
    public int getOptimizeCallThreshold() { return 100; } // 默认阈值为100次调用
    public int getSlowThresholdMs() { return slowRouteThresholdMs; }
    
    // 路由策略相关方法
    public boolean isWeightedRoutingEnabled() { return false; } // 默认禁用权重路由
    public boolean isGrayReleaseEnabled() { return false; } // 默认禁用灰度发布
}