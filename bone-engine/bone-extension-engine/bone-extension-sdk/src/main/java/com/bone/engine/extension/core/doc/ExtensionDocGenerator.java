...
import com.bone.engine.extension.api.model.metadata.ExtensionPointDocMetadata;
...
/**
 * 扩展文档生成器
 * 增强 @ExtPointDoc 注解的功能，实现自动生成扩展点文档
 */
@Slf4j
@Component
public class ExtensionDocGenerator {
     ...
     */
    private void generateMainDocumentation(List<Class<?>> extensionPointClasses, String outputDir) throws IOException {
        ...
        try (FileWriter writer = new FileWriter(file)) {
            ...
            for (Class<?> clazz : extensionPointClasses) {
                ...
                if (docAnnotation != null) {
                    writer.write(String.format("- [%s](%s.md) - %s\n", 
                            clazz.getSimpleName(), 
                            clazz.getSimpleName(), 
                            docAnnotation.description()));
                } else {
                    writer.write(String.format("- [%s](%s.md)\n", 
                            clazz.getSimpleName(), 
                            clazz.getSimpleName()));
                }
            }
     ...
     * @throws IOException IO异常
     */
    private void generateExtensionPointDocumentation(Class<?> extensionPointClass, String outputDir) throws IOException {
    ...
    public String generateOnlineDocumentation(List<Class<?>> extensionPointClasses) {
        ...
        html.append(".extension-point { margin: 20px 0; padding: 15px; border: 1px solid #ddd; border-radius: 5px; }");
        ...
        for (Class<?> clazz : extensionPointClasses) {
            ...
            for (Method method : methods) {
                ...
                html.append(String.format("<p><strong>参数:</strong> %s</p>", getParameterDescription(method)));
                html.append(String.format("<p><strong>返回值:</strong> %s</p>", method.getReturnType().getSimpleName()));
