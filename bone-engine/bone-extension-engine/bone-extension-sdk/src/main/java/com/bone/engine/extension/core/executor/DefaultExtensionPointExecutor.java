 ...
 * 默认扩展点执行器
 *
 * <p>实现扩展点的安全执行，提供以下核心功能： 1. 安全的方法执行环境 2. 异常捕获和转换 3. 执行性能监控
 *
 * @since 1.0.0
 */
public class DefaultExtensionPointExecutor implements ExtensionPointExecutor {
  ...
  @Autowired private ExtensionPermissionManager extensionPermissionManager;

  @Nullable private StudioExecutionLogReporter studioReporter;

  @Nullable private ExtensionExecutionGuard executionGuard;

  @Nullable private ExtensionMetricsCollector metricsCollector;
   ...
   */
  @Override
  public <T> T execute(Object implementation, BizContext<?> context, Method method, Object[] args)
      throws Throwable {
    // 1. 验证参数有效性
    ...
    try {
      // 3. 检查权限
      if (extensionPermissionManager != null
          && !extensionPermissionManager.checkPermission(implementation, method)) {
        String errorMsg =
            String.format(
                "Extension point %s does not have permission to call method %s",
   ...
   */
  private void validateExecutionParameters(
      Object implementation, BizContext<?> context, Method method) {
   ...
   * @throws Throwable 执行过程中的异常
   */
  private <T> T invokeForGuard(Object implementation, Method method, Object[] args)
      throws Exception {
    try {
  ...
  private <T> T executeMethod(Object implementation, Method method, Object[] args)
      throws Throwable {
    try {
  ...
  private void handleSecurityException(
      SecurityException ex, String className, String methodName, long startTime) {
    long executionTime = System.currentTimeMillis() - startTime;

    log.error(
        "Security exception in extension point: {}.{} after {}ms - {}",
        className,
        methodName,
        executionTime,
        ex.getMessage());
  ...
  private void handleExecutionException(
      Exception e, String className, String methodName, long startTime) {
    long executionTime = System.currentTimeMillis() - startTime;

    log.error(
        "Execution exception in extension point: {}.{} after {}ms",
        className,
        methodName,
        executionTime,
        e);
