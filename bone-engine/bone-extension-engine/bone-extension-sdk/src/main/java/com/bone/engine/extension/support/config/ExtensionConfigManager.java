 ...
 * 扩展点配置管理器
 *
 * <p>负责管理扩展点的全局配置，包括缓存设置、路由规则、扩展点开关等 <strong>主要功能：</strong>
 *
 * <ul>
 *   <li>提供统一的配置访问接口
 *   <li>支持扩展点特定配置
 *   <li>作为ExtensionProperties的适配层
 * </ul>
 *
...
public class ExtensionConfigManager {
   ...
   * 获取全局配置值
   *
   * <p>直接从ExtensionProperties获取配置，提供统一的访问接口
   */
   ...
   * 获取布尔类型的全局配置值
   *
   * <p>直接从ExtensionProperties获取配置，提供类型转换
   */
  ...
  /** 获取扩展点特定配置的布尔值 */
  public boolean getExtPointConfigBoolean(
      String extPointName, String configKey, boolean defaultValue) {
  ...
  public void setExtPointConfig(String extPointName, String configKey, String value) {
    extPointConfigs.computeIfAbsent(extPointName, k -> new HashMap<>()).put(configKey, value);
  }
  ...
  public boolean isExtPointCacheEnabled(String extPointName) {
    ...
    return getExtPointConfigBoolean(
        extPointName, "cache.enabled", getGlobalConfigBoolean("cache.enabled", true));
  }
  ...
  /** 清理配置缓存 */
  /** 清除扩展点特定配置缓存 （全局配置直接从ExtensionProperties获取，无需缓存） */
  ...
  /**
   * 获取所有配置项 注意：此方法仅返回扩展点特定配置，全局配置应通过getGlobalConfig方法获取
   *
   * @return 所有配置项的Map
   */
