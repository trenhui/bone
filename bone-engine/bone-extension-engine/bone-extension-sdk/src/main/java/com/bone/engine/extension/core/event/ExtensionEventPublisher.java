 ...
 *
 * <p>提供统一的事件发布接口，支持同步和异步事件发布，便于监控和扩展扩展点的行为 实现观察者模式，降低系统组件间的耦合度 集成版本管理事件，支持多版本扩展点的事件通知
 *
 * @author renhui.trh
 * @since 1.0.0
 */
public interface ExtensionEventPublisher {
   ...
   */
  <T> void publishBeforeInvoke(
      Object source, String extensionPointName, String extensionImplName, BizContext<T> bizContext);
   ...
   */
  <T> void publishAfterInvokeSuccess(
      Object source,
   ...
   * @param executionTimeMs 执行耗时（毫秒）
   */
  <T> void publishAfterInvokeFailure(
      Object source,
      String extensionPointName,
      String extensionImplName,
      BizContext<T> bizContext,
   ...
   * @param bizContext 业务上下文
   */
  <T> void publishExtensionNotFound(
   ...
   * @param <T> 业务上下文类型
   */
  <T> void publishVersionCompatibilityCheck(
      Object source,
      String extensionPointName,
   ...
   * @param <T> 业务上下文类型
   */
  <T> void publishDeprecatedVersionUsed(
      Object source,
      String extensionPointName,
