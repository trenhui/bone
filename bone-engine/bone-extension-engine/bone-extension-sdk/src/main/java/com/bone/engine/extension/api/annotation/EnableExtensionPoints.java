 ...
 *
 * <p>在Spring Boot应用中启用企业级插件化框架，支持智能路由、多租户隔离和动态扩展能力。
 *
 * @see ExtensionPoint
 * @see Extension
 * @since 1.0.0
...
public @interface EnableExtensionPoints {
   ...
   * 自定义路由器实现类
   *
   * <p>必须实现 ExtensionPointRouter 接口，优先级高于内置路由器
   */
   ...
   * 自定义执行器实现类
   *
   * <p>必须实现 ExtensionExecutor 接口，优先级高于内置执行器
   */
   ...
   * 路由策略
   *
   * <p>支持：default(按精度), score（评分）、priority（优先级）、weight（权重）、first（首次匹配）
   */
