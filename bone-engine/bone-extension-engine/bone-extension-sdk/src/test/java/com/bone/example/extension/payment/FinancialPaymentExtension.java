 ...
 * 金融支付扩展点实现类
 * <p>
 * 提供金融机构场景下的支付处理逻辑，包括费用计算、合规检查等功能
 */
@Extension(
    name = "金融支付扩展实现",
...
public class FinancialPaymentExtension implements PaymentExtPoint {
     ...
     * 支付前验证
     * <p>
     * 在支付处理前验证支付请求的合法性，特别针对金融机构的合规要求
     * 
    ...
    public PaymentCalculationResult calculatePayment(final BizContext<PaymentTestRequest> context) {
        ...
        logger.info("金融支付金额计算完成，订单ID: {}, 原始金额: {}, 手续费: {}, 最终金额: {}",
                request.getOrderId(), originalAmount, FEE_AMOUNT, finalAmount);
