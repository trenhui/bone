...
public class PaymentService {
     ...
     * 主力支付处理方法（生产推荐）
     */
    public PaymentResult processPayment(PaymentTestRequest request,
        ...
        try (ExtensionScope scope = ExtensionContextManager.with(context)) {
            ...
            log.info("开始处理支付请求 | tenant={} | orderId={} | requestId={} | amount={} | method={}",
                    tenant, orderId, requestId, request.getAmount(), request.getPaymentMethod());

            return executePaymentFlow(request, context, requestId);

        } catch (Exception e) {
            log.error("支付处理系统异常 | orderId={} | requestId={}", request.getOrderId(), context.getRequestId(), e);
            throw new PaymentProcessingException("支付系统繁忙，请稍后重试", e);
        }
    }

    private PaymentResult executePaymentFlow(PaymentTestRequest request,
                                             BizContext<PaymentTestRequest> context,
                                             String requestId) {
        ...
        try {
            ...
            log.info("支付处理成功 | orderId={} | txnId={} | finalAmount={}",
                    request.getOrderId(), coreResult.getTransactionId(), coreResult.getAmount());
    ...
    private void ensureContextData(PaymentTestRequest request, BizContext<PaymentTestRequest> context) {
        if (context.getData() == null) {
            log.debug("BizContext 中 data 字段为空，框架自动补齐 | orderId={}", request.getOrderId());

            // 完美重建，字段一个不漏
            BizContext<PaymentTestRequest> enriched = BizContext.<PaymentTestRequest>builder()
    ...
    private void invokePostPayProcess(PaymentResult result, BizContext<PaymentTestRequest> origContext) {
        try {
            BizContext<PaymentResult> postContext = BizContext.<PaymentResult>builder()
    ...
    private void validateCalculation(PaymentCalculationResult result) {
        if (result == null || result.getFinalAmount() == null ||
