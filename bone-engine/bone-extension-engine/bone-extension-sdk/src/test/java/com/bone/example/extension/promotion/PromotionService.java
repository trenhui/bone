 ...
 * 促销服务核心实现类（企业级终极版）
 *
 * <p>特性： - 完全基于 BizContext 驱动路由 - 优雅降级 + 熔断保护 - 完整日志链路 - 严格参数校验 + 防御性编程 - 线程安全 + 资源自动释放
 */
@Slf4j
@Component
public class PromotionService {
  ...
  /** 主力促销计算方法（生产推荐） */
  public PromotionResult calculatePromotion(
      PromotionRequest request, BizContext<PromotionRequest> context) {
    ...
    try (ExtensionScope scope = ExtensionContextManager.with(context)) {
      ...
      log.info(
          "开始计算促销 | tenant={} | requestId={} | subtotal={} | orderType={} | userLevel={}",
          tenant,
          requestId,
          request.getSubtotal(),
  ...
  private PromotionResult executePromotionCalculation(
      PromotionRequest request, BizContext<PromotionRequest> context, String requestId) {
    try {
      return promotionExtPoint.calculatePromotion(context);
    } catch (Exception e) {
      // 检查异常类型或消息，确定是否为路由失败
      log.warn(
          "促销扩展点路由失败，使用默认结果 | tenant={} | requestId={} | 错误={}",
          context.getTenant(),
          requestId,
          e.getMessage());
      throw e;
  ...
  private void ensureContextData(PromotionRequest request, BizContext<PromotionRequest> context) {
    if (context.getData() == null) {
      ...
      // 完美重建上下文
      BizContext<PromotionRequest> enriched =
          BizContext.<PromotionRequest>builder()
