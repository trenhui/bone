...
public class ProductSpecificPromotionExtension implements PromotionExtPoint {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ProductSpecificPromotionExtension.class);
    ...
    public PromotionResult calculatePromotion(BizContext<PromotionRequest> context) {
        ...
        for (PromotionRequest.OrderItem item : items) {
            ...
            if ("ELECTRONICS".equals(item.getCategory())) {
                BigDecimal itemOriginal = item.getUnitPrice().multiply(BigDecimal.valueOf(item.getQuantity()));
                ...
                // 创建商品级别的促销信息
                PromotionResult.AppliedPromotion itemPromotion = PromotionResult.AppliedPromotion.builder()
                        .promotionId(PROMOTION_ID)
                        .promotionName(PROMOTION_NAME)
                        .promotionType(PROMOTION_TYPE)
                        .discountAmount(itemSaving)
                        .description(String.format("商品 %s (ID: %s) 享受5%%折扣", item.getProductName(), item.getProductId()))
                        .build();
