...
public class MedicalClaimService {
    ...
    private MedicalClaimResult executeClaimFlow(final MedicalClaimRequest request, 
        ...
        try {
            ...
            // 执行验证
            final ValidationResult validationResult = invokeWithFallback(
                    () -> extPoint.validateClaim(context),
            ...
            // 执行理赔处理
            MedicalClaimResult result = invokeWithFallback(
                    () -> extPoint.processClaim(context),
    ...
    private <T> T invokeWithFallback(Supplier<T> supplier, T fallback) {
        try {
            ...
            log.warn("扩展点调用异常，触发降级 | error={}", e.getMessage(), e);
            return fallback;
        }
    }
    ...
    public ValidationResult validateClaim(final MedicalClaimRequest request) {
        try {
            ...
            try (ExtensionScope scope = ExtensionContextManager.with(context)) {
                ...
                // 执行验证
                final ValidationResult result = invokeWithFallback(
                        () -> extPoint.validateClaim(context),
                        ValidationResult.success()
                );
            ...
            log.error("理赔请求验证业务异常 | env={} | claimId={} | code={}", 
                    environment, request != null ? request.getClaimId() : "未知", e.getErrorCode(), e);
            return ValidationResult.fail(e.getErrorCode(), e.getMessage());
        } catch (Exception e) {
            log.error("理赔请求验证系统异常 | env={} | claimId={}", 
                    environment, request != null ? request.getClaimId() : "未知", e);
