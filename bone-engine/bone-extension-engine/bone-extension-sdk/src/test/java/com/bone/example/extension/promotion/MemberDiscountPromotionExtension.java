...
@Extension(
    ...
    condition =
        "#data.userLevel == 'VIP' || #data.userLevel == 'GOLD' || #data.userLevel == 'DIAMOND'",
    order = 120,
    enabled = true,
    version = "1.0.0")
public class MemberDiscountPromotionExtension implements PromotionExtPoint {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(MemberDiscountPromotionExtension.class);
  ...
  public PromotionResult calculatePromotion(BizContext<PromotionRequest> context) {
    ...
    // 创建应用的促销信息
    PromotionResult.AppliedPromotion appliedPromotion =
        PromotionResult.AppliedPromotion.builder()
            ...
            .description(
                String.format(
                    "会员等级折扣：%s享%s折", memberLevel, discountRate.multiply(BigDecimal.valueOf(10))))
            .build();
