 ...
 * 默认积分服务实现类
 *
 * <p>提供积分价值计算和积分兑换规则管理的标准实现 支持积分上限限制、精确的货币价值计算和异常处理
 */
@Service
public class DefaultPointsService implements PointsService {
   ...
   * 计算积分的货币价值
   *
   * <p>将用户持有的积分精确转换为对应的货币价值， 包含参数验证、上限检查和精确的金额计算
   *
  ...
  public BigDecimal calculatePointsValue(final int points) {
    ...
    if (points > MAX_POINTS_PER_CALCULATION) {
      LOGGER.warn(
          "Points exceeds calculation limit: [requestedPoints={}, maxLimit={}]",
          points,
          MAX_POINTS_PER_CALCULATION);
    }
    ...
    try {
      ...
      final BigDecimal calculatedValue =
          pointsDecimal.multiply(rate).setScale(2, RoundingMode.HALF_UP);

      LOGGER.info(
          "Points value calculation completed: [points={}, calculatedValue={}]",
          points,
          calculatedValue);
