 ...
 * 优惠券服务接口
 *
 * <p>定义优惠券相关的核心业务操作，包括优惠券验证和折扣计算功能。 所有实现类应确保财务计算的精确性，并提供严格的参数验证。
 */
public interface CouponService {
   ...
   */
  ValidationResult validateCoupon(
      final String couponId, final String userId, final BigDecimal amount);

  /**
   * 计算优惠券实际折扣金额
   *
   * <p>根据优惠券类型、规则和订单金额精确计算可享受的折扣金额。 实现类应确保返回的折扣金额不会超过订单总额，并保持适当的精度。
   *
   * @param couponId 优惠券ID，不能为空或空白
