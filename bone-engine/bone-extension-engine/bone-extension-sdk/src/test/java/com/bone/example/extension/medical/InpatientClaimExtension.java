 ...
 * 住院理赔扩展点实现类
 *
 * <p>专门处理住院类型的医疗保险理赔场景，提供完整的验证和处理逻辑。 实现了对住院期间各项费用的验证、计算和理赔金额确定。
 */
// 运行时路由配置 - 负责匹配和选择
@Extension(
    name = "住院理赔扩展实现",
...
public class InpatientClaimExtension implements MedicalClaimExtPoint {
   ...
   * 验证住院理赔请求的合法性
   *
   * <p>执行严格的参数验证，确保上下文和请求数据的完整性和有效性。 对住院特定信息进行验证，包括入院日期、出院日期、住院天数等。
   *
  ...
  public ValidationResult validateClaim(final BizContext<MedicalClaimRequest> context) {
    ...
    if (request.getClaimType() != MedicalClaimRequest.ClaimType.INPATIENT) {
      log.warn(
          "理赔类型不匹配 | requestId={} | claimId={} | expected=INPATIENT | actual={}",
          requestId,
          request.getClaimId(),
    ...
    if (!validationResult.isSuccess()) {
      log.warn(
          "住院理赔特定信息验证失败 | requestId={} | claimId={} | userId={} | error={}",
          requestId,
          request.getClaimId(),
          request.getUserId(),
    ...
    log.info(
        "住院理赔请求验证通过 | requestId={} | claimId={} | userId={} | hospitalStayDays={}",
        requestId,
        request.getClaimId(),
        request.getUserId(),
        calculateHospitalStayDays(request));
   ...
   * 处理住院理赔请求
   *
   * <p>在验证通过的基础上，进行理赔金额计算和结果生成。 实现住院费用的理赔计算逻辑，考虑住院天数、费用类型等因素。
   *
  ...
  public MedicalClaimResult processClaim(final BizContext<MedicalClaimRequest> context) {
    ...
    // 计算每日平均费用
    BigDecimal dailyAverageCost =
        calculateDailyAverageCost(request.getTotalAmount(), hospitalStayDays);

    // 根据住院天数和费用计算理赔金额
    BigDecimal approvedAmount =
        calculateApprovedAmount(request.getTotalAmount(), hospitalStayDays, dailyAverageCost);
    ...
    log.info(
        "住院理赔处理完成 | requestId={} | claimId={} | totalAmount={} | approvedAmount={} | hospitalStayDays={}",
        requestId,
        request.getClaimId(),
  ...
  private ValidationResult validateInpatientInfo(final MedicalClaimRequest request) {
    ...
    if (hospitalStayDays < MIN_HOSPITAL_STAY_DAYS) {
      return ValidationResult.fail(
          "INVALID_STAY_DURATION", "住院天数必须大于等于" + MIN_HOSPITAL_STAY_DAYS + "天");
    }

    if (hospitalStayDays > MAX_HOSPITAL_STAY_DAYS) {
      return ValidationResult.fail(
          "EXCESSIVE_STAY_DURATION", "住院天数不能超过" + MAX_HOSPITAL_STAY_DAYS + "天");
    }
    ...
    if (dailyAverageCost.compareTo(DAILY_COST_LIMIT) > 0) {
      return ValidationResult.fail("EXCESSIVE_DAILY_COST", "每日平均费用超出限制，限制金额: " + DAILY_COST_LIMIT);
    }
  ...
  private BigDecimal calculateApprovedAmount(
      final BigDecimal totalAmount, final long days, final BigDecimal dailyCost) {
    // 计算基础报销金额
    BigDecimal baseApprovedAmount =
        totalAmount.multiply(REIMBURSEMENT_RATE).setScale(2, java.math.RoundingMode.HALF_UP);
