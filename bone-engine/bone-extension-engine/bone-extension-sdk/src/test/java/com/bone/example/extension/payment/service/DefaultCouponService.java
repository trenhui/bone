 ...
 * 优惠券服务标准实现
 * <p>
 * 提供优惠券验证、折扣计算等核心业务功能的标准实现。
 * 支持严格的参数校验、金额阈值验证和精确的折扣计算，确保财务数据的准确性和业务规则的一致性。
 */
@Service
public class DefaultCouponService implements CouponService {
    ...
    private static final BigDecimal DEFAULT_DISCOUNT_AMOUNT = new BigDecimal("10.00").setScale(2, RoundingMode.HALF_UP);
    private static final BigDecimal MINIMUM_ORDER_THRESHOLD = new BigDecimal("100.00").setScale(2, RoundingMode.HALF_UP);
     ...
     */
    @Override
    public ValidationResult validateCoupon(final String couponId, final String userId, final BigDecimal amount) {
        ...
        try {
            ...
            if (amount.compareTo(MINIMUM_ORDER_THRESHOLD) < 0) {
                final String errorMessage = String.format("订单金额未达到优惠券使用门槛(最低需要%.2f元)", 
                                                        MINIMUM_ORDER_THRESHOLD.doubleValue());
                LOGGER.warn("优惠券验证失败: 用户[{}]订单金额未达最低门槛，要求: {}, 实际: {}", 
                           userId, MINIMUM_ORDER_THRESHOLD, amount);
            ...
            return ValidationResult.fail(ERROR_CODE_VALIDATION_FAILURE, "优惠券验证过程中发生错误: " + e.getMessage());
        }
    }
     ...
     * 计算优惠券实际折扣金额
     * <p>
     * 根据优惠券配置和订单金额精确计算可应用的折扣金额。
     * 确保折扣不超过订单总额，并使用HALF_UP舍入模式保证财务计算的准确性。
     * 
     * @param couponId 优惠券ID，不能为空或空白
    ...
    public BigDecimal calculateDiscount(final String couponId, final BigDecimal amount) {
        ...
        try {
            ...
            if (discountAmount.compareTo(amount) > 0) {
                ...
                LOGGER.warn("调整折扣金额以匹配订单金额: [couponId={}, 调整后折扣={}]", 
                           couponId, discountAmount);
            }
            
            LOGGER.info("优惠券折扣计算完成: [couponId={}, 折扣金额={}]", 
                       couponId, discountAmount);
     ...
     */
    private void validateParameters(final String couponId, final String userId, final BigDecimal amount) {
        validateCouponId(couponId);
