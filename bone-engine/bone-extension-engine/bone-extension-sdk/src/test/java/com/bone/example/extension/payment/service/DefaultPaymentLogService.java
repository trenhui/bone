 ...
 * 默认支付日志服务实现类
 *
 * <p>提供支付操作的标准化日志记录服务，包括支付请求日志、支付结果日志和异常日志 支持详细的交易信息记录和格式化输出
 */
@Service
public class DefaultPaymentLogService implements PaymentLogService {
   ...
   * 记录支付结果日志
   *
   * <p>记录完整的支付结果信息，包括交易ID、金额、状态等关键信息 成功时记录常规信息，失败时包含详细的错误信息
   *
  ...
  public void logPayment(final PaymentResult result) {
    ...
    if (!result.isSuccess()) {
      logBuilder
          .append(formatField("错误码", safeValue(result.getErrorCode())))
   ...
   * 记录支付异常日志
   *
   * <p>专门用于记录支付过程中发生的异常情况，包含完整的错误上下文信息
   *
   ...
   */
  @Override
  public void logPaymentException(
   ...
   * 记录支付请求日志
   *
   * <p>记录支付请求的详细信息，用于请求追踪和问题排查 包含订单、用户、金额等核心信息
   *
   * @param orderId 订单ID
   * @param userId 用户ID
   * @param amount 支付金额
