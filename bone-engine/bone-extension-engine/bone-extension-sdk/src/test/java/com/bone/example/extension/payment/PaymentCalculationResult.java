 ...
 * 支付计算结果领域对象 - 精炼实现
 *
 * <p>支持 Builder 模式构建，确保对象不可变性。 包含支付金额计算相关明细。
 *
 * <h3>设计原则：</h3>
 *
 * <ul>
 *   <li>不可变性：字段 final，通过 Builder 构建
 *   <li>完整性：包含所有计算相关信息
 *   <li>业务方法：如 getDiscountAmount() 计算折扣
 *   <li>扩展性：extendedAttributes 支持自定义属性
 *   <li>安全性：字段非null校验，金额默认零值
 * </ul>
 */
public class PaymentCalculationResult {
  ...
  private PaymentCalculationResult(Builder builder) {
    this.originalAmount =
        Objects.requireNonNull(builder.originalAmount, "originalAmount cannot be null");
    ...
    this.extendedAttributes =
        builder.extendedAttributes != null
            ? new HashMap<>(builder.extendedAttributes)
            : new HashMap<>();
  }
