...
public class ExtPointCommandApplicationService {
    ...
    return idempotentExecutor.execute(
        ...
        () -> {
          ...
          return StudioCommandResponses.created(
              StudioCommandResponses.resourceLocation("points", saved.getId()), "创建扩展点成功", saved);
        });
  }

  public ResponseEntity<ApiResponse<ExtPoint>> updatePoint(
      Long id, ExtPoint body, Integer expectedVersion) {
  ...
  public ResponseEntity<ApiResponse<ExtPoint>> enablePoint(Long id, boolean enabled) {
    ...
    return ResponseEntity.ok(ApiResponse.success((enabled ? "启用" : "禁用") + "扩展点成功", updated));
  }
