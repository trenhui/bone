...
public class ExtensionStudioCommandHandler {
            ...
            return idempotentExecutor.execute(
                    ...
                    () -> {
                        ...
                        return StudioCommandResponses.created(
                                StudioCommandResponses.resourceLocation("plugins", saved.getId()),
                                "创建插件成功",
                                saved);
                    });
    ...
    public ResponseEntity<ApiResponse<Extension>> uploadPlugin(
        ...
        return uploadPlugin(
                idempotencyKey, fingerprint, file, extPointId, name, version, className, description, pluginId);
    }
    ...
    public ResponseEntity<ApiResponse<Extension>> uploadPlugin(
        ...
        try {
            return idempotentExecutor.execute(
                    ...
                    requestFingerprint,
                    () -> {
                        Extension saved = extensionCommandHandler.uploadPluginArtifact(
    ...
    public ResponseEntity<?> deployPlugin(Long id, Boolean syncParam, String idempotencyKey) {
        ...
        if (resolveDeploySync(syncParam)) {
            return idempotentExecutor.execute(idempotencyKey, "POST", path, fingerprint, () -> lifecycle(id, true, "部署"));
        }
        return idempotentExecutor.execute(
                idempotencyKey,
                "POST",
    ...
    public ResponseEntity<ApiResponse<Extension>> rollbackPlugin(
        ...
        String fingerprint =
                StudioIdempotencyService.fingerprint(body != null ? objectMapper.writeValueAsString(body) : "");
    ...
    public ResponseEntity<ApiResponse<PluginExecutionLog>> simulatePlugin(Long id) {
        try {
            return ResponseEntity.ok(
                    ApiResponse.success(
                            "模拟调用成功", extensionCommandHandler.simulatePluginExecution(id)));
        } catch (IllegalArgumentException | IllegalStateException ex) {
            return StudioCommandResponses.badRequest(ex.getMessage());
        }
    }
