...
@ConditionalOnProperty(
        ...
        matchIfMissing = true)
public class InMemoryStudioAuditStore implements StudioAuditStore {
    ...
    public PageResult<StudioAuditEntry> queryByCursor(
        ...
        List<StudioAuditEntry> sorted = entries.stream()
                .filter(e -> !StringUtils.hasText(action) || action.equals(e.getAction()))
                .filter(e -> !StringUtils.hasText(resourceType) || resourceType.equals(e.getResourceType()))
                .sorted(Comparator.comparing(StudioAuditEntry::getId, Comparator.nullsLast(Comparator.reverseOrder())))
                .toList();
        List<StudioAuditEntry> page = sorted.stream()
