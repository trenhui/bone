...
4. `domain/service/` + `infrastructure/`：可复用逻辑按性质分流——领域规则下沉 `domain/service`（如 `OrderLookup` 租户隔离校验），技术横切放 `infrastructure`（如 `AggregatePersistence` 保存+事件发布、`TenantSupport` 租户上下文）  
