 ...
 * 关闭超时未支付支付单定时任务。
 *
 * <p>定时扫描仍处于 PENDING/PAYING 且创建时间早于超时阈值的支付单，逐笔下发 {@link CloseExpiredPaymentCommand}（经应用服务执行，
 * adapter 不直连 domain 仓储）。
 *
 * <p><b>全租户扫描（E-2）</b>：与 {@link CancelExpiredOrderJob} 同理——定时线程无请求上下文，按"当前租户"扫描 只会落到降级后的平台租户
