 ...
 * <p>
 * 注意：此处仅记录消费结果，不应再次调用 {@code PayOrderCommandHandler}（避免重复支付）。
 */
@Slf4j
@Component
...
public class OrderPaidIntegrationMqListener implements RocketMQListener<String> {
    ...
    public void onMessage(String envelopeJson) {
        try {
            OrderPaidIntegrationEvent event = objectMapper.readValue(envelopeJson, OrderPaidIntegrationEvent.class);
            log.info(
