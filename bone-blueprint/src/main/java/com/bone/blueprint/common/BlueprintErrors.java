 ...
 * 用它设置响应状态），而<em>业务码</em>只能以字符串拼进 message——于是「码」与「状态」天然是两份数据。 若在每个抛出点手写 {@code new
 * BizException(404, ORDER_NOT_FOUND + ": " + id)}，状态与码的配对就散落在全模块几十处，任一处不一致都没有机制发现（错误码登记 §17 明文「禁止把
 * HTTP 码与业务码混为同一个整数」）。
 *
 * <p><b>本类的做法</b>：把配对收进 {@link #DEFAULT_HTTP_STATUS} 一张表，抛出点只表达<strong>业务语义</strong>（哪个码 +
 * 什么上下文）。表与《Bone-错误码登记》§6 的 {@code BP_} 表逐行对应，是代码侧的唯一副本。
 *
 * <p><b>新增错误码的强制约束</b>：{@link BlueprintErrorCodes} 的每个 public String 常量都必须在表中登记状态， 否则类加载即抛 {@link
...
public final class BlueprintErrors {
   ...
   * 码 → 默认 HTTP 状态（真源）。
   *
   * <p>与《Bone-错误码登记》§6 {@code BP_} 表一一对应；{@link #checkEveryCodeRegistered()} 保证不会漏。
   */
  ...
  public static int httpStatusOf(String errorCode) {
    ...
    if (status == null) {
      throw new IllegalStateException(
          "错误码未登记默认 HTTP 状态: "
              + errorCode
              + "；请在 BlueprintErrors 的 DEFAULT_HTTP_STATUS 中登记，"
  ...
  /**
   * 校验 {@link BlueprintErrorCodes} 的每个码常量都在状态表中。
   *
  ...
  private static void checkEveryCodeRegistered() {
    for (Field field : BlueprintErrorCodes.class.getDeclaredFields()) {
      ...
      if (!DEFAULT_HTTP_STATUS.containsKey(code)) {
        throw new IllegalStateException(
            "错误码常量未在 BlueprintErrors 登记默认 HTTP 状态: "
                + "BlueprintErrorCodes."
                + field.getName()
                + " = \""
                + code
