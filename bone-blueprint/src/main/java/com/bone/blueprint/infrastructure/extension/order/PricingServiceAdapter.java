...
/**
 * {@link PricingService} 基础设施实现：装配扩展点运行期上下文并调用 {@link OrderPriceCalculator}。
 *
 * <p>扩展引擎上下文（{@code BizContext} / {@code ExtensionContextManager}）属技术设施，下沉到本适配器； 应用层（{@code
 * CreateOrderCommandHandler}）仅依赖 {@link PricingService} 接口，不再引入 {@code com.bone.engine.*}。 {@code
