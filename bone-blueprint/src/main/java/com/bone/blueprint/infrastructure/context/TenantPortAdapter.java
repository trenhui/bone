...
/**
 * 租户上下文适配器：经 {@link TenantPort} 端口向应用层提供当前租户 ID（基础设施实现）。
 *
 ...
 *
 * <p><b>降级审计（E-2）</b>：异步分支（定时任务 / 线程池 / MQ 消费 / Outbox 中继）若未显式传递租户上下文， {@code TenantContext}
...
@Slf4j
@Component
public class TenantPortAdapter implements TenantPort {
    ...
    if (tenantId == null) {
      log.warn(
          "[TenantContext 缺失] 线程 {} 无租户上下文，降级为平台租户 tenantId={}，调用来源={}。"
              + "请确认该异步入口（定时任务/线程池/MQ/Outbox）已显式传递租户或已在 README 登记打洞（E-2）",
