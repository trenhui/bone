 ...
 * domain/gateway}（E-4.3 列表）；{@code OrderMessageSender}、 {@code OrderOutboxRelayPort}、{@code
 * TenantProvider} 同理。E-10.2 决策树亦将其归入「应用流程需要的技术能力 → application/port/out」。{@code
 * PaymentGateway}/{@code InventoryGateway} 才是合法的 Domain Gateway（账户余额、 库存这类业务规则依赖的外部事实）。
 *
 * <p><b>为何实现在 infrastructure</b>：Outbox 记录只有 PENDING/SENT/FAILED 的<strong>技术状态</strong>，无业务不变量；
 * 基础设施实现（{@code OrderOutboxWriterImpl}）负责把领域事件转换为跨边界集成事件（ACL 职责），领域层不感知协议细节。
 *
