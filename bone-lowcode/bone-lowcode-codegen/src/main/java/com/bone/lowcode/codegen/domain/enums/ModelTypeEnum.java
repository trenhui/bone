package com.bone.lowcode.codegen.domain.enums;
...
public enum ModelTypeEnum {
    ...
    private static String javaModuleFilePath(String path) {
        return "${table.moduleName}/" + // 顶级模块
                "src/main/java/" + ContextUtil.getCurrentPackagePath() + "/" + path + ".java";
    }

    // 构建 Java SDK 模块文件路径
    private static String javaModuleSdkFilePath(String path) {
        return "${table.moduleName}-sdk/" + // 顶级模块
                "src/main/java/" + ContextUtil.getCurrentPackagePath() + "/sdk/" + path + ".java";
    }
}
