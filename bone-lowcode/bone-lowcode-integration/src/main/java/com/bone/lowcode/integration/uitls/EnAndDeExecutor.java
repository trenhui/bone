...
import static com.bone.lowcode.integration.uitls.EncryptionUtils.*;
import static com.bone.lowcode.integration.uitls.RsaUtils.decrypt;
import static com.bone.lowcode.integration.uitls.RsaUtils.encrypt;
       ...
       //使用枚举判断加解密
        ...
        if (enAndDeEnum == EnAndDeEnum.ENCRYPT) {
            if ("RSA".equals(encrypt_type)) {
                ...
                result = encryptWith3DES(content, secret, decryptCode);
            } else if ("AES".equals(encrypt_type)) {  // 添加 AES 加密逻辑
                result = encryptWithAES(content, secret);  // 需要实现 AES 加密方法
            }
            ...
            if ("RSA".equals(decrypt_type)) {
                ...
                result = decryptWith3DES(content, secret, decryptCode);
            } else if ("AES".equals(decrypt_type)) {  // 添加 AES 解密逻辑
                result = decryptWithAES(content, secret);  // 需要实现 AES 解密方法
            }
        }
