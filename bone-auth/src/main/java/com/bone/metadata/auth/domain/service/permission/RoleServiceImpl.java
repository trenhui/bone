...
import com.bone.metadata.auth.application.enums.permission.RoleCodeEnum;
import com.bone.metadata.auth.application.enums.permission.RoleTypeEnum;
import com.bone.metadata.auth.application.vo.role.RolePageReqVO;
import com.bone.metadata.auth.application.vo.role.RoleSaveReqVO;
import com.bone.metadata.auth.domain.entity.permission.RoleDO;
import com.bone.metadata.auth.domain.mapper.permission.RoleMapper;
...
import static com.bone.metadata.auth.application.enums.ErrorCodeConstants.*;
import static com.bone.metadata.auth.application.enums.logger.LogRecordConstants.*;
...
public class RoleServiceImpl implements RoleService {
    ...
    void validateRoleDuplicate(String name, String code, Long id) {
        ...
        if (RoleCodeEnum.isSuperAdmin(code)) {
            throw new BizException(ROLE_ADMIN_CODE_ERROR.getCode(), ROLE_ADMIN_CODE_ERROR.getMsg());
        }
        // 1. 该 name 名字被其它角色所使用
        RoleDO role = roleMapper.selectByName(name);
        if (role != null && !role.getId().equals(id)) {
            throw new BizException(ROLE_NAME_DUPLICATE.getCode(), ROLE_NAME_DUPLICATE.getMsg());
        }
        // 2. 是否存在相同编码的角色
        ...
        if (role != null && !role.getId().equals(id)) {
            throw new BizException(ROLE_CODE_DUPLICATE.getCode(), ROLE_CODE_DUPLICATE.getMsg());
        }
    }
    ...
    RoleDO validateRoleForUpdate(Long id) {
        ...
        if (role == null) {
            throw new BizException(ROLE_NOT_EXISTS.getCode().toString());
        }
        // 内置角色，不允许删除
        if (RoleTypeEnum.SYSTEM.getType().equals(role.getType())) {
            throw new BizException(ROLE_CAN_NOT_UPDATE_SYSTEM_TYPE_ROLE.getCode().toString());
        }
        return role;
    }
...
//            unless = "#result == null")
    ...
    public void validateRoleList(Collection<Long> ids) {
        ...
        ids.forEach(id -> {
            ...
            if (role == null) {
                throw new BizException(ROLE_NOT_EXISTS.getCode(),ROLE_NOT_EXISTS.getMsg());
            }
            if (!CommonStatusEnum.ENABLE.getStatus().equals(role.getStatus())) {
                throw new BizException(ROLE_NOT_EXISTS.getCode(),role.getName());
            }
        });
    }
