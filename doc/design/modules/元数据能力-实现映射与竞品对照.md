# 元数据能力：三模块定义、协作关系与竞品对照

> **文档性质**：`bone-metadata-sdk` / `bone-metadata-server` / `bone-metadata-engine` 及代码生成的**单一真源**（定义 + As-Is 映射 + 行业对标）。  
> **读者**：产品、架构、研发；修订时请同步 [2. 元数据管理模块详细设计方案](./2.%20元数据管理模块详细设计方案.md)、[主 PRD §4.4](../../prd/BONE产品需求文档正式版.md)、[BONE X Studio](../BONE-X-Studio-详细设计方案.md) 文首对照表。  
> **版本**：2026-05-17 · **修订**：§1.1 两类 Field、Generator As-Is 边注、masterdata 边界、generator-app；API §13.2 迁移登记

---

## 1. 产品能力 vs 工程模块（先读此表）

PRD「元数据管理 / 应用生成」是**产品能力包**，不等于某一个 Maven 模块。实现由多模块协作：

| 产品能力（PRD §4.4） | 主要实现 | 状态（As-Is） |
|----------------------|----------|---------------|
| 平台持久化、仓储 CRUD、扩展字段 EAV | **bone-metadata-sdk** | ✅ 已用于 IAM/Masterdata/Blueprint 等 |
| 扩展字段元数据集中 API | **bone-metadata-server** :9001 | ✅ `/v1/metadata/fields:*` |
| 规则 / 表达式 / SmartQL / 动态操作 | **bone-metadata-engine** | ⚠️ 库存在，**未**接入平台单体 |
| 业务实体 `meta_*` 建模与管理 API | 详设 DDL + 未来管理面 | 🔶 表在 `bone-init.sql` §4；REST 多为 **[Vision]** |
| 代码生成与模板 | **studio-generator**、**bone-tool/bone-codegen** | 🔶 As-Is 以**物理库表反向解析**为主；读 `meta_*` 发布快照为 Vision |
| 建模控制台 UI | **bone-metadata-app** :3004 | 🔶 菜单骨架在，页面多为占位 |
| 代码生成控制台 UI | **bone-generator-app** :3009 | 🔶 对接 `studio-generator`（数据源/模板/生成历史） |

**结论**：三模块 `bone-metadata-*` 是「元数据能力族」的核心实现，**不**单独等于 PRD 中的完整「元数据管理模块」。

### 1.1 术语：两类「字段」（必读）

| 术语 | 含义 | API / 表 | 勿混淆 |
|------|------|----------|--------|
| **扩展字段（EAV）** | 运行时给业务表**分配物理列**、扩展属性注册 | `POST /v1/metadata/fields:allocate` 等；sdk `MetadataService` | ≠ 建模 catalog |
| **建模字段（catalog）** | 应用生成用的实体模型字段定义 | 表 `meta_field`（及 `meta_entity`）；规划 REST `/api/metadata/fields` | ≠ server 的 `fields:*` |

详设 API：**§5.0** = 扩展字段（As-Is）；**§5.2** = 建模字段（Vision）。对外表述时避免统称「字段 API 已实现」。

---

## 2. 三模块定义（官方表述）

### 2.1 bone-metadata-sdk — 平台数据面（必选）

| 项 | 说明 |
|----|------|
| **定位** | Bone **默认持久化栈**（P0）：`@EnableSqlRepositories`、`Repository` / `FluentQuery` / `Criteria`、SQL 模板、多数据源；扩展字段 **EAV** 与列分配。 |
| **层级** | **平台基础设施**（与 `bone-datasource`、`bone-core` 同级），不是可单独售卖的「第四个引擎」。 |
| **形态** | Maven `jar`，嵌入各业务 Spring Boot 进程。 |
| **MetadataService** | `EMBEDDED`（本进程 + DB）/ `REMOTE`（Feign → server）。 |
| **不负责** | 业务实体 `meta_*` 全套控制台 API；SmartQL/规则运行时；独立 HTTP 服务。 |

文档：[bone-metadata-sdk/README.md](../../../bone-engine/bone-metadata-sdk/README.md)

### 2.2 bone-metadata-server — 扩展字段控制面（选配）

| 项 | 说明 |
|----|------|
| **定位** | 将 SDK 内 **扩展字段元数据**能力以 REST 集中暴露，供多应用 `REMOTE` 模式共享。 |
| **层级** | **控制面 / 部署单元**（薄壳），不是「智能元数据引擎」本体。 |
| **形态** | Spring Boot 可执行 Jar，默认端口 **9001**。 |
| **已实现 API** | `POST/GET /v1/metadata/fields:search`、`fields:searchByNames`、`fields:allocate`、`health` |
| **不负责** | `meta_entity` 实体建模 CRUD；代码生成；规则引擎。 |

文档：[bone-metadata-server/README.md](../../../bone-engine/bone-metadata-server/README.md)

### 2.3 bone-metadata-engine — 智能元数据引擎（可选接入）

| 项 | 说明 |
|----|------|
| **定位** | README **四大引擎** 之「智能元数据引擎」：动态模型、SpEL 表达式、校验、业务规则、SmartQL、动态数据访问等 **计算面**。 |
| **层级** | **产品引擎**（`core` + `starter`，包 `com.bone.metadata.engine`）。 |
| **形态** | 库 + 自动配置；**默认不参与**平台单体启动。 |
| **与 SDK** | 当前 **无** Maven 依赖；接入后应通过 SPI 读写字段/模型元数据，避免双注册表。 |
| **不负责** | 替代 SDK 做 JDBC 仓储；替代 Generator 输出 DDD 工程源码。 |

文档：[9. SmartMeta 引擎模块技术说明](./9.%20SmartMeta%20引擎模块技术说明.md)

---

## 3. 协作关系

### 3.1 依赖与调用（As-Is）

```mermaid
flowchart TB
  subgraph apps["业务应用（IAM / Masterdata / TPA / Blueprint …）"]
    APP["@EnableSqlRepositories"]
  end

  subgraph sdk["bone-metadata-sdk（数据面）"]
    REPO["Repository 代理 / SqlBuilder"]
    MS["MetadataService"]
    EAV["ExtensionCoordinator / EAV"]
  end

  subgraph srv["bone-metadata-server :9001（选配）"]
    API["/v1/metadata/fields:*"]
  end

  subgraph eng["bone-metadata-engine（选配，未接平台）"]
    ME["MetadataEngine / ExpressionEngine / Rules"]
  end

  subgraph other["应用生成（非 metadata-server 职责）"]
    GEN["studio-generator :8085"]
    GENUI["bone-generator-app :3009"]
    METAUI["bone-metadata-app :3004"]
  end

  APP --> REPO
  REPO --> EAV
  MS --> EAV
  MS -->|REMOTE| API
  API --> MS

  eng -.->|规划：元模型桥接| MS
  METAUI -.->|Vision：meta_* REST| APP
  GEN -.->|As-Is：JDBC 读物理表| REPO
  GEN -.->|Vision：meta_* 发布快照| APP
  GENUI --> GEN
```

### 3.2 部署模式

| 模式 | 配置 | 适用 |
|------|------|------|
| **嵌入式** | `metadata.sdk.deploymentMode=EMBEDDED`（默认） | 单应用内扩展字段，直连 MySQL |
| **远程** | `REMOTE` + Feign 指向 server | 多服务共享扩展字段注册表 |
| **引擎增强** | 宿主引入 `bone-metadata-engine-starter` | 规则/表达式/动态页；与生成代码共存 |

### 3.3 与代码生成的分工（Bone 路线：混合 C）

| 能力 | 元数据引擎（engine） | 代码生成（generator） |
|------|----------------------|------------------------|
| 字段校验公式、业务规则 | ✅ 运行时配置 | ❌ 不替代 |
| 动态表单 / 轻量 CRUD 解释执行 | ✅ 逐步增强 | ❌ |
| DDD 分包、CommandHandler、ArchUnit 门禁 | ❌ | ✅ 生成或 Blueprint 手写 |
| 可交付 Git 仓库、信创审计 | ❌ 纯运行时难满足 | ✅ |
| 扩展点实现、复杂集成 | 🔶 编排 | ✅ 骨架 + 手写 |

**原则**：有引擎后 **仍需要** 代码生成（范围可收窄为「标准骨架 + 列表页」）；对标 Salesforce/Microsoft 的「元数据运行时」而非 Workday「零导出源码」时，才考虑大幅削减生成。

---

## 4. 与 PRD「三大能力 / 四大引擎」映射

| PRD 三大能力 | README 四大引擎 | 工程落点 |
|--------------|-----------------|----------|
| 应用生成 | 智能元数据引擎 | **engine**（智能）+ **generator**（源码）+ **meta_* / UI**（建模） |
| （横切） | — | **sdk** 持久化所有平台模块 |
| 企业集成 / 主数据 | 集成 / 主数据 | 各平台服务；持久化仍走 **sdk** |

---

## 5. API 真源（避免混用路径）

### 5.1 已实现（As-Is）— 扩展字段 EAV only

> 路径**无** `/api` 前缀；与 [Bone-API-规范](../../architecture/Bone-API-规范.md) §13.2 过渡登记一致。均为 **§1.1 扩展字段**，非 `meta_field` 建模 CRUD。

| 方法 | 路径 | 模块 |
|------|------|------|
| POST | `/v1/metadata/fields:search` | server → sdk |
| POST | `/v1/metadata/fields:searchByNames` | 同上 |
| POST | `/v1/metadata/fields:allocate` | 同上 |
| GET | `/v1/metadata/health` | 同上 |

### 5.2 规划（Vision，详设 §5）— 建模 catalog + 生成

| 前缀 | 示例 | 说明 |
|------|------|------|
| `/api/metadata/*` 或 `/api/v1/metadata/*` | `entities`、`/fields`（建模）、`generate`、`templates` | 业务实体与代码生成；**未**在 server 实现；生成主责 **generator** 域 |

OpenAPI 与网关路由以各模块 Controller 及 [Bone-API-规范](../../architecture/Bone-API-规范.md) 为准。

---

## 6. 行业竞品对照（元数据引擎 vs 代码生成）

| 厂商 | 元数据引擎角色 | 是否生成完整应用源码 | 与 Bone 对齐点 |
|------|----------------|----------------------|----------------|
| **Salesforce** | Org 配置 + Metadata API；声明式为主 | 否（Apex/LWC 手写 + DX 源码仓） | server ≈ Metadata API 子集；Bone 额外要 **Generator** |
| **Microsoft Dataverse** | 表/关系元数据；模型驱动 UI | 否；Code Apps 仅生成 **TS 客户端** | sdk ≈ 数据面；engine ≈ Power Fx/插件逻辑 |
| **Workday XO** | 全栈元数据运行时 | 否（Extend 仍在租户内） | engine 方向接近；Bone **开源交付** 保留生成 |
| **Coupa** | Custom Fields / Custom Objects API | 否（外部集成） | server 扩展字段类似；非应用工厂 |
| **OutSystems** | 可视化模型 | **是**（C#/JS/SQL） | 接近 **studio-generator** |
| **Mendix** | 模型即应用（默认解释执行） | 可选 MxSDK 生成 | Bone 选 **生成 + 可选引擎** 混合 |

参考阅读：

- [Salesforce Metadata API](https://developer.salesforce.com/blogs/2022/03/demystifying-the-salesforce-metadata-api)
- [Dataverse tables and metadata](https://learn.microsoft.com/en-us/power-apps/maker/data-platform/create-edit-metadata)
- [Workday Platform Technology (XO)](https://knowledge.workday.com/en-au-ebook/workday-platform-technology/chapter-2)
- [Coupa Custom Objects API](https://docs.coupa.com/en/developer-documentation/the-coupa-core-api/resources/transactional-resources/custom-object-api-custom_objectsid)
- [Low-code: Code Generation vs Metadata](https://blog.ensembleui.com/low-code-platforms-code-generation-vs-metadata/)

---

## 7. 需求实现状态（META-* 速查）

| 需求 ID | 能力 | SDK | Server | Engine | Generator |
|---------|------|-----|--------|--------|-----------|
| META-001 | 业务实体/字段/关系 | 持久化 🔶 | — | — | — |
| META-001 | 扩展字段 EAV | ✅ | ✅ | — | — |
| META-002 | 代码生成（物理表 / 模板） | — | — | — | 🔶 As-Is |
| META-002 | 代码生成（读 meta_* 快照） | 🔶 | — | — | Vision |
| — | 规则/表达式 | — | — | 🔶 | — |

图例：✅ 已用；🔶 部分/规划；— 非主责。

---

## 8. 与 bone-masterdata 的边界

| 维度 | 元数据管理（应用生成 / catalog） | 企业主数据（bone-masterdata） |
|------|----------------------------------|-------------------------------|
| **产品目标** | 建模、发布、生成 DDD 工程 | 主数据实体治理、质量规则、数据记录 |
| **表 / 域** | `meta_*`（init §4） | 主数据域表与 API（如 `MasterDataEntityController`） |
| **As-Is** | `meta_*` 有 DDL；REST/UI 未闭环 | 主数据 CRUD 已部分落地 |
| **PRD 衔接** | META-001 建模；META-001「转主数据」为 **跨模块** 流程 | §4.5 主数据管理 |

**务实口径**：短期勿假定 `metadata-server` 已提供实体 CRUD；「业务实体 → 主数据实体」需在 **管理面 API + masterdata** 间用 ADR 定义同步方式（复制定义、发布事件等），避免 `meta_*` 与主数据双写无规范。

---

## 9. 文档维护 checklist

修订三模块代码或 API 时，请检查：

- [ ] 本文 §2 定义、§1.1 两类 Field、§8 masterdata 边界是否仍准确  
- [ ] Generator 边注（§3.1）：As-Is 物理表 vs Vision `meta_*`  
- [ ] [2. 元数据管理模块详细设计方案](./2.%20元数据管理模块详细设计方案.md) 文首 As-Is 链接  
- [ ] [Bone-API-规范](../../architecture/Bone-API-规范.md) §13.2 `/v1/metadata` 迁移行  
- [ ] [modules/README](./README.md) 索引表  
- [ ] [主 PRD §4.4](../../prd/BONE产品需求文档正式版.md) 实现映射小节  
- [ ] [bone-engine/README.md](../../../bone-engine/README.md) 模块表  
- [ ] [wiki/03](../../wiki/03-本地开发与构建.md) 端口  
