...
详设 API：**§5.0** = 扩展字段（As-Is）；**§5.1～5.3** = 建模 catalog（**As-Is REST 已落地**；版本历史等为 P1）。对外表述时区分扩展字段与 catalog 路径。
...
| **As-Is** | ✅ 主线（catalog + generator + sdk 已落地） | ✅ **CRUD v1**（`/api/v1/runtime/**` @ :9001，engine `JdbcRuntimeRecordService` + server `CatalogRuntimeEntityProvider`）；⏳ 列裁剪/过滤/排序、`If-Match`；💡 SmartQL/规则/投影查询。详见 [元数据详设 §0.1 / §2.4](./2.%20元数据管理模块详细设计方案.md) |
...
| **As-Is** | `meta_*` + catalog REST/UI **MVP-2 基线已验收** | 主数据 CRUD 已部分落地 |
