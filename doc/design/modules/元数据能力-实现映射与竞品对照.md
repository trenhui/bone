...
| **As-Is** | ✅ 主线（catalog + generator + sdk 已落地） | 🔶 **动态 CRUD 基线**在 `bone-metadata-server`（`/api/v1/runtime/**`，engine 提供 `JdbcRuntimeRecordService`）；SmartQL/规则仍分阶段 |
...
> 与 [Bone-API-规范](../../architecture/Bone-API-规范.md) 一致，使用 **`/api/v1/metadata/**`** 前缀。下表为 **§1.1 扩展字段 EAV**，非 `meta_field` 建模 catalog CRUD。
...
### 5.2 建模 catalog（**As-Is**）与代码生成

| 前缀 | 示例 | 说明 |
|------|------|------|
| `/api/v1/metadata` | `entities`、`entities/{id}/fields`、`relationships` | 业务实体 catalog（**As-Is** REST）；版本历史等为 P1 |
| `/api/v1/runtime` | `entities/{entityCode}/records` | 模式 B 动态 CRUD（**As-Is** · server + engine） |
| `/api/v1/generator/**` | `code-generation`、`generation-tasks`、模板等 | 主责 **studio-generator**（`GeneratorApiPaths`）；非 server 内嵌 |
