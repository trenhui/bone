...
> **版本**：2026-05-20 · **本轮修订**：§1.3 As-Is（runtime 基线已在 server）、§5 真源、§7 模块列订正、§10 与 §8 一致  
> **产品理念**：[README.md](../../../README.md) · 与 [BONE-总体架构](../../architecture/BONE-总体架构设计方案.md) §2.1.1、§3.1 对齐  
> **HTTP 真源**：OpenAPI [metadata-runtime-v1.yaml](../../architecture/openapi/metadata-runtime-v1.yaml) · 网关 `bone-platform/bone-gateway/src/main/resources/application.yml`（`/api/v1/metadata/**`、`/api/v1/runtime/**` → `:9001`）。  
...
|----------------------|----------|---------------|
| 平台持久化、仓储 CRUD、扩展字段（预留列/JSON/EAV） | **bone-metadata-sdk** | ✅ 已用于 IAM/Masterdata/Blueprint 等 |
| 扩展字段元数据集中 API | **bone-metadata-server** :9001 | ✅ `/api/v1/metadata/fields:*` |
| 规则 / 表达式 / SmartQL / 动态操作 | **bone-metadata-engine** | 🔶 库 + **server** 已暴露 `/api/v1/runtime/**` 基线；规则/SmartQL 仍分阶段 |
...
| 术语 | 含义 | API / 表 | 勿混淆 |
|------|------|----------|--------|
| **扩展字段** | 运行时给业务表的动态属性：SDK 支持三种存储模式——**预留列**（默认，分配物理列 + 回收复用）、**JSON 列**（半结构化）、**EAV 键值对**（极低频，非推荐首选） | `POST /api/v1/metadata/fields:allocate` 等；sdk `ExtensionCoordinator` / `MetadataService` | ≠ 建模 catalog |
...
详设 API：**§5.0** = 扩展字段（As-Is）；**§5.1～5.3** = 建模 catalog（**As-Is REST 已实现**；版本历史等为 P1）。对外表述时区分扩展字段与 catalog 路径。

### 1.3 双模式交付（产品定位 · 必读）

智能元数据引擎对外提供 **两种可并存、按场景选型** 的交付路径（通常称为 *Code Generation* vs *Metadata Runtime*，参见 [Ensemble UI 对照文](https://blog.ensembleui.com/low-code-platforms-code-generation-vs-metadata/)）。
...
| **同类产品参照** | OutSystems、JHipster、Salesforce DX | Salesforce 声明式标准对象、Mendix 默认运行时、Directus / Hasura |
| **As-Is** | ✅ 主线（catalog + generator + sdk 已实现） | ✅ **CRUD v1**（`/api/v1/runtime/**` @ :9001，engine `JdbcRuntimeRecordService` + server `CatalogRuntimeEntityProvider`）；⏳ 列裁剪/过滤/排序、`If-Match`；💡 SmartQL/规则/投影查询。详见 [元数据详设 §0.1 / §2.4](./2.%20元数据管理模块详细设计方案.md) |
| **演进** | 生成器收窄为「骨架 + 逃逸舱」 | 引擎成熟后 **可替代** 模式 A 中的 **标准 CRUD 生成**；不排斥模式 A 用于合规与深度定制 |

```text
         meta_* catalog（server）+ 扩展字段（sdk）
                │
    ┌───────────┴───────────┐
    ▼                       ▼
模式 A                  模式 B
...
### 1.2 API 路径约定（避免与扩展字段 API 冲突）

| 能力 | 前缀 / 形态 | 示例 | 说明 |
|------|-------------|------|------|
| 扩展字段（As-Is，动作式） | `/api/v1/metadata` | `POST …/fields:search`、`…/fields:allocate` | **动作式**资源名 + 冒号动作 |
| 建模 catalog（As-Is） | `/api/v1/metadata` | `GET/POST …/entities`、`…/entities/{id}/fields`、`…/relationships` | **REST 资源**；catalog 字段**必须**挂在实体下，禁止与扩展字段共用顶层 `…/fields` |
| 代码生成 | `/api/v1/generator/**`（`GeneratorApiPaths`） | 见 studio-generator 详设 | **非** metadata-server 主责 |
...
| 项 | 说明 |
|----|------|
| **定位** | Bone **默认持久化栈**（P0）：`@EnableSqlRepositories`、`Repository` / `FluentQuery` / `Criteria`、SQL 模板、多数据源；扩展字段存储三模式（**预留列默认**/JSON/EAV）与列分配。 |
...
| **As-Is 不负责** | 代码生成、规则引擎。 |
| **catalog（As-Is）** | 同进程 **catalog** 包：`meta_*` 实体/字段/关系 REST（`/api/v1/metadata/*`，与扩展字段路径分离，见 §1.2）。 |
...
| **不负责** | 替代 `bone-metadata-server` 的 catalog 管理 UI/API；替代扩展字段 `fields:allocate` 注册（仍走 server/sdk）。 |
...
flowchart TB
  ...
  subgraph sdk["bone-metadata-sdk（数据面）"]
    ...
    MS["MetadataService"]
    EXT["ExtensionCoordinator（预留列/JSON/EAV）"]
  end
  ...
  subgraph eng["bone-metadata-engine（库；由 server 装配 runtime）"]
    ME["MetadataEngine / ExpressionEngine / Rules"]
  end

  subgraph other["应用生成（非 metadata-server 职责）"]
    GEN["studio-generator :8086"]
...
| 扩展字段读写（预留列/JSON/EAV） | 🔶 经 sdk；与 catalog 正交 | — |
...
1. **短期**：模式 A 为平台主线；engine 与 sdk 建立元模型桥接。  
...
> 与 [Bone-API-规范](../../architecture/Bone-API-规范.md) 一致，使用 **`/api/v1/metadata/**`** 前缀。下表为 **§1.1 扩展字段**能力（存储模式预留列/JSON/EAV 由 sdk 路由，不含 §1.2 建模 catalog 与 §1.3 runtime CRUD，那两者见 §5.2）。

| 方法 | 路径 | 权限 scope（✅ @PreAuthorize） | 模块 |
|------|------|--------------------------------|------|
| POST | `/api/v1/metadata/fields:search` | `metadata:read` | server → sdk |
| POST | `/api/v1/metadata/fields:searchByNames` | `metadata:read` | 同上 |
| POST | `/api/v1/metadata/fields:allocate` | `metadata:write`（返回 `201`） | 同上 |
| GET | `/api/v1/metadata/health` | 公开 | 同上 |

### 5.2 建模 catalog + runtime CRUD（**As-Is**）与代码生成

> 详设见 [§5.1–§5.3.1 / §5.3.2 checklist](./2.%20元数据管理模块详细设计方案.md)；权限/状态码以详设 §5.0 + §5.A 为真源。

| 前缀 | 示例 | 权限（✅ As-Is） | 说明 |
|------|------|------------------|------|
| `/api/v1/metadata` | `entities`、`entities/{id}/fields`、`relationships` | `metadata:read|write` + 创建端 `201 + Location` | 业务实体 catalog；版本历史等为 P1 |
| `/api/v1/runtime` | `entities/{entityCode}/records` | `metadata:read|write` + 创建端 `201 + Location` | 模式 B 动态 CRUD（server + engine） |
| `/api/v1/generator/**` | `code-generation`、`generation-tasks`、模板等 | 由 generator 自管（独立服务） | 主责 **studio-generator**（`GeneratorApiPaths`）；非 server 内嵌 |
...
| **Coupa** | Custom Fields API | 否 | server 扩展字段 API 类似 |
...
| META-001 | 扩展字段（预留列/JSON/EAV） | ✅ | ✅ | — | — |
| META-002 | 代码生成（物理表 / 模板） | — | — | — | 🔶 As-Is |
| META-002 | 代码生成（读 meta_* 快照） | ✅ | — | — | `metadataSource=CATALOG_SNAPSHOT` |
| META-002B | 运行时动态 CRUD（`delivery_mode=RUNTIME`） | ✅ 列元数据 | REST + 装配（`RuntimeRecordController`） | ✅ `JdbcRuntimeRecordService` + 单测 | 跳过 RUNTIME 实体代码生成 |
| — | 规则/表达式 | — | — | 🔶 | — |
...
| **As-Is** | `meta_*` + catalog REST/UI **MVP-2 基线已验收** | 主数据 CRUD 已部分实现 |
| **PRD 衔接** | META-001 建模；「转主数据」为 **跨模块** 流程 | §4.5 主数据管理 |

**务实口径**：「业务实体 → 主数据实体」须在 **catalog API + bone-masterdata** 间用 ADR 定义同步方式（复制定义、发布事件等），避免 `meta_*` 与 `mdm_*` 双写无规范；高级治理与版本历史仍分阶段。
...
| 阶段 | 范围 | 验收要点 | 状态 |
|------|------|----------|------|
| **MVP-1** | `bone-metadata-sdk` + `bone-metadata-server` **扩展字段 API**；IAM；Shell/微应用骨架；Helm | `fields:*` 可调通；平台模块持久化走 SDK | ✅ 扩展字段链路 |
