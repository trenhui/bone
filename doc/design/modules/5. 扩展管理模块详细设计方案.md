...
| 控制台微应用 | ✓（主流程） | `bone-extension-app`：概览/扩展点/插件/观测页；`extensionApi` + Vitest | 未覆盖 `:bind`/`:unbind` UI（API As-Is 已有） |
| Wasm 沙箱 | [Vision] | 无 | — |
| 插件市场 | [Vision] | 无 | — |
...
| **前端**（`bone-extension-app`） | **As-Is 主流程已完成**（扩展点/插件 CRUD、上传、LRO 部署、回滚、运行时发布、观测页）；`:bind`/`:unbind` 仅 API、无专用 UI |
...
0. **[Target]** 控制台补齐 `:bind`/`:unbind` 与制品下载 UI（后端 API 已具备）
