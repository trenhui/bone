...
| 组件 | 职责 | 模块 |
|------|------|------|
| `ExtensionManagementController` | 统一 REST（仅注入 Handler，**禁止**直注 `*Service`） | bone-extension-studio |
| `CreateExtPointCommandHandler` / `DeployExtensionCommandHandler` 等 | 命令处理器（写事务边界） | studio |
