...
| **[Target]** | 目标态，须在 OpenAPI / 代码实现前标注迁移期 |
...
| 未实现 Backlog | [附录 A](#附录-a-target--vision-backlog未实现)、[`backlog.yaml`](../../../tools/extension-compliance-collector/backlog.yaml) |
...
> **姊妹篇**：[`5a. 扩展管理模块核心场景及用例设计方案.md`](./5a.%20扩展管理模块核心场景及用例设计方案.md) —— 扩展与租户/应用的三层归属模型（宿主载体 / 责任归口 `app_id` / 路由维度）、治理角色与权限码全集、核心用例（UC-EX-P/D/T/G）与端到端串联流程、割裂修补清单 G1–G8（G1 应用归属、G2 租户码校验为 P0）、待裁决清单。本文（详设）保持技术真源（数据模型/API/安全/门禁），用例与角色治理演进走姊妹篇。
...
**[Target]**：插件依赖图、部署状态机、Redis 集群幂等（见 [附录 A](#附录-a-target--vision-backlog未实现)）。  
...
> As-Is 机器证据（OpenAPI/源码扫描）见 [附录 C](#附录-c-as-is-证据ci-派生)；未实现项见 [附录 A](#附录-a-target--vision-backlog未实现)。
...
| 控制台微应用 | ✓ | `bone-extension-app`：概览/扩展点/插件/依赖图/插件市场/观测页 + 「更多」菜单含 `:bind`/`:unbind` 弹窗（未绑定插件部署按钮自动禁用并提示）；`extensionApi` + Vitest（含 `bindPlugin` / `unbindPlugin` / `getDeploymentState` / `getDependencyGraph` / `installMarketplaceItem` 单测） | — |
| 部署状态机可视化 | ✓ | `GET /plugins/{id}/deployment-state` + `DeploymentStateDiagram`（详见 §3.3，含 UPLOADED→…→DEPRECATED 全态迁移图） | `DeploymentStateMachineTest` |
| 依赖图 | ✓ | `GET /dependency-graph` + `PluginDependencyGraphQueryHandler`，前端 SVG 可视化（节点=插件，边=`config_json.dependencies`） | 单测覆盖（含未解析依赖判定） |
| 插件市场（v1） | ✓ | `GET /marketplace`、`POST /marketplace/{itemId}:install`（目录由 `classpath:marketplace/items.json` 提供） | `JsonResourceMarketplaceCatalogTest` |
| Wasm 沙箱 | [Vision] | 无 | — |
| 插件市场（OCI / 远端仓库） | [Vision] | 当前仅静态 JSON 目录 | — |
...
同类产品对标：Salesforce/Fusion **Extensibility SPI**、SAP **BAdI**、JDK **ServiceLoader** 的企业版路由。
...
3. **模糊匹配**：维度规则含通配 `*`
4. **默认实现**：`is_default = true`（存于 `config_json.defaultImpl`）
...
**[Target]**（参考 PF4J / Equinox 的类加载隔离做法）：
...
**ExtPoint 与 Plugin 分层红线（EXT-PL-01）**：扩展点（ExtPoint）与插件运行时（Plugin Runtime）是两个层级，不得混为一谈：

| 层级 | 本质 | 范围 |
|------|------|------|
| **ExtPoint** | 应用内扩展契约 | SPI、Registry、Routing、Priority、Condition、Version |
| **Plugin Runtime** | 模块化运行时 | Artifact、Lifecycle、Isolation、Sandbox、Tracing |

第一阶段**只实现 ExtPoint**；Plugin Runtime（独立 ClassLoader、依赖隔离、Sandbox）必须作为**独立方案**单独设计，不得在 Extension Engine 内自然生长为 OSGi / Kubernetes 级运行时。上述 [Target] / [Vision] 隔离能力仅作为 Plugin Runtime 独立方案推进，不并入 ExtPoint 核心。
...
| [`ExtPointCommandApplicationService`](../../../bone-engine/bone-extension-engine/bone-extension-studio/src/main/java/com/bone/engine/extension/studio/application/ExtPointCommandApplicationService.java) | 扩展点写侧（含 `:enable`/`:disable`/`delete` 编排） | studio |
| [`ExtensionCommandApplicationService`](../../../bone-engine/bone-extension-engine/bone-extension-studio/src/main/java/com/bone/engine/extension/studio/application/ExtensionCommandApplicationService.java) | 扩展实现写侧（`deployExtension`/`undeployExtension`/`publishRuntime` 等） | studio |
| [`ExtensionStudioApplicationService`](../../../bone-engine/bone-extension-engine/bone-extension-studio/src/main/java/com/bone/engine/extension/studio/application/ExtensionStudioApplicationService.java) | 控制台门面（`deployPlugin` LRO/同步分流、`undeployPlugin`、`deletePlugin` 等） | studio |
| [`StudioIdempotencySupport`](../../../bone-engine/bone-extension-engine/bone-extension-studio/src/main/java/com/bone/engine/extension/studio/application/support/StudioIdempotencySupport.java) / [`StudioIdempotentExecutor`](../../../bone-engine/bone-extension-engine/bone-extension-studio/src/main/java/com/bone/engine/extension/studio/application/support/StudioIdempotentExecutor.java) | 幂等键存取与冲突 409 | studio support（**仅 ApplicationService 内部调用**） |
| [`StudioLroSupport`](../../../bone-engine/bone-extension-engine/bone-extension-studio/src/main/java/com/bone/engine/extension/studio/application/support/StudioLroSupport.java) | 长任务 operationId 注册与状态查询 | studio support |
...
**模块性质**（真源：[Bone-DDD E-5.4](../../architecture/Bone-DDD-最终实践方案.md)）：`bone-extension-studio` 为**应用 / 控制面 BFF**（虽位于 `bone-engine/`），须满足 DDD P0；`bone-extension-sdk` 为**引擎 SDK**（仅 D0/D1）。
...
**新代码**：REST 仅落 `adapter/web/controller`；不新增 `application/usecase` 或 `domain/store`；业务异常归入项目异常根并使用稳定错误码，允许有业务意义的语义子类；SDK 插件侧异常继承 `com.bone.engine.extension.api.exception.ExtensionBizException`。
...
| `exts_extension_impl` | `is_default` | 兜底实现标记（存于 `config_json.defaultImpl`） | As-Is |
| `exts_extension_impl` | `config_json.traffic` | **灰度比例（0-100）**，SDK 实际使用 | As-Is |
...
> **平台实现检查单**（[API 规范 §8.1](../../architecture/Bone-API-规范.md#81-模块横切约定索引详设-50) 索引到本节）。各字段语义见 [API 规范 §6.1/§7/§8/§14](../../architecture/Bone-API-规范.md)。
...
| `/points/{id}` | PATCH | 200（412 If-Match 失败） | 部分更新 | write |
...
| `/plugins/{id}` | PATCH | 200（412 If-Match 失败） | 部分更新 | write |
| `/plugins/{id}` | DELETE | **204 无 body** | 删除 | write |
| `/plugins/{id}/versions` | GET | 200 | 版本列表 | read |
| `/plugins/{id}/versions/{ver}:download` | GET | 200（`application/octet-stream`，逻辑路径） | 制品下载（`file_path` 存相对路径） | deploy |
...
> 当前 SDK / Studio **未注册** Micrometer 指标；`extension_*` 计数器属规划，实现后须遵循 [Bone-可观测性规范.md](../../architecture/Bone-可观测性规范.md) 命名与基数约束。
...
**演进重点**（完整清单见 [附录 A](#附录-a-target--vision-backlog未实现)）：

0. **[Target]** 控制台补齐制品下载入口（待后端 `:download` 鉴权 URL 实现后再封装 UI）
...
> **已实现项不得写入 backlog**；验收见 [附录 C](#附录-c-as-is-证据ci-派生) 与 §2 符合度矩阵。

| Tier | ID | 项 | 引用 | 跟踪 |
|------|-----|-----|------|------|
| **Target** | `bytecode-hotload-classloader` | 字节码热载 / 独立 ClassLoader 隔离 | 详设 §4.4 | As-Is 仅路由元数据热更；JAR 归档不加载字节码 |
| **Target** | `resilience4j-circuit` | Resilience4j 按插件熔断 | 详设 §4.4 | 已按插件实现 JDK 舱壁 + extension_bulkhead_rejected_total；熔断器 / CircuitBreaker 仍待 Resilience4j |
...
| **Vision** | `plugin-marketplace-remote` | 插件市场（OCI / 远端仓库） | 详设 §12.2 | v1 已提供静态 JSON 目录（marketplace/items.json）；远端 OCI / 凭证 / 评分仍待 |
...
| `patch-partial-update` | PATCH 部分更新（points/plugins） | 源码命中 |
| `deployment-status-ddl` | deployment_status DDL + 状态机落库 | 源码命中 |
| `mime-magic-number` | JAR magic-number 校验 | 源码命中 |
| `artifact-download-auth-url` | 制品鉴权下载 GET :download | 源码命中 |
| `rollout-percent-traffic-merge` | rollout_percent ↔ config_json.traffic 收敛（ADR-0014） | 源码命中 |
| `red-metrics-micrometer` | RED 指标 extension_invoke_* / deploy / LRO | 源码命中 |
| `idempotency-redis-cluster` | Idempotency Redis（可配置 backend=redis） | 源码命中 |
| `per-plugin-bulkhead` | 按插件舱壁 + extension_bulkhead_rejected_total | 源码命中 |
| `deployment-state-machine-api` | 部署状态机视图 GET /plugins/{id}/deployment-state | 源码命中 |
| `plugin-dependency-graph-api` | 插件依赖图 GET /dependency-graph | 源码命中 |
| `plugin-marketplace-v1` | 插件市场 v1（静态 JSON 目录 + :install） | 源码命中 |
...
- **Docs-as-Code 三定律实现**：As-Is 证据改由 `tools/extension-compliance-collector` 从 OpenAPI/源码派生；附录 A 仅保留未实现 Backlog（`backlog.yaml`）；新增附录 C
- **§1.2**：移除「v2.3 文档收紧」历史段落，横切 API 收敛为 As-Is 一句 + 指向 §7.1 / 附录 C
- **§5.3**：核心组件表增加仓库源码相对链接；补充 `StudioWebExceptionHandler`、`ArchitectureTest`
- **§13**：演进重点删除已实现的「DELETE 204」
...
- **§7.1 横切约定**：扩展为 **API 规范实现检查单**（对齐 [API 规范 §6/§7/§8/§14](../../architecture/Bone-API-规范.md) 全部要求）：时间格式、字符串枚举、`X-Request-Id` 回显、DELETE 204、PATCH、`Idempotency-Key` 冲突 409、`Sunset`/`Deprecation`
