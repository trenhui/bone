...
└── bone-extension-studio/       # 应用 BFF — Batch D 迁移中
    ├── controller/              # REST（As-Is 顶层；[Target] → adapter/web/controller）
    ├── service/                 # 应用编排（[Target] → application/command|query）
    ├── domain/model/
    ├── domain/store/            # 端口（[Target] → domain/repository，§14.5）
    ├── domain/repository/       # 部分已存在，与 store 并存
    ├── infrastructure/persistence/
    ├── security/、sync/、config/
```

**[Target]**（DDD v3.6 Batch D，2026-11-30）：`adapter/web` + `application/command|query` + `domain/repository`；引入 `ArchitectureTest` + `bone-architecture-test`（**禁止新增** `*UseCase` / `domain/store`）。

**新代码**：在 As-Is 树内改动时，优先向 Target 包落点；不得新增 `application/usecase`。
