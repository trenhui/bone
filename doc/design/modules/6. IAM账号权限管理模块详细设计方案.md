...
**HTTP 真源**：Controller `bone-platform/bone-iam/.../adapter/web/controller/{Auth,Account,Role,Permission,Audit}Controller.java` · 路径常量 `PlatformApiPaths.IAM_V1`。  
...
| ID | 标题 | 状态 | 关联章节 |
|----|------|------|----------|
| ADR-IAM-001 | 默认 RBAC + 权限码字符串作为 JWT authorities | **Accepted** | §3.5.1 |
| ADR-IAM-002 | 五层权限模型（Owner/Deny/ABAC/ReBAC/RBAC/Default）分阶段交付 | **Proposed** | §3.5.3、架构 §10.2 |
| ADR-IAM-003 | 进程内 PolicyEvaluator → 独立 authz-service 演进 | **Proposed** | §3.5.2 |
| ADR-IAM-004 | JWT 签名算法 HS256 → RS256/EdDSA + JWKS 轮换 | **Proposed** | §7.2.2 |
| ADR-IAM-005 | 多租户隔离三模式（行级 / Schema / 库）切换策略 | **Proposed** | §4.5、§2.5 |
| ADR-IAM-006 | 平台审计总线（全平台 AOP + WORM）落地路径 | **Proposed** | §3.6、§7.3 |
| ADR-IAM-007 | 密码哈希 BCrypt → Argon2id 迁移 | **Proposed** | §7.2.1 |
| ADR-IAM-008 | `iam_audit_log` 扩列 vs 平台审计总线职责划分 | **Proposed** | §4.4 |
| ADR-IAM-009 | 账号直连权限（`iam_account_permission`）是否纳入 MVP | **Proposed** | §1.4、PRD IAM-003 |
