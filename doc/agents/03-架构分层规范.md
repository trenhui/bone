...
>
> **入站适配器的命名与协议标记**（协议子包、`*Controller` 必须在 `controller/` 下、类名何时才带协议标记、各协议自持 DTO）见 [DDD 规范 E-13.5](../architecture/Bone-DDD-最终实践方案.md#e-135-入站适配器命名与协议标记)。存量 IAM / masterdata / system 的 `adapter/web/converter/` 属待收敛（新增一律 `assembler/`）。
