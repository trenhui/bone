# BONE 产品需求文档（PRD）v2.0

## —— 企业级全栈开源快速开发平台（基于已实现 Bone Metadata SDK 的优化版）

> **文档性质**：正式产品需求文档，可直接指导研发排期、测试用例编写  
> **适用产品**：BONE企业级全栈开源快速开发平台  
> **对标标准**：业界最佳实践、OutSystems / Mendix 元数据驱动架构、Apifox 数据建模  
> **版本**：v2.0 Final | **发布日期**：2026‑04‑19 | **文档状态**：✅ 已发布（优化版） | **密级**：内部机密  
...
| **v2.0** | **2026-04-19** | **产品+架构组** | **技术委** | **优化版**：增加多租户、K8s部署、审计日志WORM存储、工期调整、质量门禁、开源社区策略、自研SDK优化指南、统一实体模型 | **Final** |
...
| **系统错误** | system | tenant_id, error_code, error_message, service, timestamp | 每次错误 |
...
- **降级方案**：提供 MyBatis Plus 兼容层，在 SDK 出现严重性能问题时可快速切换。
...
| R006 | 自研 Bone Metadata SDK 性能或稳定性不足 | 高 | 中 | 持续性能压测，增加监控指标，提供降级方案（MyBatis Plus 兼容层） |
...
- **核心框架**：AGPL v3.0（保证开源传染性，防止云厂商白嫖）。
- **商业插件**：专有许可证，需购买商业版授权。
- **SDK 及示例代码**：MIT 许可证。

### 11.2 代码仓库与治理

- GitHub 组织：`bone-platform`
- 仓库划分：
  - `bone-core`：核心引擎（AGPL）
  - `bone-sdk`：Metadata SDK（MIT）
  - `bone-ui`：管理前端（AGPL）
  - `bone-charts`：Helm Chart（Apache 2.0）
  - `bone-plugins`：官方插件（专有，仅二进制）
- 治理模式：BDFL（产品经理最终决策）+ 技术委员会（核心贡献者）。
...
- 官网：https://bone.dev（示例）
- 文档：https://docs.bone.dev，使用 Docusaurus，支持中英文。
- 示例应用：电商订单系统、CRM 系统，提供完整代码。
...
| 文档名称 | 版本 | 用途 |
|----------|------|------|
| BONE技术架构设计方案 | v2.0 | 技术架构参考 |
| BONE详细设计方案 | v2.0 | 详细设计参考 |
| Bone Metadata SDK 开发指南 | 1.0 | 自研 SDK 使用说明 |
| Kubernetes 部署最佳实践 | - | 生产环境部署 |
| 开源社区治理指南 | - | 社区运营参考 |
