...
> **版本**：5.5.17（domain 分组形态统一批次：① 按 [ADR-0036](./adr/0036-domain-model-package-single-standard.md) 把 domain 分组从「两种合法形态」收敛为**唯一形态 `domain/model/{聚合}/`**——聚合根 / 聚合内实体 / 值对象在 `model/` 内的聚合包中平铺，`event/` `projection/` 为聚合内子包，而 `repository` / `gateway` / `service` 及模块自有的领域端口子包（如 `extension/`）留在 `domain/` 根；**禁止** `model/aggregate|entity|valueobject|event` 角色桶（与 E-10.3「非角色子目录」及 ADR-0030 的 `domain/model/<聚合>/projection` 落点冲突）；② 消除记法歧义：原记法 `domain/model/{aggregate|entity|valueobject|event}` 实测被读成三种意思（`model/` 平铺类桶、`model/{聚合}` 子构件、扁平不用 `model/`），bone-masterdata 与 bone-integration 已落到「一个 domain 两套分组并存」；③ E-10 参考图 / 双形态条款 / E-10.3 样例 / E-4.1 与 E-13.1 的投影落点、`doc/agents/03`，以及 blueprint / iam / system / masterdata / integration / metadata-server / extension-studio 的模块 README 登记同批补齐；④ D1 形态与 D2 值对象子包名 `valueobject` 经架构师同日确认；存量按 [E-0.2](#e-02-存量不符合规范代码的处理) 触达即收敛，全平台一次性统一由 ADR-0036 D4 走受控批量收敛通道授权——**代码迁移由架构师手工执行**，本版本只落规范与登记；2026-09-22）
> **上一版 5.5.16**（应用层只保留 ApplicationService 批次：① **废止 `application/service/` 子包**——其 `*Service` 编排器与 `*ApplicationService` 同层竞争「用例入口」身份，制造职责归属模糊与认知负担，[ADR-0033](./adr/0033-application-collaboration-service.md) 即日起撤销（superseded）；② IAM 该包下三个类按职责归位：`AuditService` 落库逻辑并入 `AuditApplicationService`、`AuthService` 的登录定位/密码校验内联进 `AuthApplicationService`（`AccountApplicationService` 查重改直调 `AccountRepository.findByUsernameInTenant`）、`RoleHierarchyResolver` 作为独立 `*Resolver` 助手移至 `application/` 根（受 bone-iam 模块级规则 `domainCoreShouldOnlyDependOnAllowedPackages` 约束——**该规则不在共享规则库，待提升**；带 `@Service` 的领域类不被允许，故不进 `domain/service`）；③ E-10.2 / E-13.2 / E-13.4 与「ApplicationService 平铺」小节同步改写，删去「应用层协作服务」第三类构件；2026-09-22）
...
> **同批追加（5.5.13c）**：⑧ blueprint 支付读侧按 ADR-0030 §P4 同模式折叠——删除 `application/query/port/PaymentQueryPort` 与 `infrastructure/query/PaymentQueryAdapter`，两个全租户运维扫描方法并入 `domain/repository/PaymentRepository`（Criteria 通道 + `disableTenantFilter()`，方法名后缀 `AllTenants`），行投影迁 `domain/payment/projection`，`@EnableSqlRepositories` 同步摘除 `infrastructure.query`；该模块自此**无任何 `*QueryPort`**。⑨ 出站实现包落点统一（E-13.3 补条）——**按端口来源分，不按实现性质分**：`MockPaymentSignaturePortAdapter` 实现的是 `application/port/out` 技术端口，由 `infrastructure/gateway/payment` 归位 `infrastructure/signature`；`gateway/mock` 撤销，`MockInventoryGatewayAdapter` 落 `infrastructure/gateway/inventory`。⑩ 新增门禁 `schedule_only_calls_all_tenants_repository_methods`（blueprint 模块级，`adapter.schedule` 调域仓储只许 `*AllTenants` 结尾——域仓储合并读写后自带 `save` / `update` / `delete`，仅按包豁免不足），`adapter_no_domain_repository_all_packages` 的豁免由类名名单放宽为 `..adapter.schedule..` 整包。⑪ `OrderPriceCalculator` 内嵌的 `public static class OrderPriceRequest` 抽为独立 record `domain/extension/order/OrderPriceRequest`（该内嵌类是全仓最后一处「domain 接口内嵌可变静态类」）。
...
默认使用**语义化 ApplicationService** 作为入站边界（[ADR-0028](./adr/0028-application-service-first-selective-cqrs.md)）。复杂度升级时才引入 `Command` / `Handler`：写意图需显式契约或多入口时加 `Command`，命令异步/跨事务时 `CommandHandler` 落在 Outbox 消费端/任务端，读模型与聚合分歧时 ApplicationService 内升级 QueryPort（复杂读走 QueryPort；**仅当读侧需要独立路由/生命周期时才进 `QueryHandler`**）。判据见 [E-3.7](#e-37-入口构件决策)。

> **落地现状**：`*ApplicationService` 是**目标形态**。blueprint 样板已完整收敛——`OrderApplicationService` 和 `PaymentApplicationService` 统一承载全部读写用例（含两段式事务、远程渠道调用、验签幂等、乐观锁冲突处理），原 Handler 全部内联消除，适配器零 Handler 依赖；bone-iam 已按 [ADR-0032](./adr/0032-controlled-batch-convergence.md) 的受控批量收敛通道完成同形态收敛；其余 bone-platform 模块的存量应用层仍有 `*CommandHandler` / `*QueryHandler` 承载，按 E-0.2 的迁移节奏逐步收敛。**本节不登记迁移计数**：正文只写判据，实时进度用 `python3 scripts/check-ddd-gate-state.py --metrics` 现算（G-1.8）——写进正文的数字在下一次重构后就会变成错的（本版本即因此删掉一处）。所以这里的 ApplicationService 路径是**新代码基准**：评审不要把"存量 Handler 形态"判成违规；反过来，新增模块也不能拿"存量都这么写"当理由继续堆 Handler。
...
- **迁移纪律（不设数字 KPI，只设触达规则）**：新模块默认从 ApplicationService 起步（AS-01：默认入口而非唯一入口），**不因目录模板预生成** `*CommandHandler` / `*QueryHandler`（确需时按 AS-03 的结构性触发显式创建）；存量 Handler 一律**触达即收敛**——本次改动触及某用例时，若同义 ApplicationService 已存在就把 Handler 内联进去，没触及就不动。**明确不设"每季度净降 N 个"式指标**：阈值型 KPI 会诱发"为达标而合并 / 改名"的仪式化改造，正是 E-3.2 要禁的东西。[G-1.8](#g-18-实施状态指标) 的计数只用于观察趋势与暴露长期悬置，不构成任务分解或考核依据。
- **受控批量收敛通道（E-3.11 第 3 条的例外通道）**：默认仍是"触达即收敛"，一次性批量收敛只在**同时满足五条**时允许，且授权与登记必须落在 ADR（见 [ADR-0032](./adr/0032-controlled-batch-convergence.md)），不得只在 commit message 里写一句"架构师已授权"：① 范围限于**单个模块**，可回滚为一次提交；② **对外契约全程不变**并给出证据（HTTP 契约 / DTO / 返回类型 / 能力元数据 / 事件 payload，任一项变动即不适用本通道）；③ 目标模块的测试与 ArchUnit 全绿，且 freeze 基线**只收缩不新增**；④ 有**负向探针**证明被影响的规则仍在判定（不是空跑、不是靠空匹配兜底）；⑤ 收敛后的形态就是目标态本身——把 Handler 内联进同义 ApplicationService，而不是换个名字保留同一层。批量收敛是**通道不是豁免**：产物仍受全部 P/E/G 条文约束，ADR 必须写明判定依据、等价性证据与代价（如服务类方法数上升带来的拆分压力）。
...
Controller 可以直接依赖 Handler 或合法的 ApplicationService。**新代码默认选 ApplicationService 作为唯一入口**，适配器零 Handler 依赖（blueprint 样板验证：Web Controller / RPC Controller / Schedule Job 全部只依赖 ApplicationService）。存量 Handler 在迁移期间允许直接被适配器依赖，但修改时优先内联到同义 ApplicationService。下列情形禁止；它们靠语义评审判定，没有对应的机器门禁：
...
检测方式：核心判据是**该层是否承载了独立的架构能力**——独立的路由、异步边界、生命周期或多聚合协调。如果 `handle()` 的全部逻辑能被同义 `ApplicationService` 的某个方法清晰承载（共享同一套依赖、事务策略、领域事件发布），则判定为 Ceremonial，无论代码量多少。blueprint 样板已验证：`ProcessPaymentCallbackCommandHandler`（验签 + 幂等状态机 + Outbox 落库，~80 行）和 `InitiatePaymentCommandHandler`（两段式 TransactionTemplate + 远程渠道调用，~80 行）全部内联到 `PaymentApplicationService` 后，代码更聚合、依赖更收敛。评审时追问一个问题：**"这个 Handler 删掉后，ApplicationService 会多一行方法体吗？"**——如果答案是"不会，逻辑能自然融入某个现有方法"，那它就是仪式层。
...
>
> **本聚合读已合并进写侧仓储（ADR-0030，2026-09-19 启用）**：`domain/repository` 在承载写侧 + 聚合加载之外，可声明返回**域层投影类型**（`List` / `Optional` / `PageResult` 的元素为包名含 `.domain.` 的投影 / 值对象）与 `long` 计数的本聚合读方法——`bone-blueprint` 的 `OrderRepository` 即此形态：`findOrderWithItems`（`@Sql` + `MANUAL` 显式 `tenantId`，返回 `List<OrderWithItemsProjection>`）、`findOrderPage`（Criteria，返回 `PageResult<OrderHeadProjection>`）、`findExpiredUnpaidOrdersAllTenants`（`@TenantScope(ALL)` 全租户扫描）都驻留同一接口；投影类型置于 `domain/order/projection`（受 P0-4 白名单放宽容纳，见 E-4.1 / [G-1.6](#g-16-core--e-条文与门禁映射) 门禁②）。**取数归口同步前移**：合并后 blueprint **不再有 `infrastructure/query` 适配层，也不再为支付读侧保留 `QueryPort`**——应用服务直注域仓储（`PaymentApplicationService` → `PaymentRepository`，同聚合单表全租户扫描用 Criteria + `disableTenantFilter()`、方法名后缀 `AllTenants`）。代价须一并认知：域仓储同时握有 `save` / `update` / `delete`，**「读侧能力最小化」这道护栏由接口粒度降级为方法粒度**——拿到 `XxxRepository` 的调用方即握有写能力，因此「谁能持有该接口」需靠代码评审而非编译器把关（门禁只能按方法名区分读写）。约束不因此放宽：跨聚合读 / 报表 / 搜索 / 多聚合组合仍须走 `QueryPort`（见下），聚合不得透出 adapter，投影映射只在读侧 / application 发生。
...
domain 内部分组只有一种目标形态：**聚合构件全部置于 `domain/model/{聚合}/` 下**（[ADR-0036](./adr/0036-domain-model-package-single-standard.md)）：
...
- **禁止角色子目录**：不设 `domain/model/aggregate|entity|valueobject|event` 这类跨聚合角色桶——它与 [E-10.3](#e-103-按聚合的垂直切分样例)「聚合根平铺、非角色子目录」直接冲突，也会把整模块聚成一个横切大桶；
- **端口留在 `domain/` 根**：`repository` / `gateway` / `service` 以及模块自有的领域端口子包（如 blueprint 的 `extension/`）都不是模型构件，不进 `model/`；
- **同一 `domain` 包内不得并存两套分组**：`domain/{aggregate}` 与 `domain/model/{aggregate}` 并存即违规。

**存量条款**：`domain/{aggregate}` 平铺、`model/` 平铺类桶、以及 `domain/{聚合}` 与 `model/{聚合}` 双树并存，均为**存量**，按 [E-0.2](#e-02-存量不符合规范代码的处理) 触达即收敛；本轮全平台一次性统一走受控批量收敛通道，逐模块迁移清单与顺序见 [ADR-0036](./adr/0036-domain-model-package-single-standard.md)。**新代码一律按目标形态**；模块 README 的 `E-10 domain 分组形态登记` 章节登记「目标形态 + 本模块迁移状态」。
