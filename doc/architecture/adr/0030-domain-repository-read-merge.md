...
| C2 | **聚合内实体在事件订阅侧不可达**（代价） | `@Cascade` 集合保持 `@Transient`，`findById` **明确不**回填 → 订阅器拿不到明细，须经读侧投影（`OrderItemInventoryExecutor` 经 `OrderRepository.findOrderWithItems`）。结果是命令语境用查询模型取数 | **明确不做**读回填（见 [Bone-Metadata-SDK-能力需求.md](../Bone-Metadata-SDK-能力需求.md) 需求二）。**不要**把 `items` 改回可回填 |
