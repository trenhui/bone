...
- 与 Spring Security 的既有机制一致，实现成本低。
- 无状态 JWT 适合水平扩展。
...
- **As-Is（2026-05-20）**：`AccountAuthoritiesQueryHandler` 经 `iam_account_role` → `iam_role_permission` → `iam_permission.code` 解析；`LoginCommandHandler` / `refresh` 写入 JWT `scopes`；`bone-init.sql` 预置 SUPER_ADMIN 绑定；无绑定管理员回退 `DefaultPermissionCodes`。
