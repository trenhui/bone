 ...
 * 元数据服务 REST 控制器
 * <p>
 * 1. POST /v1/metadata/fields:search      — 复合查询（分页+排序+过滤）
 * 2. POST /v1/metadata/fields:searchByNames — 按名称列表精确查询
 * 3. POST /v1/metadata/fields:allocate            — 批量创建字段（事务+唯一性校验）
 * 4. GET  /v1/metadata/health            — 健康检查（公开无需认证）
 */
@RestController
...
public class MetadataController {
    ...
    /**
     * 复合查询扩展字段（分页 + 排序 + 多维过滤）
     * POST /v1/metadata/fields:search
     */
    ...
    public ResponseEntity<List<FieldMetadata>> searchFields(
            @Parameter(description = "查询条件实体", required = true, content = @Content(schema = @Schema(implementation = AllocationContext.class)))
            @Valid @RequestBody AllocationContext ctx) {
    ...
    /**
     * POST /v1/metadata/fields:searchByNames
     * 按名称精确批量查询扩展字段
     */
    ...
    @Operation(
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "查询成功",
                            content = @Content(array = @ArraySchema(schema = @Schema(implementation = FieldMetadata.class)))),
                    @ApiResponse(responseCode = "400", description = "请求参数无效"),
                    @ApiResponse(responseCode = "404", description = "未找到匹配字段")
            }
    )
    @PreAuthorize("hasAnyAuthority('metadata:read')")
    public ResponseEntity<List<FieldMetadata>> searchFieldsByNames(@Parameter(description = "查询条件实体", required = true, content = @Content(schema = @Schema(implementation = FieldsByNamesRequest.class)))
                                                                   @Valid @RequestBody FieldsByNamesRequest request) {
    ...
    /**
     * POST /v1/metadata/fields:allocate
     * 批量分配并持久化新的扩展字段
     */
    ...
    public ResponseEntity<List<FieldMetadata>> allocateAndPersistFields(@Parameter(description = "批量创建字段请求实体列表", required = true,
            content = @Content(array = @ArraySchema(schema = @Schema(implementation = FieldMetadata.class))))
                                                                        @Valid @RequestBody List<FieldMetadata> toCreate) {
        List<FieldMetadata> created = metadataService.allocateAndPersistFields(toCreate);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }

    /**
     * GET /v1/metadata/health
     * 服务健康检查（公开，无需认证）
     */
    @GetMapping("/health")
    @Operation(
