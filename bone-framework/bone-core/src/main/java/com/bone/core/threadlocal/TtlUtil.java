...
public final class TtlUtil {
     ...
     * 在当前线程中执行带上下文的任务
     */
    public static void runWithContext(Runnable task, Map<TransmittableThreadLocal<?>, Object> context) {
     ...
     */
    public static <V> V callWithContext(Callable<V> task, Map<TransmittableThreadLocal<?>, Object> context) throws Exception {
        Map<TransmittableThreadLocal<?>, Object> backup = TransmittableThreadLocal.capture();
        try {
            TransmittableThreadLocal.replay(context);
            return task.call();
