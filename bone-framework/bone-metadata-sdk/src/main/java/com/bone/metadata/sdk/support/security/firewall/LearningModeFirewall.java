...
/**
 * 智能学习模式 SQL 防火墙。
 * 首次遇到未审批的表会调用 LearningModeService.requestApproval(sql)，返回 false；
 * 审批通过后调用 approveTable(tableName) 即可放行。
 */
public class LearningModeFirewall {
    ...
    // 模式签名 -> 审批状态
    private final ConcurrentHashMap<String, ApprovalStatus> patternApprovals = new ConcurrentHashMap<>();
    ...
    public boolean validate(String sql) {
        ...
        if (learningEnabled && !allowedTables.contains(key)) {
            ...
            ApprovalStatus status = patternApprovals
                    .computeIfAbsent(signature, sig -> {
    ...
    public static class SqlSecurityException extends RuntimeException {
        public SqlSecurityException(String msg) { super(msg); }
    }
