...
- ⚠ **`Criteria.or(Consumer)` 是坏的，别用**（2026-09-20 读源码确认）：它把 OR 片段经私有 `addNativeCondition` 塞进 `new Condition(null, 片段, null, Operator.EQ)`，而 `Condition.toSql()` 对 EQ 渲染 `%s = :%s` ⇒ 产出 `m.(name LIKE :p1 OR code LIKE :p2) = :null` 这类坏 SQL（且片段里缺 `m.` 别名）。需要 OR 语义时走 `QueryBuilder.where(w -> {w.and(..); w.or(..);})`。`Criteria` 可用能力：`eq/ne/gt/gte/lt/lte/like/notLike/likeLeft/likeRight/in/notIn/orderByAsc|Desc/page(int,int)/disableTenantFilter/eqOrNull/likeIfPresent`；分页另可用 `Repository.pageByCriteria(Criteria)`（返回 `PageResult<T>`）。
- ⚠ **`..domain.repository..` 也豁免 `QueryBuilder`**（不只 `Criteria`）：`domain_no_query_builder` 判定谓词是「依赖 `@ReadSideOnly` 注解类」，`QueryBuilder` 与 `Criteria` 都带该注解，而豁免条件是包名 `..domain.repository..` ⇒ 把 `QueryBuilder` 放在该包的 `default` 方法里同样合法。2026-09-20 迁 `BoneApplicationRepository` 到 `com.bone.iam.domain.repository` 后新增 `findPage` 即走此路径（IAM 现 11 个仓储全在该包，仅 `BoneModuleRepository` 仍在 `domain.app.repository`）。
- **仓储方法白名单**（`domainRepositoriesShouldOnlyDeclareWhitelistedMethods`，只管 `..domain.repository..`）：方法名禁词 `insert/update/delete/persist/flush/merge/exists`；返回类型须为 聚合根 / `Optional<聚合根>` / 标量(void,boolean,Boolean,long,Long,int,Integer) / `List|Optional|PageResult<领域读模型>`。判定用 FQN 含 `.domain.` ⇒ `PageResult<BoneApplication>` 合规。
...
- ⚠ **外置模板文件名必须 == 方法名**：`SqlTemplateGovernanceTest` 门禁①按 `resources/sql/<接口包>/<接口简名>/<方法名>.sql` 取模板 ⇒ 重命名仓储读方法**必须 `git mv` 同名 `.sql`**，否则该方法（抽象+非 `default`）被判「无通道的裸读方法」。
- ⚠ **blueprint 的 `AllTenants` 后缀是门禁判据，重命名不可丢**：`all_tenants_scan_only_by_schedule` 用 `nameEndingWith("AllTenants")` 识别全租户入口，本地 `schedule_only_calls_all_tenants_repository_methods` 同口径（两者都只把调放宽给 `..adapter.schedule..`）。**这两条常态匹配 0 次调用 ⇒ 绿 ≠ 在工作**：改动/新增后缀名后必须打负向探针（2026-09-20 改名后两针均真报红）。现存三入口（2026-09-20 由 `CreatedExpired…`/`PayableExpired…` 等改名）：`findExpiredUnpaidOrdersAllTenants`（其模板实现 `…AllTenantsBySql`）/ `findExpiredOpenPaymentsAllTenants` / `findSettledPaymentsCreatedBeforeAllTenants`。
...
- **租户上下文端口（E-2）惯例**：端口放 `domain/gateway/TenantProvider`，实现放 `infrastructure/gateway/TenantProviderGatewayAdapter`（技术端口→`infrastructure/<能力>`，不带 `@Component` 时由模块 config 显式 `@Bean`；IAM 用 `@Component`）。先例 metadata-server（其实现把缺失回退成 `1`，**IAM 刻意不回退**——保留 `null` 以区分「未认证/租户未知」与「平台租户 0」，否则未认证查询会被静默翻译成以某租户身份查询）。
