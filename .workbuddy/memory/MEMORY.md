...
- `.claude/agents/` 是**喂 AI 的文件**（`skills/*.md` + `agents/*.md`），**不在任何门禁覆盖内**，极易长期过期。2026-09-19 已重写三份 skills（`backend-java` / `db-migration` / `frontend-react`，原本全是 MyBatis / Flyway / Umi 的过期论断）并修 `engineer.md`（MyBatis "mapper" 切片 + CLAUDE.md 引用）/`guardian.md`（把 80% 当项目覆盖率红线）。**日后改动这些文件须人工对齐真源**：`backend-java`→`doc/agents/03`+SDK；`db-migration`→`doc/deployment/sql/README.md`（**无** Flyway/Liquibase）；`frontend-react`→`doc/architecture/bone-前端架构.md`（Vite+Qiankun+`@bone/ui`）。

## 10. ADR-0030 已采纳：写侧仓储合并本聚合读（2026-09-19 落地）
- **状态**：ADR-0030 **已采纳**，blueprint P2 试点已落地验证；`OrderQueryPort` / `OrderReadRepository` 已**删除**，订单读方法（`@Sql` + Criteria）与域层投影（`domain/order/projection/`）并入 `OrderRepository`，应用层**直注域仓储**（方案 X，无端口）。`PaymentQueryPort` / `PaymentQueryAdapter` 保持不变。规范 E-4.1 / E-4.2 / CORE-05 已按此改写（同批）。
- **SDK 能力真源（已核验代码）**：`@TenantScope` 用**锚点标记** `/*bone:tenant*/` 由 `TenantSqlRewriter` 替换（**非** MyBatis 占位符、**非** jsqlparser）；默认 `MANUAL`（向后兼容，**不是**"缺标注即启动报错"）；软删**不自动注入**，须手写且 JOIN 子表都要带。外置模板路径 `sql/<包>/<Class>/<方法>.sql`；`SqlConfigProperties.loadPriority` 默认已是 **`classpath-first`**（`fallbackEnabled=true`）⇒ 外置优先、`@Sql` 兜底。
- **规范 / 门禁文档坑**：① `Bone-DDD-最终实践方案.md` 正文**禁用 `R[1-9]`**（`check-ddd-doc-drift.py` 判 v4 残留）⇒ ADR 的 R 编号在正文须改称「门禁①–⑥」；② 正文引用**尚未落地**的 ArchUnit 规则名会被 `check-ddd-gate-state.py` 判假 ⇒ 必须登记该脚本 `KNOWN_MISSING`（本次登记 4 条 P3 规则名）；③ 规范正文的 ArchUnit 规则名/编号改动后**三道脚本都要重跑**。
- **门禁落地度（09-19 收官）**：R1–R6 **全部有机器载体**。R2 = P0-4 `domainRepositoriesShouldOnlyDeclareWhitelistedMethods` 白名单放宽（容纳 `List`/`Optional`/`PageResult` 承载 `.domain.` 投影 + `long` 计数）；R5 = **同一条 ArchUnit 规则改造**（新增 `isReadSideRepositoryMethod`，`default`/`@Sql` 方法按读侧排除，否则"写 + 本聚合读"合并后误算第二聚合）；**R1/R3/R4/R6 = blueprint 模块级治理测试 `bone-blueprint/src/test/java/com/bone/blueprint/SqlTemplateGovernanceTest.java`**——**不是 ArchUnit 规则**，因其要读外置 `.sql` / SQL 文本 `FROM`/`JOIN` 表集合 / README 的 E-1.2 声明，"同模板 ID 是否并存 `@Sql` 与 `.sql`"这类跨 Java+资源判定不在字节码里；仍由 `mvn test` 执行。R6 的表集合来自 blueprint `README.md` 新增的 `### 本上下文拥有的表（E-1.2 数据所有权声明）`（`表名|用途|归属聚合`）。G-1.6 的 6 行写真实载体名（不写虚构 ArchUnit 规则名）；`check-ddd-gate-state.py` 的 `KNOWN_MISSING` 里 4 条虚构规则名已删。
- **blueprint 采集器**：`tools/blueprint-compliance-collector/collect.py` 的 `read-port` 检查项已按合并形态改写（旧 sql 路径 `sql/order/...` 曾致 sql evidence 恒为空）。改模块源码后必须 `collect.py` 重算 `doc/_generated/blueprint/` 并以 `--check` 验证。
- **SDK 文档已补 `@TenantScope`（09-19）**：速查手册新增 §3.5（四模式表 + 锚点 + 失败关闭 + `softDelete` 仅单表）、使用指南新增 §6.3.1、最佳实践 §6.7 重写、§8.1/§14/§15 同步。可复用事实（已核源码）：默认回退链 = `sql/<包路径>/<接口简名>/<方法名>.sql` → `sql-templates/<接口简名>/<方法名>.yaml` → 注解 `@Sql`；`RepositoryFactoryBean#invoke` **仅 `AUTO`** 时调 `TenantSqlRewriter`（值取 `TenantContext.getTenantIdAsLong()`），故注解与外置 `.sql` 同等生效；AUTO 无锚点 + JOIN → `IllegalStateException`，缺上下文 → `MissingTenantContextException`；`softDelete=true` 追加**裸列名** `AND deleted = 0`（仅单表安全）。已纠正两处旧文档事实错误：`load-priority` 默认是 **`classpath-first`**（非 `annotation-first`）、yaml 路径是 `sql-templates/<接口简名>/<方法名>.yaml`（非拼接）。
- ⚠ **SDK 文档不在任何 DDD/CI 门禁覆盖内**：`check-ddd-doc-drift.py` 只扫 `doc/architecture/Bone-DDD-最终实践方案.md`（1 个文档），故 `bone-engine/bone-metadata-sdk/doc/` 的漂移 CI 不可见，须人工核对。
